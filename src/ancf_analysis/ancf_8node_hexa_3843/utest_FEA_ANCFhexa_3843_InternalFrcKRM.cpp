...
bool ANCFBrickTest<ElementVersion, MaterialVersion>::GeneralizedInternalForceSmallDispNoVelCheck(int msglvl) {
    ...
    if (!load_validation_data("UT_ANCFBrick_3843_IntFrcSmallDispNoVel.txt", Expected_InternalForceSmallDispNoVel))
        return false;
...
bool ANCFBrickTest<ElementVersion, MaterialVersion>::GeneralizedInternalForceNoDispSmallVelCheck(int msglvl) {
    ...
    if (!load_validation_data("UT_ANCFBrick_3843_IntFrcNoDispSmallVel.txt", Expected_InternalForceNoDispSmallVel))
        return false;
...
bool ANCFBrickTest<ElementVersion, MaterialVersion>::AxialDisplacementCheck(int msglvl) {
    ...
    if (msglvl >= 1) {
        ...
        std::cout << "Axial Pull Test - Angular misalignment Checks (all angles less than 0.1 deg)";
        if (passed_angles)
            print_green(" - Test PASSED\n\n");
        else
            print_red(" - Test FAILED\n\n");
...
template <typename ElementVersion, typename MaterialVersion>
bool ANCFBrickTest<ElementVersion, MaterialVersion>::CantileverTipLoadCheck(int msglvl) {
    // =============================================================================
    ...
    if (msglvl >= 1) {
        ...
        std::cout << "Cantilever Beam (Tip Load) - Off-axis Angular misalignment Checks (all angles less than 0.1 deg)";
        if (passed_angles)
            print_green(" - Test PASSED\n\n");
        else
            print_red(" - Test FAILED\n\n");
...
template <typename ElementVersion, typename MaterialVersion>
bool ANCFBrickTest<ElementVersion, MaterialVersion>::CantileverGravityCheck(int msglvl) {
    // =============================================================================
...
bool ANCFBrickTest<ElementVersion, MaterialVersion>::AxialTwistCheck(int msglvl) {
    ...
    bool passed_twist = abs(Percent_Error) < 5.0;
    // check the off-axis angles which should be zeros
    ...
    if (msglvl >= 1) {
        std::cout << "Axial Twist - Twist Angle Check (Percent Error less than 5%) = " << Percent_Error << "%";
        if (passed_twist)
            print_green(" - Test PASSED\n");
        else
            print_red(" - Test FAILED\n");
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "-------------------------------------" << std::endl;
    ANCFBrickTest<ChElementHexaANCF_3843_TR01, ChMaterialHexaANCF> ChElementHexaANCF_3843_TR01_test;
    if (ChElementHexaANCF_3843_TR01_test.RunElementChecks(0))
        print_green("ChElementHexaANCF_3843_TR01 Element Checks = PASSED\n");
    else
