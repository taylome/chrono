...
// =============================================================================
//
// Unit test for the formulation of the fully parameterized 8 node ANCF brick element 3833.
//
...
ANCFBrickTest::ANCFBrickTest(bool useContInt) {
    ...
    // Enable the efficient ANCF method for calculating the application of gravity to the element.  The default method
    // for applying gravity to the mesh must be turned off so that the force due to gravity is not double accounted for.
...
bool ANCFBrickTest::MassMatrixCheck(int msglvl) {
    // =============================================================================
...
bool ANCFBrickTest::GeneralizedGravityForceCheck(int msglvl) {
    // =============================================================================
    ...
    // For efficiency, a fast ANCF specific generalized force due to gravity is optionally included with the generalized
    // internal force calculation.  The contribution from generalized force due to gravity is calculated by calculating
    // the difference between the generalized internal force vector with and without gravity enable.  This isolates any
    // potential errors coming from the generalized internal force calculation and it ensures that the generalized force
    // due to gravity is disabled corrected when switched off.
    ChVectorDynamic<double> InternalForceNoDispNoVelWithGravity;
    InternalForceNoDispNoVelWithGravity.resize(3 * NSF);
    ...
    InternalForceNoDispNoVelNoGravity.resize(3 * NSF);
    m_element->SetGravityOn(false);
    m_element->ComputeInternalForces(InternalForceNoDispNoVelNoGravity);

    ChVectorDynamic<double> InternalForceDueToGravity =
        InternalForceNoDispNoVelWithGravity - InternalForceNoDispNoVelNoGravity;
...
bool ANCFBrickTest::GeneralizedInternalForceNoDispNoVelCheck(int msglvl) {
    // =============================================================================
...
bool ANCFBrickTest::GeneralizedInternalForceSmallDispNoVelCheck(int msglvl) {
    // =============================================================================
    ...
    Expected_InternalForceSmallDispNoVelNoGravity.resize(3 * NSF, 1);
    if (!load_validation_data("UT_ANCFBrick_3843_IntFrcSmallDispNoVelNoGravity.txt",
                              Expected_InternalForceSmallDispNoVelNoGravity))
        return false;
    ...
    InternalForceSmallDispNoVelNoGravity.resize(3 * NSF);
    m_element->SetGravityOn(false);
    ...
    double MaxAbsError =
        (InternalForceSmallDispNoVelNoGravity - Expected_InternalForceSmallDispNoVelNoGravity).cwiseAbs().maxCoeff();
    ...
    if (msglvl >= 1) {
        std::cout << "Generalized Internal Force - Small Displacement, No Velocity (Max Abs Error) = " << MaxAbsError
                  << " (" << MaxAbsError / Expected_InternalForceSmallDispNoVelNoGravity.cwiseAbs().maxCoeff() * 100
                  << "%)";
...
bool ANCFBrickTest::GeneralizedInternalForceNoDispSmallVelCheck(int msglvl) {
    // =============================================================================
    ...
    Expected_InternalForceNoDispSmallVelNoGravity.resize(3 * NSF, 1);
    if (!load_validation_data("UT_ANCFBrick_3843_IntFrcNoDispSmallVelNoGravity.txt",
                              Expected_InternalForceNoDispSmallVelNoGravity))
        return false;
    ...
    InternalForceNoDispSmallVelNoGravity.resize(3 * NSF);
    m_element->SetGravityOn(false);
    ...
    double MaxAbsError =
        (InternalForceNoDispSmallVelNoGravity - Expected_InternalForceNoDispSmallVelNoGravity).cwiseAbs().maxCoeff();
    ...
    if (msglvl >= 1) {
        std::cout << "Generalized Internal Force - Small Displacement, No Velocity (Max Abs Error) = " << MaxAbsError
                  << " (" << MaxAbsError / Expected_InternalForceNoDispSmallVelNoGravity.cwiseAbs().maxCoeff() * 100
                  << "%)";
...
bool ANCFBrickTest::JacobianNoDispNoVelNoDampingCheck(int msglvl) {
    // =============================================================================
...
bool ANCFBrickTest::JacobianSmallDispNoVelNoDampingCheck(int msglvl) {
    // =============================================================================
    ...
    if (!load_validation_data("UT_ANCFBrick_3843_JacSmallDispNoVelNoDamping.txt",
...
bool ANCFBrickTest::JacobianNoDispNoVelWithDampingCheck(int msglvl) {
    // =============================================================================
...
bool ANCFBrickTest::JacobianSmallDispNoVelWithDampingCheck(int msglvl) {
    // =============================================================================
...
bool ANCFBrickTest::JacobianNoDispSmallVelWithDampingCheck(int msglvl) {
    // =============================================================================
...
bool ANCFBrickTest::AxialDisplacementCheck(int msglvl) {
    // =============================================================================
    ...
    for (int i = 1; i <= num_elements; i++) {
        ...
        element->SetMaterial(material);
        element->SetAlphaDamp(0.0);
        // Enable the efficient ANCF method for calculating the application of gravity to the element (note that the
        // acceleration due to gravity is zero for this test, so this setting could also have been set to false.)  In
        // general, the default mesh level gravity calculation needs to be disabled if this setting is enable to prevent
        // double counting the force due to gravity.
    ...
    class MyLoaderTimeDependentTipLoad : public ChLoaderUVWatomic {
      public:
        ...
        MyLoaderTimeDependentTipLoad(std::shared_ptr<ChLoadableUVW> mloadable)
...
bool ANCFBrickTest::CantileverCheck(int msglvl) {
    // =============================================================================
    ...
    if (msglvl >= 2) {
        std::cout << "Cantilever Beam - ANCF Tip Position: " << point << "m" << std::endl;
        std::cout << "Cantilever Beam - ANCF Tip Displacement: " << Displacement_Model << "m" << std::endl;
        std::cout << "Cantilever Beam - Analytical Tip Displacement: " << Displacement_Theory << "m" << std::endl;
        std::cout << "Cantilever Beam - ANCF Tip Angles: (" << Tip_Angles.x() * CH_C_RAD_TO_DEG << ", "
                  << Tip_Angles.y() * CH_C_RAD_TO_DEG << ", " << Tip_Angles.z() * CH_C_RAD_TO_DEG << ")deg"
                  << std::endl;
    }
    if (msglvl >= 1) {
        std::cout << "Cantilever Beam - Tip Displacement Check (Percent Error less than 5%) = " << Percent_Error << "%";
        if (passed_displacement)
            print_green(" - Test PASSED\n");
        else
            print_red(" - Test FAILED\n");

        std::cout << "Cantilever Beam - Off-axis Angular misalignment Checks (all angles less than 0.001 deg)";
        if (passed_angles)
            print_green(" - Test PASSED\n\n");
        else
            print_red(" - Test FAILED\n\n");
...
bool ANCFBrickTest::AxialTwistCheck(int msglvl) {
    // =============================================================================
