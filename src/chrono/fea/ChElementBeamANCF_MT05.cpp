...
// =============================================================================
//
// MT05 = MT04 + optimizations for stiffness tensor calcs, but still no pre-computation
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT05::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT05::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT05::ChElementBeamANCF_MT05() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT05::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT05::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT05::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT05::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT05::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementBeamANCF_MT05::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT05::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT05::ComputeNodalMass() {
...
void ChElementBeamANCF_MT05::SetAlphaDamp(double a) {
    m_Alpha = a;
...
void ChElementBeamANCF_MT05::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ...
    if (GetStrainFormulation() == ChElementBeamANCF_MT05::StrainFormulation::CMPoisson) {
        for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
...
void ChElementBeamANCF_MT05::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, double xi, double eta, double zeta, ChVectorN<double, 6>& D0, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT05::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, double xi, double eta, double zeta, ChMatrix33<double>& Dv, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChMatrix33<double> F;                   //Deformation Gradient tensor (non-symmetric tensor)
    ...
    //      thin three-dimensional beam elements in the absolute nodal coordinate formulation, 
    //      Proceedings of Multibody Dynamics 2005 ECCOMAS Thematic Conference, Madrid, Spain, 2005.
    double det_0xi = J_0xi.determinant();
    P_transpose_scaled.row(0).noalias() = -det_0xi*F.col(0)*S(0);
    P_transpose_scaled.row(1).noalias() = -det_0xi*F.col(1)*S(1);
    P_transpose_scaled.row(2).noalias() = -det_0xi*F.col(2)*S(2);
...
void ChElementBeamANCF_MT05::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
...
void ChElementBeamANCF_MT05::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT05::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT05::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT05::Calc_Sxi_D(ChMatrixNM<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT05::CalcCoordMatrix(ChMatrixNM<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT05::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT05::CalcCoordDerivMatrix(ChMatrixNM<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT05::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT05::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT05::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNM<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT05::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT05::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
//}
//
// void ChElementBeamANCF_MT05::EvaluateSectionFrame(const double u,
//                                             const double v,
//                                             ChVector<>& point,
//                                             ChQuaternion<>& rot) {
//    // this is not a corotational element, so just do:
...
//}
//
// void ChElementBeamANCF_MT05::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT05::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT05::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT05::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT05::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT05::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT05::ComputeNF(
...
double ChElementBeamANCF_MT05::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT05::ComputeTangent(const double U) {
    ChMatrixNM<double, 9, 3> e_bar;
    ChMatrixNM<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT05(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT05::GetStaticGQTables() {
    return &static_tables_MT05;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT05 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT05::ChMaterialBeamANCF_MT05(double rho,        // material density
...
void ChMaterialBeamANCF_MT05::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
