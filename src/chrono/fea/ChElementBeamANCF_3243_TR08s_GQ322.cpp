...
void ChElementBeamANCF_3243_TR08S_GQ322::PrecomputeInternalForceMatricesWeights() {
    ...
    ChMatrixNMc<double, 8, 36> SD_precompute_D0;
    ChMatrixNMc<double, 8, 9> SD_precompute_Dv;
...
void ChElementBeamANCF_3243_TR08S_GQ322::ComputeInternalJacobianDamping(ChMatrixRef& H,
    ...
    ChMatrixNM<double, 24, 3> ChunkSDvA;
    ChMatrixNM<double, 24, 3> ChunkSDvB;
    ChMatrixNM<double, 24, 3> ChunkSDvC;
    ...
    Scaled_Combined_partial_epsilon_partial_e_Transpose.block<24, 3>(0, 72).noalias() =
        Dv(0, 0) * ChunkSDvA + Dv(0, 1) * ChunkSDvB + Dv(0, 2) * ChunkSDvC;
    Scaled_Combined_partial_epsilon_partial_e_Transpose.block<24, 3>(0, 75).noalias() =
        Dv(1, 0) * ChunkSDvA + Dv(1, 1) * ChunkSDvB + Dv(1, 2) * ChunkSDvC;
    Scaled_Combined_partial_epsilon_partial_e_Transpose.block<24, 3>(0, 78).noalias() =
    ...
    for (unsigned int i = 0; i < 8; i++) {
        Jacobian_CompactPart_Line.block(0, i, 1, 8 - i) = Kfactor * m_SD_precompute_col_ordered.block<1, 45>(i, 0) * S_scaled_SD_precompute_col_ordered.block(i, 0, 8 - i, 45).transpose();
