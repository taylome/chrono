...
// =============================================================================
// Higher order ANCF shell element with 8 nodes. Description of this element (3833_TR06B) and its internal forces may be
...
// Constructor
// ------------------------------------------------------------------------------
ChElementShellANCF_3833_TR06B::ChElementShellANCF_3833_TR06B()
...
void ChElementShellANCF_3833_TR06B::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementShellANCF_3833_TR06B::AddLayer(double thickness,
                                             double theta,
...
void ChElementShellANCF_3833_TR06B::SetDimensions(double lenX, double lenY) {
    m_lenX = lenX;
    m_lenY = lenY;
}
...
void ChElementShellANCF_3833_TR06B::SetMidsurfaceOffset(const double offset) {
    m_midsurfoffset = offset;
}

// Set the value for the single term structural damping coefficient.

void ChElementShellANCF_3833_TR06B::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementShellANCF_3833_TR06B::GetGreenLagrangeStrain(const double xi,
                                                                   const double eta,
                                                                   const double zeta) {
...
ChMatrix33<> ChElementShellANCF_3833_TR06B::GetPK2Stress(const double layer,
                                                         const double xi,
                                                         const double eta,
...
double ChElementShellANCF_3833_TR06B::GetVonMissesStress(const double layer,
                                                         const double xi,
                                                         const double eta,
                                                         const double layer_zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
...
void ChElementShellANCF_3833_TR06B::SetupInitial(ChSystem* system) {
...
void ChElementShellANCF_3833_TR06B::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3833_TR06B::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementShellANCF_3833_TR06B::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementShellANCF_3833_TR06B::ComputeNodalMass() {
...
void ChElementShellANCF_3833_TR06B::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3833_TR06B::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                             double Kfactor,
                                                             double Rfactor,
                                                             double Mfactor) {
    ...
    ChVectorN<double, (NSF * (NSF + 1)) / 2> H_CompactPart = Mfactor * m_MassMatrix;
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (unsigned int GQpnt = 0; GQpnt < NIP; GQpnt++) {
            ...
            for (unsigned int i = 0; i < NSF; i++) {
                for (unsigned int j = i; j < NSF; j++) {
                    H_CompactPart(idx) -= S_SD.row(i).dot(Sbar_xi_D.row(j));
                    idx++;
                }
            }
    ...
    for (unsigned int j = 0; j < NSF; j++) {
        for (unsigned int i = j; i < NSF; i++) {
            H(3 * i, 3 * j) += H_CompactPart(idx);
            H(3 * i + 1, 3 * j + 1) += H_CompactPart(idx);
            H(3 * i + 2, 3 * j + 2) += H_CompactPart(idx);
            if (i != j) {
                H(3 * j, 3 * i) += H_CompactPart(idx);
                H(3 * j + 1, 3 * i + 1) += H_CompactPart(idx);
                H(3 * j + 2, 3 * i + 2) += H_CompactPart(idx);
            }
            idx++;
        }
    }
...
void ChElementShellANCF_3833_TR06B::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementShellANCF_3833_TR06B::EvaluateSectionFrame(const double xi,
                                                         const double eta,
                                                         ChVector<>& point,
                                                         ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementShellANCF_3833_TR06B::EvaluateSectionPoint(const double xi, const double eta, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR06B::EvaluateSectionVelNorm(const double xi, const double eta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR06B::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3833_TR06B::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3833_TR06B::LoadableStateIncrement(const unsigned int off_x,
                                                           ChState& x_new,
                                                           const ChState& x,
...
void ChElementShellANCF_3833_TR06B::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3833_TR06B::ComputeNF(
...
double ChElementShellANCF_3833_TR06B::GetDensity() {
...
ChVector<> ChElementShellANCF_3833_TR06B::ComputeNormal(const double xi, const double eta) {
...
void ChElementShellANCF_3833_TR06B::ComputeMassMatrixAndGravityForce() {
...
void ChElementShellANCF_3833_TR06B::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementShellANCF_3833_TR06B::Calc_Sxi_compact(VectorN& Sxi_compact,
                                                     double xi,
                                                     double eta,
                                                     double zeta,
                                                     double thickness,
...
void ChElementShellANCF_3833_TR06B::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact,
                                                        double xi,
                                                        double eta,
                                                        double zeta,
                                                        double thickness,
...
void ChElementShellANCF_3833_TR06B::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta,
                                                         double thickness,
...
void ChElementShellANCF_3833_TR06B::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                          double xi,
                                                          double eta,
                                                          double zeta,
                                                          double thickness,
...
void ChElementShellANCF_3833_TR06B::Calc_Sxi_D(MatrixNx3c& Sxi_D,
                                               double xi,
                                               double eta,
                                               double zeta,
                                               double thickness,
...
void ChElementShellANCF_3833_TR06B::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3833_TR06B::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3833_TR06B::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3833_TR06B::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3833_TR06B::CalcCombinedCoordMatrix(Matrix6xN& ebar_ebardot) {
...
void ChElementShellANCF_3833_TR06B::Calc_J_0xi(ChMatrix33<double>& J_0xi,
                                               double xi,
                                               double eta,
                                               double zeta,
                                               double thickness,
...
double ChElementShellANCF_3833_TR06B::Calc_det_J_0xi(double xi,
                                                     double eta,
                                                     double zeta,
                                                     double thickness,
                                                     double zoffset) {
...
void ChElementShellANCF_3833_TR06B::RotateReorderStiffnessMatrix(ChMatrixNM<double, 6, 6>& D, double theta) {
...
ChQuadratureTables static_tables_3833_TR06B(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3833_TR06B::GetStaticGQTables() {
    return &static_tables_3833_TR06B;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3833_TR06B::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)

ChElementShellANCF_3833_TR06B::Layer::Layer(double thickness,
                                            double theta,
                                            std::shared_ptr<ChMaterialShellANCF> material)
