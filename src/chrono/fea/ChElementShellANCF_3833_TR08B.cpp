...
// =============================================================================
// Higher order ANCF shell element with 8 nodes. Description of this element (3833_TR08B) and its internal forces may be
...
//
// =============================================================================
// TR08B = Vectorized Continuous Integration without Data Caching for the Jacobian
...
void ChElementShellANCF_3833_TR08B::AddLayer(double thickness,
                                             double theta,
...
void ChElementShellANCF_3833_TR08B::SetDimensions(double lenX, double lenY) {
    m_lenX = lenX;
    m_lenY = lenY;
}
...
void ChElementShellANCF_3833_TR08B::SetMidsurfaceOffset(const double offset) {
...
ChMatrix33<> ChElementShellANCF_3833_TR08B::GetGreenLagrangeStrain(const double xi,
                                                                   const double eta,
                                                                   const double zeta) {
...
ChMatrix33<> ChElementShellANCF_3833_TR08B::GetPK2Stress(const double layer,
                                                         const double xi,
                                                         const double eta,
...
double ChElementShellANCF_3833_TR08B::GetVonMissesStress(const double layer,
                                                         const double xi,
                                                         const double eta,
                                                         const double layer_zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
...
void ChElementShellANCF_3833_TR08B::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    // Zero out the Fi since the contribution from each GQ point will be added to it
    Fi.setZero();
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        FiMatrixForm.noalias() += m_SD.block<NSF, 3 * NIP>(kl * NSF, 0) * P_Block;
    }
}
...
void ChElementShellANCF_3833_TR08B::ComputeKRMmatricesGlobal(ChMatrixRef H,
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (unsigned int i = 0; i < NSF; i++) {
            ...
            for (unsigned int j = i; j < NSF; j++) {
                H_CompactPart(idx) -= Kfactor * (S_scaled_SD_row_i.dot(m_SD.row(j + kl * NSF)));
                idx++;
            }
        }
    }
...
void ChElementShellANCF_3833_TR08B::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementShellANCF_3833_TR08B::EvaluateSectionFrame(const double xi,
                                                         const double eta,
                                                         ChVector<>& point,
                                                         ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementShellANCF_3833_TR08B::EvaluateSectionPoint(const double xi, const double eta, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR08B::EvaluateSectionVelNorm(const double xi, const double eta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
ChVector<> ChElementShellANCF_3833_TR08B::ComputeNormal(const double xi, const double eta) {
...
void ChElementShellANCF_3833_TR08B::ComputeMassMatrixAndGravityForce() {
...
void ChElementShellANCF_3833_TR08B::Calc_Sxi_compact(VectorN& Sxi_compact,
                                                     double xi,
                                                     double eta,
                                                     double zeta,
                                                     double thickness,
...
void ChElementShellANCF_3833_TR08B::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact,
                                                        double xi,
                                                        double eta,
                                                        double zeta,
                                                        double thickness,
...
void ChElementShellANCF_3833_TR08B::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta,
                                                         double thickness,
...
void ChElementShellANCF_3833_TR08B::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                          double xi,
                                                          double eta,
                                                          double zeta,
                                                          double thickness,
...
void ChElementShellANCF_3833_TR08B::Calc_Sxi_D(MatrixNx3c& Sxi_D,
                                               double xi,
                                               double eta,
                                               double zeta,
                                               double thickness,
...
void ChElementShellANCF_3833_TR08B::CalcCombinedCoordMatrix(Matrix6xN& ebar_ebardot) {
...
void ChElementShellANCF_3833_TR08B::Calc_J_0xi(ChMatrix33<double>& J_0xi,
                                               double xi,
                                               double eta,
                                               double zeta,
                                               double thickness,
...
double ChElementShellANCF_3833_TR08B::Calc_det_J_0xi(double xi,
                                                     double eta,
                                                     double zeta,
                                                     double thickness,
                                                     double zoffset) {
...
void ChElementShellANCF_3833_TR08B::RotateReorderStiffnessMatrix(ChMatrixNM<double, 6, 6>& D, double theta) {
...
// ============================================================================
// Implementation of ChElementShellANCF_3833_TR08B::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)

ChElementShellANCF_3833_TR08B::Layer::Layer(double thickness,
                                            double theta,
                                            std::shared_ptr<ChMaterialShellANCF> material)
