...
//
// =============================================================================
// TR06B = Gerstmayr and Shabana with Precomputation and Analytic Jacobian
...
ChElementBeamANCF_3243_TR06B::ChElementBeamANCF_3243_TR06B()
...
void ChElementBeamANCF_3243_TR06B::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementBeamANCF_3243_TR06B::SetDimensions(double lenX, double thicknessY, double thicknessZ) {
    m_lenX = lenX;
...
void ChElementBeamANCF_3243_TR06B::SetMaterial(std::shared_ptr<ChMaterialBeamANCF> beam_mat) {
    m_material = beam_mat;
}

// Set the value for the single term structural damping coefficient.

void ChElementBeamANCF_3243_TR06B::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementBeamANCF_3243_TR06B::GetGreenLagrangeStrain(const double xi,
                                                                  const double eta,
                                                                  const double zeta) {
...
ChMatrix33<> ChElementBeamANCF_3243_TR06B::GetPK2Stress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBeamANCF_3243_TR06B::GetVonMissesStress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
void ChElementBeamANCF_3243_TR06B::SetupInitial(ChSystem* system) {
...
void ChElementBeamANCF_3243_TR06B::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_3243_TR06B::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementBeamANCF_3243_TR06B::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementBeamANCF_3243_TR06B::ComputeNodalMass() {
...
void ChElementBeamANCF_3243_TR06B::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    for (unsigned int GQpnt = 0; GQpnt < NIP_Dv; GQpnt++) {
        ...
        // Calculate the transpose of the (1st Piola Kirchhoff Stress tensor = F*SPK2) scaled by the negative of the
        // determinate of the element Jacobian.  Note that SPK2 is symmetric.
        FiMatrixForm.noalias() += Sbar_xi_D * (epsilon_combined.asDiagonal() * F.transpose());
    }
}
...
void ChElementBeamANCF_3243_TR06B::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                            double Kfactor,
                                                            double Rfactor,
                                                            double Mfactor) {
    ...
    for (unsigned int GQpnt = 0; GQpnt < NIP_Dv; GQpnt++) {
        ...
        // Calculate the contribution from the Mass Matrix and expand(SD*SPK2*SD')
        // Only the upper triangular values are unique, so only those values are calculated here
        MatrixNx3c S_SD = Sbar_xi_D * SPK2.asDiagonal();
        unsigned int idx = 0;
        for (unsigned int i = 0; i < NSF; i++) {
            for (unsigned int j = i; j < NSF; j++) {
                Jac_CompactPart(idx) -= S_SD.row(i).dot(Sbar_xi_D.row(j));
    ...
    for (unsigned int j = 0; j < NSF; j++) {
        for (unsigned int i = j; i < NSF; i++) {
            H(3 * i, 3 * j) += Jac_CompactPart(idx);
            H(3 * i + 1, 3 * j + 1) += Jac_CompactPart(idx);
            H(3 * i + 2, 3 * j + 2) += Jac_CompactPart(idx);
            if (i != j) {
                H(3 * j, 3 * i) += Jac_CompactPart(idx);
                H(3 * j + 1, 3 * i + 1) += Jac_CompactPart(idx);
                H(3 * j + 2, 3 * i + 2) += Jac_CompactPart(idx);
            }
            idx++;
        }
    }
...
void ChElementBeamANCF_3243_TR06B::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementBeamANCF_3243_TR06B::EvaluateSectionFrame(const double xi, ChVector<>& point, ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementBeamANCF_3243_TR06B::EvaluateSectionPoint(const double xi, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR06B::EvaluateSectionVel(const double xi, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR06B::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_3243_TR06B::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_3243_TR06B::LoadableStateIncrement(const unsigned int off_x,
                                                          ChState& x_new,
                                                          const ChState& x,
...
void ChElementBeamANCF_3243_TR06B::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_3243_TR06B::ComputeNF(
...
double ChElementBeamANCF_3243_TR06B::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at coordinate xi [-1 to 1].

ChVector<> ChElementBeamANCF_3243_TR06B::ComputeTangent(const double xi) {
    VectorN Sxi_xi_compact;
    Calc_Sxi_xi_compact(Sxi_xi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR06B::ComputeMassMatrixAndGravityForce() {
...
void ChElementBeamANCF_3243_TR06B::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_3243_TR06B::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR06B::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR06B::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR06B::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta) {
...
void ChElementBeamANCF_3243_TR06B::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
    VectorN Sxi_D_col;
...
void ChElementBeamANCF_3243_TR06B::CalcCoordVector(Vector3N& e) {
...
void ChElementBeamANCF_3243_TR06B::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementBeamANCF_3243_TR06B::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementBeamANCF_3243_TR06B::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementBeamANCF_3243_TR06B::CalcCombinedCoordMatrix(Matrix6xN& ebar_ebardot) {
...
void ChElementBeamANCF_3243_TR06B::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBeamANCF_3243_TR06B::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
ChQuadratureTables static_tables_3243_TR06B(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_3243_TR06B::GetStaticGQTables() {
    return &static_tables_3243_TR06B;
}
