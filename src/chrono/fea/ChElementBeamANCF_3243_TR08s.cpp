...
#define CHELEMENTBEAMANCF3243TR08S_UT 1
...
ChElementBeamANCF_3243_TR08S::ChElementBeamANCF_3243_TR08S()
...
void ChElementBeamANCF_3243_TR08S::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementBeamANCF_3243_TR08S::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_3243_TR08S::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_3243_TR08S::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_3243_TR08S::ComputeKRMmatricesGlobal(ChMatrixRef H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
...
void ChElementBeamANCF_3243_TR08S::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
#ifdef CHELEMENTBEAMANCF3243TR08S_UT
    //Upper Triangular
    unsigned int idx = 0;
    for (unsigned int i = 0; i < 8; i++) {
        for (unsigned int j = i; j < 8; j++) {
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_3243_TR08S::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
    ...
    ChMatrixNM<double, 8, 8> MassMatrixCompactSquare;
...
void ChElementBeamANCF_3243_TR08S::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    unsigned int GQ_idx_xi = 3;        // 4 Point Gauss-Quadrature;
    unsigned int GQ_idx_eta_zeta = 1;  // 2 Point Gauss-Quadrature;

    ChMatrixNMc<double, 8, 48> SD_precompute_D0;
    ChMatrixNMc<double, 8, 12> SD_precompute_Dv;
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_eta_zeta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_eta_zeta].size(); it_zeta++) {
                ...
                SD_precompute_D0.block(0, 3 * index, 8, 3) = SD;
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
        ...
        ChMatrixNMc<double, 8, 3> SD = Sxi_D * J_0xi.inverse();
        SD_precompute_Dv.block(0, 3 * it_xi, 8, 3) = SD;
        m_GQWeight_det_J_0xi_Dv(it_xi) = -J_0xi.determinant() * GQ_weight;
    }

    for (auto i = 0; i < 16; i++) {
        m_SD_precompute_col_ordered.col(i) = SD_precompute_D0.col(3 * i);
        m_SD_precompute_col_ordered.col(i + 16) = SD_precompute_D0.col(3 * i + 1);
        m_SD_precompute_col_ordered.col(i + 32) = SD_precompute_D0.col(3 * i + 2);
    }
    for (auto i = 0; i < 4; i++) {
        m_SD_precompute_col_ordered.col(i + 48) = SD_precompute_Dv.col(3 * i);
        m_SD_precompute_col_ordered.col(i + 52) = SD_precompute_Dv.col(3 * i + 1);
        m_SD_precompute_col_ordered.col(i + 56) = SD_precompute_Dv.col(3 * i + 2);
    }
}

/// This class computes and adds corresponding masses to ElementGeneric member m_TotalMass
void ChElementBeamANCF_3243_TR08S::ComputeNodalMass() {
...
void ChElementBeamANCF_3243_TR08S::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementBeamANCF_3243_TR08S::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBeamANCF_3243_TR08S::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
    ...
    ChVectorN<double, 16> SPK2_0_D0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(0, 0).cwiseProduct(
    ...
    ChVectorN<double, 16> SPK2_1_D0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(16, 0).cwiseProduct(
    ...
    ChVectorN<double, 16> SPK2_2_D0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(32, 0).cwiseProduct(
    ...
    ChVectorN<double, 16> SPK2_3_D0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(16, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(32, 0));
    ...
    ChVectorN<double, 16> SPK2_4_D0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(32, 0));
    ...
    ChVectorN<double, 16> SPK2_5_D0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<16, 1>(16, 0));
...
void ChElementBeamANCF_3243_TR08S::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
...
void ChElementBeamANCF_3243_TR08S::ComputeInternalJacobianDamping(ChMatrixRef& H,
                                                                 double Kfactor,
                                                                 double Rfactor,
                                                                 double Mfactor) {
    ChMatrixNM<double, 8, 6> ebar_ebardot;
    ...
    ChMatrixNM<double, 24, 108> partial_epsilon_partial_e_Transpose;
    //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 24, 108> partial_epsilon_partial_e_Transpose;
    ...
    //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 24, 108>
    ...
    ChMatrixNM<double, 24, 4> ChunkSDvA;
    ChMatrixNM<double, 24, 4> ChunkSDvB;
    ChMatrixNM<double, 24, 4> ChunkSDvC;
    ...
    for (unsigned int i = 0; i < 8; i++) {
        for (unsigned int j = i; j < 8; j++) {
            double d = Kfactor * m_SD_precompute_col_ordered.block<1, 60>(i, 0) * S_scaled_SD_precompute_col_ordered.block<1, 60>(j, 0).transpose();
    ...
    for (unsigned int i = 0; i < 8; i++) {
        Jacobian_CompactPart_Line.block(0, i, 1, 8 - i) = Kfactor * m_SD_precompute_col_ordered.block<1, 60>(i, 0) * S_scaled_SD_precompute_col_ordered.block(i, 0, 8 - i, 60).transpose();
        Jacobian_CompactPart_Line.block(0, i, 1, 8 - i) += ScaledMassMatrix.segment(idx, (8 - i));
        for (unsigned int j = i; j < 8; j++) {
            H(3 * i, 3 * j) += Jacobian_CompactPart_Line(j);
            H(3 * i + 1, 3 * j + 1) += Jacobian_CompactPart_Line(j);
            H(3 * i + 2, 3 * j + 2) += Jacobian_CompactPart_Line(j);
            if (i != j) {
                H(3 * j, 3 * i) += Jacobian_CompactPart_Line(j);
                H(3 * j + 1, 3 * i + 1) += Jacobian_CompactPart_Line(j);
                H(3 * j + 2, 3 * i + 2) += Jacobian_CompactPart_Line(j);
            }
            idx++;
        }
    }
    ...
    ChMatrixNM<double, 1, 8> Jacobian_CompactPart_Line;
    for (unsigned int i = 0; i < 8; i++) {
        Jacobian_CompactPart_Line.block(0, 0, 1, 1 + i) = Kfactor * m_SD_precompute_col_ordered.block<1, 60>(i, 0) * S_scaled_SD_precompute_col_ordered.block(0, 0, 1 + i, 60).transpose();
        for (unsigned int j = 0; j <= i; j++) {
    ...
    for (unsigned int i = 0; i < 8; i++) {
        ...
        Jacobian_CompactPart_Line = Kfactor * m_SD_precompute_col_ordered.block<1, 60>(i, 0) * S_scaled_SD_precompute_col_ordered.block(0, 0, 1 + i, 60).transpose();
        for (unsigned int j = 0; j <= i; j++) {
            double d = ScaledMassMatrix(idx) + Jacobian_CompactPart_Line(j);
...
void ChElementBeamANCF_3243_TR08S::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ChMatrixNM<double, 8, 3> e_bar;
    CalcCoordMatrix(e_bar);
...
void ChElementBeamANCF_3243_TR08S::ComputeInternalJacobians(ChMatrixNM<double, 24, 24>& JacobianMatrix,
                                                           double Kfactor,
                                                           double Rfactor) {
...
void ChElementBeamANCF_3243_TR08S::Calc_Sxi(ChMatrixNM<double, 3, 24>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_3243_TR08S::Calc_Sxi_compact(ChVectorN<double, 8>& Sxi_compact,
                                                   double xi,
                                                   double eta,
                                                   double zeta) {
...
void ChElementBeamANCF_3243_TR08S::Calc_Sxi_D(ChMatrixNMc<double, 8, 3>& Sxi_D, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR08S::CalcCoordMatrix(ChMatrixNMc<double, 8, 3>& e) {
...
void ChElementBeamANCF_3243_TR08S::CalcCoordMatrix(ChMatrixNM<double, 8, 3>& e) {
...
void ChElementBeamANCF_3243_TR08S::CalcCoordVector(ChVectorN<double, 24>& e) {
...
void ChElementBeamANCF_3243_TR08S::CalcCoordDerivMatrix(ChMatrixNMc<double, 8, 3>& edot) {
...
void ChElementBeamANCF_3243_TR08S::CalcCoordDerivVector(ChVectorN<double, 24>& edot) {
...
void ChElementBeamANCF_3243_TR08S::CalcCombinedCoordMatrix(ChMatrixNM<double, 8, 6>& ebar_ebardot) {
...
void ChElementBeamANCF_3243_TR08S::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_3243_TR08S::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNMc<double, 8, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
// ChVector<> ChElementBeamANCF_3243_TR08S::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_3243_TR08S::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_3243_TR08S::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    ChMatrixNMc<double, 8, 3> e_bar;
    ChVectorN<double, 8> Sxi_compact;
    ChMatrixNMc<double, 8, 3> Sxi_D;
...
// void ChElementBeamANCF_3243_TR08S::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_3243_TR08S::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_3243_TR08S::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_3243_TR08S::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
// void ChElementBeamANCF_3243_TR08S::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_3243_TR08S::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_3243_TR08S::ComputeNF(
...
double ChElementBeamANCF_3243_TR08S::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_3243_TR08S::ComputeTangent(const double U) {
    ChMatrixNMc<double, 8, 3> e_bar;
    ChMatrixNMc<double, 8, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_2_TR08S(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_3243_TR08S::GetStaticGQTables() {
    return &static_tables_2_TR08S;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_3243_TR08S methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_3243_TR08S::ChMaterialBeamANCF_3243_TR08S(
...
// Poisson effect
void ChMaterialBeamANCF_3243_TR08S::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G,
                                              double k1,
                                              double k2) {
