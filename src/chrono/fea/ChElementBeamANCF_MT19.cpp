...
// =============================================================================
//
// MT19 = MT18 + Attempt at taking advantage of matrix symematry
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT19::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT19::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT19::ChElementBeamANCF_MT19() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT19::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT19::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT19::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT19::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT19::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
    ...
    if (GetStrainFormulation() == ChElementBeamANCF_MT19::StrainFormulation::CMPoisson) {
    ...
    for (unsigned int i = 0; i < 9; i++) {
        for (unsigned int j = 0; j < 9; j++) {
            ...
            if (j < i) {
                H(3 * i, 3 * j) += Jacobian_CompactPart(j, i);
                H(3 * i + 1, 3 * j + 1) += Jacobian_CompactPart(j, i);
                H(3 * i + 2, 3 * j + 2) += Jacobian_CompactPart(j, i);
            }
            else {
                H(3 * i, 3 * j) += Jacobian_CompactPart(i, j);
                H(3 * i + 1, 3 * j + 1) += Jacobian_CompactPart(i, j);
...
void ChElementBeamANCF_MT19::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT19::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT19::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_MT19::ComputeNodalMass() {
...
void ChElementBeamANCF_MT19::SetAlphaDamp(double a) {
    m_Alpha = a;
...
void ChElementBeamANCF_MT19::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> e_bar_dot;
...
void ChElementBeamANCF_MT19::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT19::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
...
void ChElementBeamANCF_MT19::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNMc<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChMatrix33<double> F;                   //Deformation Gradient tensor (non-symmetric tensor)
...
void ChElementBeamANCF_MT19::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNMc<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
...
void ChElementBeamANCF_MT19::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ...
    S(0, 1) = (m_GQWeight_det_J_0xi_D0(index)*Kfactor)*2.0*D0(5)*E(0, 1);
    S(0, 2) = (m_GQWeight_det_J_0xi_D0(index)*Kfactor)*2.0*D0(4)*E(0, 2);
    //S(1, 0) = S(0, 1);
    S(1, 1) = (m_GQWeight_det_J_0xi_D0(index)*Kfactor)*D0(1)*E(1, 1);
    S(1, 2) = (m_GQWeight_det_J_0xi_D0(index)*Kfactor)*2.0*D0(3)*E(1, 2);
    //S(2, 0) = S(0, 2);
    //S(2, 1) = S(1, 2);
    ...
    Jac_Compact.triangularView<Eigen::Upper>() += Sxi_D_0xi * S.selfadjointView<Eigen::Upper>() * Sxi_D_0xi.transpose();
}

void ChElementBeamANCF_MT19::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ...
    Jac_Compact.triangularView<Eigen::Upper>() += m_GQWeight_det_J_0xi_Dv(index)*Kfactor*Sxi_D_0xi * S.asDiagonal() * Sxi_D_0xi.transpose();
    //Jac_Compact.selfadjointView<Eigen::Upper>().rankUpdate(Sxi_D_0xi.col(0).transpose(), m_GQWeight_det_J_0xi_Dv(index)*Kfactor*S(0));
    //Jac_Compact.selfadjointView<Eigen::Upper>().rankUpdate(Sxi_D_0xi.col(1).transpose(), m_GQWeight_det_J_0xi_Dv(index)*Kfactor*S(1));
    //Jac_Compact.selfadjointView<Eigen::Upper>().rankUpdate(Sxi_D_0xi.col(2).transpose(), m_GQWeight_det_J_0xi_Dv(index)*Kfactor*S(2));

}

void ChElementBeamANCF_MT19::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, double GQ_weight, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot){
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNMc<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChVectorN<double, 6> epsilon;           //Green-Lagrange strain tensor in Voigt notation
...
void ChElementBeamANCF_MT19::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT19::ComputeInternalJacobiansAnalytic(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> e_bar_dot;
    ...
    for (unsigned int i = 0; i < 9; i++) {
        for (unsigned int j = 0; j < 9; j++) {
            if (j < i) {
                JacobianMatrix(3 * i, 3 * j) += JacobianMatrixCompact(j, i);
                JacobianMatrix(3 * i + 1, 3 * j + 1) += JacobianMatrixCompact(j, i);
                JacobianMatrix(3 * i + 2, 3 * j + 2) += JacobianMatrixCompact(j, i);
            }
            else {
...
void ChElementBeamANCF_MT19::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT19::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT19::Calc_Sxi_D(ChMatrixNMc<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT19::CalcCoordMatrix(ChMatrixNMc<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT19::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT19::CalcCoordDerivMatrix(ChMatrixNMc<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT19::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT19::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT19::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNMc<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT19::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT19::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_MT19::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
...
// void ChElementBeamANCF_MT19::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT19::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT19::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT19::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT19::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT19::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT19::ComputeNF(
...
double ChElementBeamANCF_MT19::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT19::ComputeTangent(const double U) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT19(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT19::GetStaticGQTables() {
    return &static_tables_MT19;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT19 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT19::ChMaterialBeamANCF_MT19(double rho,        // material density
...
void ChMaterialBeamANCF_MT19::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
