...
// =============================================================================
// TR07S_GQ332 = a Gerstmayr style implementation of the element with pre-calculation
...
ChElementShellANCF_3443ML_TR07S_GQ332::ChElementShellANCF_3443ML_TR07S_GQ332()
...
void ChElementShellANCF_3443ML_TR07S_GQ332::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementShellANCF_3443ML_TR07S_GQ332::AddLayer(double thickness, double theta, std::shared_ptr<ChMaterialShellANCF_3443ML_TR07S_GQ332> material) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementShellANCF_3443ML_TR07S_GQ332::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementShellANCF_3443ML_TR07S_GQ332::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                           double Kfactor,
                                                           double Rfactor,
                                                           double Mfactor) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeNodalMass() {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementShellANCF_3443ML_TR07S_GQ332::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const MatrixNx6& ebar_ebardot) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        VectorNIP E0_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0));
        E0_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1));
        E0_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2));
        E0_Block.array() -= 1;
        E0_Block *= 0.5;
        VectorNIP E_BlockDamping = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 5));
        E0_Block += m_Alpha * E_BlockDamping;
        E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E1_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0));
        E1_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1));
        E1_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2));
        E1_Block.array() -= 1;
        E1_Block *= 0.5;
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 5));
        E1_Block += m_Alpha * E_BlockDamping;
        E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E2_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0));
        E2_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1));
        E2_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2));
        E2_Block.array() -= 1;
        E2_Block *= 0.5;
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 5));
        E2_Block += m_Alpha * E_BlockDamping;
        E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E3_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0));
        E3_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1));
        E3_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2));
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(
        ...
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 5));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(
        ...
        VectorNIP E4_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0));
        E4_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1));
        E4_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2));
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(
        ...
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 5));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(
        ...
        VectorNIP E5_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0));
        E5_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1));
        E5_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2));
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(
        ...
        m_SPK2_0_Block.block<18, 1>(kl*18, 0) = SPK2_0_Block;
        m_SPK2_1_Block.block<18, 1>(kl*18, 0) = SPK2_1_Block;
        m_SPK2_2_Block.block<18, 1>(kl*18, 0) = SPK2_2_Block;
        m_SPK2_3_Block.block<18, 1>(kl*18, 0) = SPK2_3_Block;
        m_SPK2_4_Block.block<18, 1>(kl*18, 0) = SPK2_4_Block;
        m_SPK2_5_Block.block<18, 1>(kl*18, 0) = SPK2_5_Block;

        //m_SPK2_0_Block.block<18, 1>(kl*m_numLayerGQPnts, 0) = D(0, 0)*E0_Block + D(0, 1)*E1_Block + D(0, 2)*E2_Block + D(0, 3)*E3_Block + D(0, 4)*E4_Block + D(0, 5)*E5_Block;
        //m_SPK2_1_Block.block<18, 1>(kl*m_numLayerGQPnts, 0) = D(1, 0)*E0_Block + D(1, 1)*E1_Block + D(1, 2)*E2_Block + D(1, 3)*E3_Block + D(1, 4)*E4_Block + D(1, 5)*E5_Block;
        //m_SPK2_2_Block.block<18, 1>(kl*m_numLayerGQPnts, 0) = D(2, 0)*E0_Block + D(2, 1)*E1_Block + D(2, 2)*E2_Block + D(2, 3)*E3_Block + D(2, 4)*E4_Block + D(2, 5)*E5_Block;
        //m_SPK2_3_Block.block<18, 1>(kl*m_numLayerGQPnts, 0) = D(3, 0)*E0_Block + D(3, 1)*E1_Block + D(3, 2)*E2_Block + D(3, 3)*E3_Block + D(3, 4)*E4_Block + D(3, 5)*E5_Block;
        //m_SPK2_4_Block.block<18, 1>(kl*m_numLayerGQPnts, 0) = D(4, 0)*E0_Block + D(4, 1)*E1_Block + D(4, 2)*E2_Block + D(4, 3)*E3_Block + D(4, 4)*E4_Block + D(4, 5)*E5_Block;
        //m_SPK2_5_Block.block<18, 1>(kl*m_numLayerGQPnts, 0) = D(5, 0)*E0_Block + D(5, 1)*E1_Block + D(5, 2)*E2_Block + D(5, 3)*E3_Block + D(5, 4)*E4_Block + D(5, 5)*E5_Block;
        ...
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 0) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 0) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 1) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 1) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 2) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 2) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(SPK2_4_Block);

        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 0) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 0) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 1) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 1) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 2) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 2) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(SPK2_3_Block);

        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 0) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 0) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 1) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 1) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 2) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 2) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(SPK2_2_Block);

        // =============================================================================

        QiCompact += m_SD_precompute_col_ordered.block<16, 54>(0, 54 * kl) * P_transpose_scaled_Block_col_ordered;
    }
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        VectorNIP E0_Block = m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 0));
        E0_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 1));
        E0_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 2));
        E0_Block.array() -= 1;
        E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E0_Block *= 0.5;

        VectorNIP E1_Block = m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0));
        E1_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1));
        E1_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2));
        E1_Block.array() -= 1;
        E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E1_Block *= 0.5;

        VectorNIP E2_Block = m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0));
        E2_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1));
        E2_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2));
        E2_Block.array() -= 1;
        E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E2_Block *= 0.5;

        VectorNIP E3_Block = m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0));
        E3_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1));
        E3_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2));
        E3_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E4_Block = m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0));
        E4_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1));
        E4_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2));
        E4_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E5_Block = m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0));
        E5_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1));
        E5_Block += m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2));
        E5_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        ...
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 0) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 0) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 1) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 1) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 2) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(0, 2) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(SPK2_4_Block);

        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 0) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 0) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 1) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 1) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 2) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(18, 2) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(SPK2_3_Block);

        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 0) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 0).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 0) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 1) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 1).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 1) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 2) =
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl, 2).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<18, 1>(36, 2) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2).cwiseProduct(SPK2_2_Block);
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
    double Kfactor,
    double Rfactor) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 48, 108> partial_epsilon_partial_e_Transpose;
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> partial_epsilon_partial_e_Transpose;
        partial_epsilon_partial_e_Transpose.resize(48, 108);

        for (auto i = 0; i < 16; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 36, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<18, 1>(54 * kl + 18, 2).transpose());
        }

        //ChMatrixNMc<double, 54, 3> scaled_F_Transpose_col_ordered =
        //    (Kfactor + m_Alpha * Rfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<54, 3>(0, 0) +
        //    (m_Alpha * Kfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<54, 3>(0, 3);
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 54, 3> scaled_F_Transpose_col_ordered;
        //scaled_F_Transpose_col_ordered.resize(54, 3);
        //scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<54, 3>(0, 0) +
        //    (m_Alpha * Kfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<54, 3>(0, 3);
        ChMatrixNMc<double, 54, 3> scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<54, 3>(54 * kl + 0, 0) +
            (m_Alpha * Kfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<54, 3>(54 * kl + 0, 3);


        for (auto i = 0; i < 3; i++) {
            scaled_F_Transpose_col_ordered.block<18, 1>(0, i).array() *= m_GQWeight_det_J_0xi_D.array();
            scaled_F_Transpose_col_ordered.block<18, 1>(18, i).array() *= m_GQWeight_det_J_0xi_D.array();
            scaled_F_Transpose_col_ordered.block<18, 1>(36, i).array() *= m_GQWeight_det_J_0xi_D.array();
        }

        // ChMatrixNM<double, 48, 108> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 48, 108> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        Scaled_Combined_partial_epsilon_partial_e_Transpose.resize(48, 108);


        for (auto i = 0; i < 16; i++) {
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 0).transpose());

            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 1).transpose());

            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(36, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<18, 1>(18, 2).transpose());
        }

        ChMatrixNM<double, 6, 6> D;
        m_layers[kl].GetMaterial()->Get_Rotated_D(D, m_layers[kl].Get_theta());

        // ChMatrixNM<double, 48, 108> DScaled_Combined_partial_epsilon_partial_e_Transpose;
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 48, 108> DScaled_Combined_partial_epsilon_partial_e_Transpose;
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> DScaled_Combined_partial_epsilon_partial_e_Transpose;
        DScaled_Combined_partial_epsilon_partial_e_Transpose.resize(48, 108);

        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 0) = D(0, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 0) + D(0, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 18) + D(0, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 36) + D(0, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 54) + D(0, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 72) + D(0, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 90);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 18) = D(1, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 0) + D(1, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 18) + D(1, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 36) + D(1, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 54) + D(1, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 72) + D(1, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 90);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 36) = D(2, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 0) + D(2, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 18) + D(2, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 36) + D(2, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 54) + D(2, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 72) + D(2, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 90);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 54) = D(3, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 0) + D(3, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 18) + D(3, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 36) + D(3, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 54) + D(3, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 72) + D(3, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 90);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 72) = D(4, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 0) + D(4, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 18) + D(4, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 36) + D(4, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 54) + D(4, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 72) + D(4, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 90);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 90) = D(5, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 0) + D(5, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 18) + D(5, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 36) + D(5, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 54) + D(5, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 72) + D(5, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<48, 18>(0, 90);

        H += partial_epsilon_partial_e_Transpose * DScaled_Combined_partial_epsilon_partial_e_Transpose.transpose();

        //===========================================================================================

        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 16, 54> S_scaled_SD_precompute_col_ordered;
        //S_scaled_SD_precompute_col_ordered.resize(16, 54);
        ChMatrixNM<double, 16, 54> S_scaled_SD_precompute_col_ordered;

        for (auto i = 0; i < 16; i++) {
            S_scaled_SD_precompute_col_ordered.block<1, 18>(i, 0).noalias() =
                m_SPK2_0_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0)) +
                m_SPK2_5_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18)) +
                m_SPK2_4_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36));

            S_scaled_SD_precompute_col_ordered.block<1, 18>(i, 18).noalias() =
                m_SPK2_5_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0)) +
                m_SPK2_1_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18)) +
                m_SPK2_3_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36));

            S_scaled_SD_precompute_col_ordered.block<1, 18>(i, 36).noalias() =
                m_SPK2_4_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0)) +
                m_SPK2_3_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18)) +
                m_SPK2_2_Block.block<18, 1>(kl * 18, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36));
        }
        ...
        for (unsigned int i = 0; i < 16; i++) {
            for (unsigned int j = i; j < 16; j++) {
                double d = Kfactor * m_SD_precompute_col_ordered.block<1, 54>(i, 54 * kl) * S_scaled_SD_precompute_col_ordered.block<1, 54>(j, 0).transpose();
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (auto i = 0; i < 16; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>(3 * i, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 1, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 18).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 36).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 54).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 72).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 36).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 36, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 18>((3 * i) + 2, 90).noalias() = m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 18).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 18>(i, 54 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<18, 1>(54 * kl + 18, 2).transpose());
        }
        ...
        ChMatrixNMc<double, 54, 3> scaled_F_Transpose_col_ordered = Kfactor * m_F_Transpose_CombinedBlock_col_ordered;
...
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta, double thickness, double zoffset) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta, thickness, zoffset);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
    ebar.row(3) = m_nodes[0]->GetDDD().eigen();
...
void ChElementShellANCF_3443ML_TR07S_GQ332::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta, double thickness, double zoffset) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta, thickness, zoffset);
...
double ChElementShellANCF_3443ML_TR07S_GQ332::Calc_det_J_0xi(double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::EvaluateSectionFrame(const double xi, const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    Matrix3x3N Sxi;
    Matrix3x3N Sxi_xi;
    Matrix3x3N Sxi_eta;
...
void ChElementShellANCF_3443ML_TR07S_GQ332::EvaluateSectionPoint(const double u, const double v, ChVector<>& point) {
    Matrix3x3N Sxi;
...
void ChElementShellANCF_3443ML_TR07S_GQ332::EvaluateSectionVelNorm(double U, double V, ChVector<>& Result) {
    Matrix3x3N Sxi_zeta;
    Vector3N edot;
    ChVector<> r_zeta;
...
void ChElementShellANCF_3443ML_TR07S_GQ332::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementShellANCF_3443ML_TR07S_GQ332::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3443ML_TR07S_GQ332::ComputeNF(
...
double ChElementShellANCF_3443ML_TR07S_GQ332::GetDensity() {
...
ChVector<> ChElementShellANCF_3443ML_TR07S_GQ332::ComputeNormal(const double U, const double V) {
    Matrix3x3N Sxi_zeta;
    Vector3N e;
    ChVector<> r_zeta;
...
ChQuadratureTables static_tables_3443ML_TR07S_GQ332(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3443ML_TR07S_GQ332::GetStaticGQTables() {
    return &static_tables_3443ML_TR07S_GQ332;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3443ML_TR07S_GQ332::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)
ChElementShellANCF_3443ML_TR07S_GQ332::Layer::Layer(ChElementShellANCF_3443ML_TR07S_GQ332* element,
    double thickness,
    double theta,
    std::shared_ptr<ChMaterialShellANCF_3443ML_TR07S_GQ332> material)
    : m_element(element), m_thickness(thickness), m_theta(theta), m_material(material) {}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialShellANCF_3443ML_TR07S_GQ332 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialShellANCF_3443ML_TR07S_GQ332::ChMaterialShellANCF_3443ML_TR07S_GQ332(
...
// Poisson effect
void ChMaterialShellANCF_3443ML_TR07S_GQ332::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G) {
...
void ChMaterialShellANCF_3443ML_TR07S_GQ332::Get_Rotated_D(ChMatrixNM<double, 6, 6>& D, double theta) {
