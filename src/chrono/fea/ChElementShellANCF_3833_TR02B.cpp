...
// =============================================================================
// Higher order ANCF shell element with 8 nodes. Description of this element (3833_TR02B) and its internal forces may be
...
//
// =============================================================================
// TR02B = Direct Textbook Translation with Reduced GQ Points
...
// Constructor
// ------------------------------------------------------------------------------
ChElementShellANCF_3833_TR02B::ChElementShellANCF_3833_TR02B()
...
void ChElementShellANCF_3833_TR02B::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementShellANCF_3833_TR02B::AddLayer(double thickness,
                                             double theta,
...
void ChElementShellANCF_3833_TR02B::SetDimensions(double lenX, double lenY) {
    m_lenX = lenX;
    m_lenY = lenY;
}
...
void ChElementShellANCF_3833_TR02B::SetMidsurfaceOffset(const double offset) {
    m_midsurfoffset = offset;
}

// Set the value for the single term structural damping coefficient.

void ChElementShellANCF_3833_TR02B::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementShellANCF_3833_TR02B::GetGreenLagrangeStrain(const double xi,
                                                                   const double eta,
                                                                   const double zeta) {
...
ChMatrix33<> ChElementShellANCF_3833_TR02B::GetPK2Stress(const double layer,
                                                         const double xi,
                                                         const double eta,
...
double ChElementShellANCF_3833_TR02B::GetVonMissesStress(const double layer,
                                                         const double xi,
                                                         const double eta,
                                                         const double layer_zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
...
void ChElementShellANCF_3833_TR02B::SetupInitial(ChSystem* system) {
...
void ChElementShellANCF_3833_TR02B::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3833_TR02B::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementShellANCF_3833_TR02B::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementShellANCF_3833_TR02B::ComputeNodalMass() {
...
void ChElementShellANCF_3833_TR02B::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3833_TR02B::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                             double Kfactor,
                                                             double Rfactor,
                                                             double Mfactor) {
...
void ChElementShellANCF_3833_TR02B::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementShellANCF_3833_TR02B::EvaluateSectionFrame(const double xi,
                                                         const double eta,
                                                         ChVector<>& point,
                                                         ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementShellANCF_3833_TR02B::EvaluateSectionPoint(const double xi, const double eta, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR02B::EvaluateSectionVelNorm(const double xi, const double eta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR02B::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3833_TR02B::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3833_TR02B::LoadableStateIncrement(const unsigned int off_x,
                                                           ChState& x_new,
                                                           const ChState& x,
...
void ChElementShellANCF_3833_TR02B::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3833_TR02B::ComputeNF(
...
double ChElementShellANCF_3833_TR02B::GetDensity() {
...
ChVector<> ChElementShellANCF_3833_TR02B::ComputeNormal(const double xi, const double eta) {
...
void ChElementShellANCF_3833_TR02B::ComputeMassMatrixAndGravityForce() {
...
void ChElementShellANCF_3833_TR02B::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
                                                                 const Vector3N& e,
                                                                 const Vector3N& edot) {
        ...
        for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[NP - 1].size(); it_xi++) {
            for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[NP - 1].size(); it_eta++) {
                for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[NT - 1].size(); it_zeta++) {
                    ...
                    Vector3N Qi;
                    ComputeInternalForcesSingleGQPnt(Qi, xi, eta, zeta, thickness, layer_midsurface_offset, D, e, edot);
...
void ChElementShellANCF_3833_TR02B::ComputeInternalForcesSingleGQPnt(Vector3N& Qi,
                                                                     double xi,
                                                                     double eta,
                                                                     double zeta,
                                                                     double thickness,
...
void ChElementShellANCF_3833_TR02B::Calc_Sxi_compact(VectorN& Sxi_compact,
                                                     double xi,
                                                     double eta,
                                                     double zeta,
                                                     double thickness,
...
void ChElementShellANCF_3833_TR02B::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact,
                                                        double xi,
                                                        double eta,
                                                        double zeta,
                                                        double thickness,
...
void ChElementShellANCF_3833_TR02B::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta,
                                                         double thickness,
...
void ChElementShellANCF_3833_TR02B::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                          double xi,
                                                          double eta,
                                                          double zeta,
                                                          double thickness,
...
void ChElementShellANCF_3833_TR02B::Calc_Sxi_D(MatrixNx3c& Sxi_D,
                                               double xi,
                                               double eta,
                                               double zeta,
                                               double thickness,
...
void ChElementShellANCF_3833_TR02B::Calc_Sxi_xi(Matrix3x3N& Sxi_xi,
                                                double xi,
                                                double eta,
                                                double zeta,
                                                double thickness,
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833_TR02B::Calc_Sxi_eta(Matrix3x3N& Sxi_eta,
                                                 double xi,
                                                 double eta,
                                                 double zeta,
                                                 double thickness,
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833_TR02B::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta,
                                                  double xi,
                                                  double eta,
                                                  double zeta,
                                                  double thickness,
...
void ChElementShellANCF_3833_TR02B::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3833_TR02B::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3833_TR02B::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3833_TR02B::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3833_TR02B::CalcCombinedCoordMatrix(Matrix6xN& ebar_ebardot) {
...
void ChElementShellANCF_3833_TR02B::Calc_J_0xi(ChMatrix33<double>& J_0xi,
                                               double xi,
                                               double eta,
                                               double zeta,
                                               double thickness,
...
double ChElementShellANCF_3833_TR02B::Calc_det_J_0xi(double xi,
                                                     double eta,
                                                     double zeta,
                                                     double thickness,
                                                     double zoffset) {
...
void ChElementShellANCF_3833_TR02B::RotateReorderStiffnessMatrix(ChMatrixNM<double, 6, 6>& D, double theta) {
...
ChQuadratureTables static_tables_3833_TR02B(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3833_TR02B::GetStaticGQTables() {
    return &static_tables_3833_TR02B;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3833_TR02B::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)

ChElementShellANCF_3833_TR02B::Layer::Layer(double thickness,
                                            double theta,
                                            std::shared_ptr<ChMaterialShellANCF> material)
