...
template <int NP, int NT>
void ChElementShellANCF_3833<NP, NT>::EvaluateSectionVelNorm(double xi, double eta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, 0);
...
void ChElementShellANCF_3833<NP, NT>::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
    ...
    // 5 GQ Points are needed in the xi, eta, and zeta directions for exact integration of the element's mass matrix, even if the reference configuration is not straight Since the
