...
#ifndef CHELEMENTBEAMANCFMT01_H
#define CHELEMENTBEAMANCFMT01_H
...
class ChApi ChMaterialBeamANCF_MT01 {
  public:
    /// Construct an isotropic material.
    ChMaterialBeamANCF_MT01(double rho,        ///< material density
    ...
    //const ChMatrixNM<double, 6, 6>& Get_D() const { return (m_D0+m_Dv); }
  ...
  private:
    ...
    void Calc_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G);
    void Calc_D0(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2);
    ...
    ChMatrixNM<double, 6, 6> m_D0;        ///< matrix of elastic coefficients (split of diagonal terms for integration across the entire element)
    ChMatrixNM<double, 6, 6> m_Dv;        ///< matrix of elastic coefficients (remainder of split, upper 3x3 terms for integration only on the beam axis)
...
class ChApi ChElementBeamANCF_MT01 : public ChElementBeam, public ChLoadableU, public ChLoadableUVW {
  public:
    using ShapeVector = ChMatrixNM<double, 1, 9>;

    ChElementBeamANCF_MT01();
    ~ChElementBeamANCF_MT01() {}
    ...
    void SetMaterial(std::shared_ptr<ChMaterialBeamANCF_MT01> beam_mat) { m_material = beam_mat; }
    ...
    /// Return the material.
    std::shared_ptr<ChMaterialBeamANCF_MT01> GetMaterial() const { return m_material; }
  ...
  public:
    ...
    //// MIKE: Start of MT Added stuff...Clean this up when everything is finished
    void CalcCoordVector(ChVectorN<double, 27>& v);
    ...
    void Calc_det_J_0xi(double& det_J_0xi, double xi, double eta, double zeta);
    void Calc_J_0xi_Inv(ChMatrix33<double>& J_0xi_Inv, double xi, double eta, double zeta);
    void ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChVectorN<double, 27>& e, ChVectorN<double, 27>& e_dot);
    ...
    void ComputeInternalForces_NoUpdate(ChVectorDynamic<>& Fi, ChVectorN<double, 27>& e, ChVectorN<double, 27>& e_dot);
    void ComputeInternalForcesTest(ChVectorDynamic<double>& Fi, ChVectorN<double, 27>& e, ChVectorN<double, 27>& e_dot);
  ...
  private:
    ...
    std::shared_ptr<ChMaterialBeamANCF_MT01> m_material;         ///< beam material
    StrainFormulation m_strain_form;                        ///< Strain formulation

    ChVectorN<double, 27> m_e0;                             ///< Element Position Coordinate Vector for the Reference Configuration
    ChVectorN<double, 27> m_e;                              ///< Element Position Coordinate Vector for the Current Configuration
    ChVectorN<double, 27> m_e_dot;                          ///< Element Velocity Coordinate Vector for the  Current Configuration

  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    friend class BeamANCF_MT01_Force;
    friend class BeamANCF_MT01_ForceNu;
    friend class BeamANCF_MT01_Jacobian;
    friend class BeamANCF_MT01_JacobianNu;
};
