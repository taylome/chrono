...
// =============================================================================
// Higher order ANCF shell element with 8 nodes. Description of this element (3833_TR13) and its internal forces may be
...
//
// =============================================================================
// TR13 = Garcia-Vallejo Based Pre-Integration Approach
// =============================================================================
...
// Constructor
// ------------------------------------------------------------------------------
ChElementShellANCF_3833_TR13::ChElementShellANCF_3833_TR13()
...
void ChElementShellANCF_3833_TR13::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementShellANCF_3833_TR13::AddLayer(double thickness,
                                            double theta,
...
void ChElementShellANCF_3833_TR13::SetDimensions(double lenX, double lenY) {
    m_lenX = lenX;
    m_lenY = lenY;
}
...
void ChElementShellANCF_3833_TR13::SetMidsurfaceOffset(const double offset) {
    m_midsurfoffset = offset;
}

// Set the value for the single term structural damping coefficient.

void ChElementShellANCF_3833_TR13::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementShellANCF_3833_TR13::GetGreenLagrangeStrain(const double xi,
                                                                  const double eta,
                                                                  const double zeta) {
...
ChMatrix33<> ChElementShellANCF_3833_TR13::GetPK2Stress(const double layer,
                                                        const double xi,
                                                        const double eta,
...
double ChElementShellANCF_3833_TR13::GetVonMissesStress(const double layer,
                                                        const double xi,
                                                        const double eta,
                                                        const double layer_zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
...
void ChElementShellANCF_3833_TR13::SetupInitial(ChSystem* system) {
    m_element_dof = 0;
    for (int i = 0; i < 8; i++) {
...
void ChElementShellANCF_3833_TR13::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3833_TR13::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementShellANCF_3833_TR13::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementShellANCF_3833_TR13::ComputeNodalMass() {
...
void ChElementShellANCF_3833_TR13::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3833_TR13::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                            double Kfactor,
                                                            double Rfactor,
                                                            double Mfactor) {
    ...
    MatrixNxN ee = ebar_plus_ebardot.transpose() * ebar;
    ...
    for (unsigned int i = 0; i < NSF; i++) {
        for (unsigned int s = i; s < NSF; s++) {
            ...
            ChMatrixNM<double, 1, NSF> ds = ee.row(s) * Ccompact.transpose();
            ChMatrixNMc<double, 3, NSF> eC = ebar * Ccompact;
            for (unsigned int k = 0; k < NSF; k++) {
                ChMatrixNM<double, 3, 3> Block = eC.col(k)*ebar_plus_ebardot.col(s).transpose();
                Block(0, 0) += ds(k);
                Block(1, 1) += ds(k);
                Block(2, 2) += ds(k);
            ...
            if (i != s) {
                ChMatrixNM<double, 1, NSF> ds = ee.row(i) * Ccompact;
                ChMatrixNMc<double, 3, NSF> eC = ebar * Ccompact.transpose();
                for (unsigned int k = 0; k < NSF; k++) {
                    ChMatrixNM<double, 3, 3> Block = eC.col(k)*ebar_plus_ebardot.col(i).transpose();
                    Block(0, 0) += ds(k);
                    Block(1, 1) += ds(k);
                    Block(2, 2) += ds(k);
...
void ChElementShellANCF_3833_TR13::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementShellANCF_3833_TR13::EvaluateSectionFrame(const double xi,
                                                        const double eta,
                                                        ChVector<>& point,
                                                        ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementShellANCF_3833_TR13::EvaluateSectionPoint(const double xi, const double eta, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR13::EvaluateSectionVelNorm(const double xi, const double eta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR13::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3833_TR13::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3833_TR13::LoadableStateIncrement(const unsigned int off_x,
                                                          ChState& x_new,
                                                          const ChState& x,
...
void ChElementShellANCF_3833_TR13::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3833_TR13::ComputeNF(
...
double ChElementShellANCF_3833_TR13::GetDensity() {
...
ChVector<> ChElementShellANCF_3833_TR13::ComputeNormal(const double xi, const double eta) {
...
void ChElementShellANCF_3833_TR13::ComputeMassMatrixAndGravityForce() {
...
void ChElementShellANCF_3833_TR13::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementShellANCF_3833_TR13::Calc_Sxi_compact(VectorN& Sxi_compact,
                                                    double xi,
                                                    double eta,
                                                    double zeta,
                                                    double thickness,
...
void ChElementShellANCF_3833_TR13::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact,
                                                       double xi,
                                                       double eta,
                                                       double zeta,
                                                       double thickness,
...
void ChElementShellANCF_3833_TR13::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact,
                                                        double xi,
                                                        double eta,
                                                        double zeta,
                                                        double thickness,
...
void ChElementShellANCF_3833_TR13::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta,
                                                         double thickness,
...
void ChElementShellANCF_3833_TR13::Calc_Sxi_D(MatrixNx3c& Sxi_D,
                                              double xi,
                                              double eta,
                                              double zeta,
                                              double thickness,
...
void ChElementShellANCF_3833_TR13::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3833_TR13::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3833_TR13::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3833_TR13::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3833_TR13::CalcCombinedCoordMatrix(Matrix6xN& ebar_ebardot) {
...
void ChElementShellANCF_3833_TR13::Calc_J_0xi(ChMatrix33<double>& J_0xi,
                                              double xi,
                                              double eta,
                                              double zeta,
                                              double thickness,
...
double ChElementShellANCF_3833_TR13::Calc_det_J_0xi(double xi,
                                                    double eta,
                                                    double zeta,
                                                    double thickness,
                                                    double zoffset) {
...
void ChElementShellANCF_3833_TR13::RotateReorderStiffnessMatrix(ChMatrixNM<double, 6, 6>& D, double theta) {
...
ChQuadratureTables static_tables_3833_TR13(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3833_TR13::GetStaticGQTables() {
    return &static_tables_3833_TR13;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3833_TR13::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)

ChElementShellANCF_3833_TR13::Layer::Layer(double thickness,
                                           double theta,
                                           std::shared_ptr<ChMaterialShellANCF> material)
