...
// =============================================================================
//
// MT20 = M18 + no explicit calculation of E
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT20::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT20::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT20::ChElementBeamANCF_MT20() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT20::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT20::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT20::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT20::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT20::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
    ...
    if (GetStrainFormulation() == ChElementBeamANCF_MT20::StrainFormulation::CMPoisson) {
...
void ChElementBeamANCF_MT20::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT20::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT20::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_MT20::ComputeNodalMass() {
...
void ChElementBeamANCF_MT20::SetAlphaDamp(double a) {
    m_Alpha = a;
    if (std::abs(m_Alpha) > 1e-10)
        m_damping_enabled = true;
}

void ChElementBeamANCF_MT20::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> e_bar_dot;
...
void ChElementBeamANCF_MT20::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT20::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    
    // -------  TESTING JUST THIS CODE - NO DAMPING SUPPORT ------

    ChVectorN<double, 9> Sxi_D_0xiCol0 = m_SD_precompute_D0.block(9 * index, 0, 9, 1);     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChVectorN<double, 9> Sxi_D_0xiCol1 = m_SD_precompute_D0.block(9 * index, 1, 9, 1);     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChVectorN<double, 9> Sxi_D_0xiCol2 = m_SD_precompute_D0.block(9 * index, 2, 9, 1);     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChVectorN<double, 3> FCol0 = e_bar.transpose()*Sxi_D_0xiCol0;
    ChVectorN<double, 3> FCol1 = e_bar.transpose()*Sxi_D_0xiCol1;
    ChVectorN<double, 3> FCol2 = e_bar.transpose()*Sxi_D_0xiCol2;
    ...
    ChVectorN<double, 6> Weighted_S_PK2;
    Weighted_S_PK2(0) = m_GQWeight_det_J_0xi_D0(index)*D0(0)*(0.5*FCol0.transpose()*FCol0 - 0.5);
    Weighted_S_PK2(1) = m_GQWeight_det_J_0xi_D0(index)*D0(1)*(0.5*FCol1.transpose()*FCol1 - 0.5);
    Weighted_S_PK2(2) = m_GQWeight_det_J_0xi_D0(index)*D0(2)*(0.5*FCol2.transpose()*FCol2 - 0.5);
    Weighted_S_PK2(3) = m_GQWeight_det_J_0xi_D0(index)*D0(3)*FCol1.transpose()*FCol2;
    Weighted_S_PK2(4) = m_GQWeight_det_J_0xi_D0(index)*D0(4)*FCol0.transpose()*FCol2;
    Weighted_S_PK2(5) = m_GQWeight_det_J_0xi_D0(index)*D0(5)*FCol0.transpose()*FCol1;

    ChVectorN<double, 3> PCol0 = FCol0*Weighted_S_PK2(0) + FCol1*Weighted_S_PK2(5) + FCol2*Weighted_S_PK2(4);
    ChVectorN<double, 3> PCol1 = FCol0*Weighted_S_PK2(5) + FCol1*Weighted_S_PK2(1) + FCol2*Weighted_S_PK2(3);
    ChVectorN<double, 3> PCol2 = FCol0*Weighted_S_PK2(4) + FCol1*Weighted_S_PK2(3) + FCol2*Weighted_S_PK2(1);

    QiCompact += Sxi_D_0xiCol0*PCol0.transpose() + Sxi_D_0xiCol1*PCol1.transpose() + Sxi_D_0xiCol2*PCol2.transpose();
}
...
//        //Carry out the multiplication with the specific provided diagonal 6x6 stiffness tensor
//        // (without explicitly forming the matrix multiplication in Voigt notation)
//        S(0, 0) = D0(0)*0.5*(F(0,0)*F(0, 0) + F(1, 0)*F(1, 0) + F(2, 0)*F(2, 0) - 1);
//        S(0, 1) = D0(5)*F(0, 1)*F(1, 0);
//        S(0, 2) = D0(4)*F(0, 2)*F(2, 0);
//        S(1, 0) = S(0, 1);
//        S(1, 1) = D0(1)*0.5*(F(0, 1)*F(0, 1) + F(1, 1)*F(1, 1) + F(2, 1)*F(2, 1) - 1);
//        S(1, 2) = D0(3)*F(1, 2)*F(2, 1);
//        S(2, 0) = S(0, 2);
//        S(2, 1) = S(1, 2);
//        S(2, 2) = D0(2)*0.5*(F(0, 2)*F(0, 2) + F(1, 2)*F(1, 2) + F(2, 2)*F(2, 2) - 1);
//    }
//    else{
...
//        //Carry out the multiplication with the specific provided diagonal 6x6 stiffness tensor
//        // (without explicitly forming the matrix multiplication in Voigt notation)
//        S(0, 0) = D0(0)*(0.5*F(0, 0)*F(0, 0)-0.5 + m_Alpha*F_dot(0,0)*F(0, 0));
//        S(0, 1) = D0(5)*(F(0, 1)*F(1, 0) + m_Alpha*0.5*(F_dot(0, 1)*F(1, 0) + F_dot(1, 0)*F(0, 1)));
//        S(0, 2) = D0(4)*(F(0, 2)*F(2, 0) + m_Alpha*0.5*(F_dot(0, 2)*F(2, 0) + F_dot(2, 0)*F(0, 2)));
//        S(1, 0) = S(0, 1);
//        S(1, 1) = D0(1)*(0.5*F(1, 1)*F(1, 1)-0.5 + m_Alpha*F_dot(1, 1)*F(1, 1));
//        S(1, 2) = D0(3)*(F(1, 2)*F(2, 1) + m_Alpha*0.5*(F_dot(1, 2)*F(2, 1) + F_dot(2, 1)*F(1, 2)));
//        S(2, 0) = S(0, 2);
//        S(2, 1) = S(1, 2);
//        S(2, 2) = D0(2)*(0.5*F(2, 2)*F(2, 2)-0.5 + m_Alpha*F_dot(2, 2)*F(2, 2));
//    }
//
//
//
...
void ChElementBeamANCF_MT20::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
...
void ChElementBeamANCF_MT20::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
...
void ChElementBeamANCF_MT20::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT20::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT20::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, double GQ_weight, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot){
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNMc<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChVectorN<double, 6> epsilon;           //Green-Lagrange strain tensor in Voigt notation
...
void ChElementBeamANCF_MT20::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT20::ComputeInternalJacobiansAnalytic(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> e_bar_dot;
...
void ChElementBeamANCF_MT20::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT20::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT20::Calc_Sxi_D(ChMatrixNMc<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT20::CalcCoordMatrix(ChMatrixNMc<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT20::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT20::CalcCoordDerivMatrix(ChMatrixNMc<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT20::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT20::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT20::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNMc<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT20::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT20::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_MT20::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
...
// void ChElementBeamANCF_MT20::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT20::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT20::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT20::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT20::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT20::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT20::ComputeNF(
...
double ChElementBeamANCF_MT20::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT20::ComputeTangent(const double U) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT20(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT20::GetStaticGQTables() {
    return &static_tables_MT20;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT20 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT20::ChMaterialBeamANCF_MT20(double rho,        // material density
...
void ChMaterialBeamANCF_MT20::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
