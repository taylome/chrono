...
// =============================================================================
//
// MT10 = MT09 with performance change tests
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT10::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT10::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT10::ChElementBeamANCF_MT10() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT10::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT10::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT10::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT10::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT10::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementBeamANCF_MT10::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT10::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT10::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_MT10::ComputeNodalMass() {
...
static void ReshapeMat2Vec(ChMatrixNM<double, 9, 3>& M, ChVectorN<double, 27>& V) {
    ...
    //Eigen::Map<ChVectorN<double, 27>> V2(M.data(), M.size());
    //V2 = V;
}


// Set structural damping.
void ChElementBeamANCF_MT10::SetAlphaDamp(double a) {
    m_Alpha = a;
    if (std::abs(m_Alpha) > 1e-10)
        m_damping_enabled = true;
}

void ChElementBeamANCF_MT10::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBeamANCF_MT10::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ...
    // Set QiCompact to zero since the results from each GQ point will be added to this vector
    // Kept in compact matrix form until the entire result is accumulated so that only a single
    // reshape is required.
    ...
    for (unsigned int index = 0; index < m_GQWeight_det_J_0xi_D0.size(); index++) {
        ComputeInternalForcesSingleGQPnt(QiCompact, index, D0, e_bar, e_bar_dot);
    }

    //Calculate the portion of the Selective Reduced Integration that account for the Poisson effect, but only on the beam axis
    if (GetStrainFormulation() == ChElementBeamANCF_MT10::StrainFormulation::CMPoisson) {
        for (unsigned int index = 0; index < m_GQWeight_det_J_0xi_Dv.size(); index++) {
            ComputeInternalForcesSingleGQPnt(QiCompact, index, Dv, e_bar, e_bar_dot);
        }
    }
...
void ChElementBeamANCF_MT10::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ...
    //ReshapeMat2Vec(QiCompact, Qi);

}

void ChElementBeamANCF_MT10::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChMatrix33<double> F;                   //Deformation Gradient tensor (non-symmetric tensor)
...
void ChElementBeamANCF_MT10::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
...
void ChElementBeamANCF_MT10::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac, double Kfactor, double Rfactor, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {

    ChMatrixNM<double, 9, 3> Sxi_D_0xi = m_SD_precompute_D0.block(9 * index, 0, 9, 3);  //Faster to use this than to reference m_SD_precompute_D0 directly in the calculations 
    ...
    epsilon << 0.5*(SxiF1e.transpose()*SxiF1e - 1),
               0.5*(SxiF2e.transpose()*SxiF2e - 1),
               0.5*(SxiF3e.transpose()*SxiF3e - 1),
               SxiF2e.transpose()*SxiF3e,
               SxiF1e.transpose()*SxiF3e,
               SxiF1e.transpose()*SxiF2e;

    ChMatrixNM<double, 9, 3> temp;
    ChVectorN<double, 27> tempReshaped;
    //Eigen::Map<ChVectorN<double, 27>> tempReshaped(temp.data(), temp.size());

    temp = Sxi_D_0xi.col(0)*SxiF1e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF1_t_SxiF1e = tempReshaped;

    temp= Sxi_D_0xi.col(1)*SxiF1e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF2_t_SxiF1e = tempReshaped;

    temp = Sxi_D_0xi.col(2)*SxiF1e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF3_t_SxiF1e = tempReshaped;

    temp = Sxi_D_0xi.col(0)*SxiF2e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF1_t_SxiF2e = tempReshaped;

    temp = Sxi_D_0xi.col(1)*SxiF2e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF2_t_SxiF2e = tempReshaped;

    temp = Sxi_D_0xi.col(2)*SxiF2e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF3_t_SxiF2e = tempReshaped;

    temp = Sxi_D_0xi.col(0)*SxiF3e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF1_t_SxiF3e = tempReshaped;

    temp = Sxi_D_0xi.col(1)*SxiF3e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF2_t_SxiF3e = tempReshaped;

    temp = Sxi_D_0xi.col(2)*SxiF3e.transpose();
    ReshapeMat2Vec(temp, tempReshaped);
    ChVectorN<double, 27> SxiF3_t_SxiF3e = tempReshaped;
    ...
    if (m_damping_enabled) {        //If linear Kelvin-Voigt viscoelastic material model is enabled 
        ...
        temp = Sxi_D_0xi.col(0)*SxiF1edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF1_t_SxiF1edot = tempReshaped;

        temp = Sxi_D_0xi.col(1)*SxiF1edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF2_t_SxiF1edot = tempReshaped;

        temp = Sxi_D_0xi.col(2)*SxiF1edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF3_t_SxiF1edot = tempReshaped;

        temp = Sxi_D_0xi.col(0)*SxiF2edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF1_t_SxiF2edot = tempReshaped;

        temp = Sxi_D_0xi.col(1)*SxiF2edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF2_t_SxiF2edot = tempReshaped;

        temp = Sxi_D_0xi.col(2)*SxiF2edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF3_t_SxiF2edot = tempReshaped;

        temp = Sxi_D_0xi.col(0)*SxiF3edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF1_t_SxiF3edot = tempReshaped;

        temp = Sxi_D_0xi.col(1)*SxiF3edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF2_t_SxiF3edot = tempReshaped;

        temp = Sxi_D_0xi.col(2)*SxiF3edot.transpose();
        ReshapeMat2Vec(temp, tempReshaped);
        ChVectorN<double, 27> SxiF3_t_SxiF3edot = tempReshaped;
        ...
        ChMatrixNM<double, 27, 27> Jac_GQ_Point =
            ...
            (D0(3)*(Kfactor + m_Alpha*Rfactor))*(SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e*SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e.transpose()) +
            (D0(4)*(Kfactor + m_Alpha*Rfactor))*(SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e*SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e.transpose()) +
            (D0(5)*(Kfactor + m_Alpha*Rfactor))*(SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e*SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e.transpose()) + 
            ...
            (D0(3)*m_Alpha*Kfactor)*(SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e*SxiF2_t_SxiF3edot_plus_SxiF3_t_SxiF2edot.transpose()) +
            (D0(4)*m_Alpha*Kfactor)*(SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e*SxiF1_t_SxiF3edot_plus_SxiF3_t_SxiF1edot.transpose()) +
            (D0(5)*m_Alpha*Kfactor)*(SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e*SxiF1_t_SxiF2edot_plus_SxiF2_t_SxiF1edot.transpose());
        ...
        ChMatrixNM<double, 9, 9> Jac_GQ_Point_Part2Compact =
            ...
            (D0(3)*(Kfactor*epsilon(3) + m_Alpha*Rfactor*epsilon_dot(3)))*(SxiF2_t_SxiF3_compact + SxiF2_t_SxiF3_compact.transpose()) +
            (D0(4)*(Kfactor*epsilon(4) + m_Alpha*Rfactor*epsilon_dot(4)))*(SxiF1_t_SxiF3_compact + SxiF1_t_SxiF3_compact.transpose()) +
            (D0(5)*(Kfactor*epsilon(5) + m_Alpha*Rfactor*epsilon_dot(5)))*(SxiF1_t_SxiF2_compact + SxiF1_t_SxiF2_compact.transpose());
        ...
        ChMatrixNM<double, 9, 9> Jac_GQ_Point_Part2Compact =
            D0(0)*epsilon(0)*SxiF1_t_SxiF1_compact +
            D0(1)*epsilon(1)*SxiF2_t_SxiF2_compact +
            D0(2)*epsilon(2)*SxiF3_t_SxiF3_compact +
...
void ChElementBeamANCF_MT10::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac, double Kfactor, double Rfactor, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {

    ChMatrixNM<double, 9, 3> Sxi_D_0xi = m_SD_precompute_Dv.block(9 * index, 0, 9, 3);  //Faster to use this than to reference m_SD_precompute_D0 directly in the calculations 
    ChMatrix33<double> F = e_bar.transpose()*Sxi_D_0xi;
    ChVectorN<double, 3> SxiF1e = F.col(0);
    ChVectorN<double, 3> SxiF2e = F.col(1);
    ChVectorN<double, 3> SxiF3e = F.col(2);
    ...
    if (m_damping_enabled) {        //If linear Kelvin-Voigt viscoelastic material model is enabled 
        ...
        ChMatrixNM<double, 27, 27> Jac_GQ_Point =
            Dv(0, 0)*(SxiF1_t_SxiF1e*SxiF1_t_SxiF1e.transpose()) +
            Dv(0, 1)*(SxiF1_t_SxiF1ee_t_SxiF2_t_SxiF2)+
            Dv(0, 2)*(SxiF1_t_SxiF1ee_t_SxiF3_t_SxiF3)+
            Dv(1, 0)*(SxiF1_t_SxiF1ee_t_SxiF2_t_SxiF2.transpose()) +
            Dv(1, 1)*(SxiF2_t_SxiF2e*SxiF2_t_SxiF2e.transpose()) +
            Dv(1, 2)*(SxiF2_t_SxiF2ee_t_SxiF3_t_SxiF3)+
        ...
        ChMatrixNM<double, 9, 9> Jac_GQ_Point_Part2Compact =
            Dv(0, 0)*epsilon(0)*SxiF1_t_SxiF1_compact +
            Dv(0, 1)*epsilon(1)*SxiF1_t_SxiF1_compact +
            Dv(0, 2)*epsilon(2)*SxiF1_t_SxiF1_compact +
            Dv(1, 0)*epsilon(0)*SxiF2_t_SxiF2_compact +
            Dv(1, 1)*epsilon(1)*SxiF2_t_SxiF2_compact +
            Dv(1, 2)*epsilon(2)*SxiF2_t_SxiF2_compact +
            Dv(2, 0)*epsilon(0)*SxiF3_t_SxiF3_compact +
            Dv(2, 1)*epsilon(1)*SxiF3_t_SxiF3_compact +
...
void ChElementBeamANCF_MT10::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT10::ComputeInternalJacobiansAnalytic(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
    ChMatrixNM<double, 9, 3> e_bar;
    ChMatrixNM<double, 9, 3> e_bar_dot;
...
void ChElementBeamANCF_MT10::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT10::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT10::Calc_Sxi_D(ChMatrixNM<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT10::CalcCoordMatrix(ChMatrixNM<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT10::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT10::CalcCoordDerivMatrix(ChMatrixNM<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT10::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT10::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT10::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNM<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT10::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT10::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_MT10::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
...
// void ChElementBeamANCF_MT10::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT10::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT10::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT10::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT10::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT10::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT10::ComputeNF(
...
double ChElementBeamANCF_MT10::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT10::ComputeTangent(const double U) {
    ChMatrixNM<double, 9, 3> e_bar;
    ChMatrixNM<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT10(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT10::GetStaticGQTables() {
    return &static_tables_MT10;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT10 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT10::ChMaterialBeamANCF_MT10(double rho,        // material density
...
void ChMaterialBeamANCF_MT10::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
