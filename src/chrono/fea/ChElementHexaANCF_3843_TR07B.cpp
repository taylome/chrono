...
ChElementHexaANCF_3843_TR07B::ChElementHexaANCF_3843_TR07B()
...
void ChElementHexaANCF_3843_TR07B::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementHexaANCF_3843_TR07B::SetDimensions(double lenX, double lenY, double lenZ) {
    m_lenX = lenX;
    m_lenY = lenY;
    m_lenZ = lenZ;
}
...
void ChElementHexaANCF_3843_TR07B::SetMaterial(std::shared_ptr<ChMaterialHexaANCF> brick_mat) {
    m_material = brick_mat;
}

// Set the value for the single term structural damping coefficient.

void ChElementHexaANCF_3843_TR07B::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementHexaANCF_3843_TR07B::GetGreenLagrangeStrain(const double xi,
                                                                  const double eta,
                                                                  const double zeta) {
...
ChMatrix33<> ChElementHexaANCF_3843_TR07B::GetPK2Stress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementHexaANCF_3843_TR07B::GetVonMissesStress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
void ChElementHexaANCF_3843_TR07B::SetupInitial(ChSystem* system) {
...
void ChElementHexaANCF_3843_TR07B::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementHexaANCF_3843_TR07B::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementHexaANCF_3843_TR07B::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementHexaANCF_3843_TR07B::ComputeNodalMass() {
...
void ChElementHexaANCF_3843_TR07B::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    Eigen::Map<ArrayNIP> F11(m_FC.block<1, NIP>(0, 0 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> F12(m_FC.block<1, NIP>(0, 1 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> F13(m_FC.block<1, NIP>(0, 2 * NIP).data(), 1, NIP);

    Eigen::Map<ArrayNIP> F21(m_FC.block<1, NIP>(1, 0 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> F22(m_FC.block<1, NIP>(1, 1 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> F23(m_FC.block<1, NIP>(1, 2 * NIP).data(), 1, NIP);

    Eigen::Map<ArrayNIP> F31(m_FC.block<1, NIP>(2, 0 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> F32(m_FC.block<1, NIP>(2, 1 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> F33(m_FC.block<1, NIP>(2, 2 * NIP).data(), 1, NIP);

    Eigen::Map<ArrayNIP> Fdot11(m_FC.block<1, NIP>(3, 0 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> Fdot12(m_FC.block<1, NIP>(3, 1 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> Fdot13(m_FC.block<1, NIP>(3, 2 * NIP).data(), 1, NIP);

    Eigen::Map<ArrayNIP> Fdot21(m_FC.block<1, NIP>(4, 0 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> Fdot22(m_FC.block<1, NIP>(4, 1 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> Fdot23(m_FC.block<1, NIP>(4, 2 * NIP).data(), 1, NIP);

    Eigen::Map<ArrayNIP> Fdot31(m_FC.block<1, NIP>(5, 0 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> Fdot32(m_FC.block<1, NIP>(5, 1 * NIP).data(), 1, NIP);
    Eigen::Map<ArrayNIP> Fdot33(m_FC.block<1, NIP>(5, 2 * NIP).data(), 1, NIP);
    ...
    m_SPK2_1 = D(0, 0) * epsilon_1 + D(0, 1) * epsilon_2 + D(0, 2) * epsilon_3 + D(0, 3) * epsilon_4 +
               D(0, 4) * epsilon_5 + D(0, 5) * epsilon_6;
    m_SPK2_2 = D(1, 0) * epsilon_1 + D(1, 1) * epsilon_2 + D(1, 2) * epsilon_3 + D(1, 3) * epsilon_4 +
               D(1, 4) * epsilon_5 + D(1, 5) * epsilon_6;
    m_SPK2_3 = D(2, 0) * epsilon_1 + D(2, 1) * epsilon_2 + D(2, 2) * epsilon_3 + D(2, 3) * epsilon_4 +
               D(2, 4) * epsilon_5 + D(2, 5) * epsilon_6;
    m_SPK2_4 = D(3, 0) * epsilon_1 + D(3, 1) * epsilon_2 + D(3, 2) * epsilon_3 + D(3, 3) * epsilon_4 +
               D(3, 4) * epsilon_5 + D(3, 5) * epsilon_6;
    m_SPK2_5 = D(4, 0) * epsilon_1 + D(4, 1) * epsilon_2 + D(4, 2) * epsilon_3 + D(4, 3) * epsilon_4 +
               D(4, 4) * epsilon_5 + D(4, 5) * epsilon_6;
    m_SPK2_6 = D(5, 0) * epsilon_1 + D(5, 1) * epsilon_2 + D(5, 2) * epsilon_3 + D(5, 3) * epsilon_4 +
    ...
    P_Block.block<NIP, 1>(0 * NIP, 0).array().transpose() = F11 * m_SPK2_1 + F12 * m_SPK2_6 + F13 * m_SPK2_5;  // PT11
    P_Block.block<NIP, 1>(1 * NIP, 0).array().transpose() = F11 * m_SPK2_6 + F12 * m_SPK2_2 + F13 * m_SPK2_4;  // PT21
    P_Block.block<NIP, 1>(2 * NIP, 0).array().transpose() = F11 * m_SPK2_5 + F12 * m_SPK2_4 + F13 * m_SPK2_3;  // PT31

    P_Block.block<NIP, 1>(0 * NIP, 1).array().transpose() = F21 * m_SPK2_1 + F22 * m_SPK2_6 + F23 * m_SPK2_5;  // PT12
    P_Block.block<NIP, 1>(1 * NIP, 1).array().transpose() = F21 * m_SPK2_6 + F22 * m_SPK2_2 + F23 * m_SPK2_4;  // PT22
    P_Block.block<NIP, 1>(2 * NIP, 1).array().transpose() = F21 * m_SPK2_5 + F22 * m_SPK2_4 + F23 * m_SPK2_3;  // PT32

    P_Block.block<NIP, 1>(0 * NIP, 2).array().transpose() = F31 * m_SPK2_1 + F32 * m_SPK2_6 + F33 * m_SPK2_5;  // PT13
    P_Block.block<NIP, 1>(1 * NIP, 2).array().transpose() = F31 * m_SPK2_6 + F32 * m_SPK2_2 + F33 * m_SPK2_4;  // PT23
    P_Block.block<NIP, 1>(2 * NIP, 2).array().transpose() = F31 * m_SPK2_5 + F32 * m_SPK2_4 + F33 * m_SPK2_3;  // PT33
    ...
    // Setup mapping for Fi in Matrix form for the generalized internal force calculations
...
void ChElementHexaANCF_3843_TR07B::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                            double Kfactor,
                                                            double Rfactor,
                                                            double Mfactor) {
    ...
    for (unsigned int i = 0; i < NSF; i++) {
        ...
        S_scaled_SD_row_i.segment(0 * NIP, NIP).array() = m_SPK2_1 * m_SD.block<1, NIP>(i, 0 * NIP).array() +
                                                          m_SPK2_6 * m_SD.block<1, NIP>(i, 1 * NIP).array() +
                                                          m_SPK2_5 * m_SD.block<1, NIP>(i, 2 * NIP).array();
        S_scaled_SD_row_i.segment(1 * NIP, NIP).array() = m_SPK2_6 * m_SD.block<1, NIP>(i, 0 * NIP).array() +
                                                          m_SPK2_2 * m_SD.block<1, NIP>(i, 1 * NIP).array() +
                                                          m_SPK2_4 * m_SD.block<1, NIP>(i, 2 * NIP).array();
        S_scaled_SD_row_i.segment(2 * NIP, NIP).array() = m_SPK2_5 * m_SD.block<1, NIP>(i, 0 * NIP).array() +
                                                          m_SPK2_4 * m_SD.block<1, NIP>(i, 1 * NIP).array() +
                                                          m_SPK2_3 * m_SD.block<1, NIP>(i, 2 * NIP).array();
...
void ChElementHexaANCF_3843_TR07B::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementHexaANCF_3843_TR07B::EvaluateElementFrame(const double xi,
                                                        const double eta,
                                                        const double zeta,
                                                        ChVector<>& point,
                                                        ChQuaternion<>& rot) {
...
void ChElementHexaANCF_3843_TR07B::EvaluateElementPoint(const double xi,
                                                        const double eta,
                                                        const double zeta,
                                                        ChVector<>& point) {
    VectorN Sxi_compact;
...
void ChElementHexaANCF_3843_TR07B::EvaluateElementVel(double xi, double eta, const double zeta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta);
...
void ChElementHexaANCF_3843_TR07B::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementHexaANCF_3843_TR07B::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementHexaANCF_3843_TR07B::LoadableStateIncrement(const unsigned int off_x,
                                                          ChState& x_new,
                                                          const ChState& x,
...
void ChElementHexaANCF_3843_TR07B::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementHexaANCF_3843_TR07B::ComputeNF(
...
double ChElementHexaANCF_3843_TR07B::GetDensity() {
...
void ChElementHexaANCF_3843_TR07B::ComputeMassMatrixAndGravityForce() {
...
void ChElementHexaANCF_3843_TR07B::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();

    m_SD.resize(NSF, 3 * NIP);
    m_kGQ.resize(1, NIP);
    m_FC.resize(6, 3 * NIP);
    m_SPK2_1.resize(1, NIP);
    m_SPK2_2.resize(1, NIP);
    m_SPK2_3.resize(1, NIP);
    m_SPK2_4.resize(1, NIP);
    m_SPK2_5.resize(1, NIP);
    m_SPK2_6.resize(1, NIP);
...
void ChElementHexaANCF_3843_TR07B::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
    Sxi_compact(0) =
...
void ChElementHexaANCF_3843_TR07B::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
void ChElementHexaANCF_3843_TR07B::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
void ChElementHexaANCF_3843_TR07B::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta) {
...
void ChElementHexaANCF_3843_TR07B::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
    VectorN Sxi_D_col;
...
void ChElementHexaANCF_3843_TR07B::CalcCoordVector(Vector3N& e) {
...
void ChElementHexaANCF_3843_TR07B::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementHexaANCF_3843_TR07B::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementHexaANCF_3843_TR07B::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementHexaANCF_3843_TR07B::CalcCombinedCoordMatrix(Matrix6xN& ebar_ebardot) {
...
void ChElementHexaANCF_3843_TR07B::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementHexaANCF_3843_TR07B::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
ChQuadratureTables static_tables_3843_TR07B(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementHexaANCF_3843_TR07B::GetStaticGQTables() {
    return &static_tables_3843_TR07B;
}
