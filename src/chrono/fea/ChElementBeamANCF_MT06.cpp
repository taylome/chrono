...
// =============================================================================
//
// MT06 = MT05 + pre-computation of key matrices for the internal force calculations
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT06::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT06::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT06::ChElementBeamANCF_MT06() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT06::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT06::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT06::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT06::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT06::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementBeamANCF_MT06::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT06::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT06::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_MT06::ComputeNodalMass() {
...
void ChElementBeamANCF_MT06::SetAlphaDamp(double a) {
    m_Alpha = a;
...
void ChElementBeamANCF_MT06::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ...
    //Calculate the portion of the Selective Reduced Integration that account for the Poisson effect, but only on the beam axis
    if (GetStrainFormulation() == ChElementBeamANCF_MT06::StrainFormulation::CMPoisson) {
...
#if false
void ChElementBeamANCF_MT06::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ...
    I_3x3.setIdentity();
    F.noalias() = e_bar.transpose()*m_SD_precompute_D0.block(9 * index, 0, 9, 3);
    ...
    if (m_damping_enabled) {
        ...
        F_dot.noalias() = e_bar_dot.transpose()*m_SD_precompute_D0.block(9 * index, 0, 9, 3);

        //Combine alpha*E_dot with E (i.e E_combined = E + Alpha*E)
    ...
    QiCompact.noalias() = m_SD_precompute_D0.block(9 * index, 0, 9, 3)*P_transpose_scaled;
...
#else //Slower
    P_transpose_scaled.noalias() = m_GQWeight_det_J_0xi_D0(index)*F*S; //S is symmetric, so S' = S
    QiCompact.noalias() = P_transpose_scaled*m_SD_precompute_D0.block(9 * index, 0, 9, 3).transpose();
    Qi << QiCompact.transpose().array();
#endif
...
void ChElementBeamANCF_MT06::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChMatrix33<double> F;                   //Deformation Gradient tensor (non-symmetric tensor)
...
void ChElementBeamANCF_MT06::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
...
void ChElementBeamANCF_MT06::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT06::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT06::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT06::Calc_Sxi_D(ChMatrixNM<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT06::CalcCoordMatrix(ChMatrixNM<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT06::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT06::CalcCoordDerivMatrix(ChMatrixNM<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT06::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT06::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT06::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNM<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT06::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT06::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_MT06::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
...
// void ChElementBeamANCF_MT06::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT06::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT06::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT06::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT06::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT06::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT06::ComputeNF(
...
double ChElementBeamANCF_MT06::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT06::ComputeTangent(const double U) {
    ChMatrixNM<double, 9, 3> e_bar;
    ChMatrixNM<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT06(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT06::GetStaticGQTables() {
    return &static_tables_MT06;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT06 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT06::ChMaterialBeamANCF_MT06(double rho,        // material density
...
void ChMaterialBeamANCF_MT06::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
