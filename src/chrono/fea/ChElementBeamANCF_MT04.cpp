...
// =============================================================================
//
// MT04 = MT03 + additional optimizations, but still no pre-computation
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT04::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT04::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT04::ChElementBeamANCF_MT04() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT04::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT04::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT04::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT04::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT04::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementBeamANCF_MT04::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT04::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT04::ComputeNodalMass() {
...
void ChElementBeamANCF_MT04::SetAlphaDamp(double a) {
    m_Alpha = a;
    if (std::abs(m_Alpha) > 1e-10)
        m_damping_enabled = true;
}

void ChElementBeamANCF_MT04::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBeamANCF_MT04::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ...
    if (GetStrainFormulation() == ChElementBeamANCF_MT04::StrainFormulation::CMPoisson) {
        for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
...
void ChElementBeamANCF_MT04::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, double xi, double eta, double zeta, ChMatrixNM<double,6,6>& D, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ...
    //        Qi(3 * i + j) = P.row(j)*Sxi_D_0xi.row(i).transpose();
    //    }
    //}
    for (unsigned int i = 0; i < 9; i++) {
            Qi.segment(3*i,3) = P*Sxi_D_0xi.row(i).transpose();
    }

    Qi *= -J_0xi.determinant();
}
...
void ChElementBeamANCF_MT04::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT04::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT04::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT04::Calc_Sxi_D(ChMatrixNM<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT04::CalcCoordMatrix(ChMatrixNM<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT04::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT04::CalcCoordDerivMatrix(ChMatrixNM<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT04::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT04::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    ChMatrixNM<double, 9, 3> Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBeamANCF_MT04::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNM<double, 9, 3> Sxi_D;
    ChMatrix33<double> J_0xi;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT04::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT04::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
//}
//
// void ChElementBeamANCF_MT04::EvaluateSectionFrame(const double u,
//                                             const double v,
//                                             ChVector<>& point,
//                                             ChQuaternion<>& rot) {
//    // this is not a corotational element, so just do:
...
//}
//
// void ChElementBeamANCF_MT04::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT04::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT04::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT04::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT04::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT04::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT04::ComputeNF(
...
double ChElementBeamANCF_MT04::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT04::ComputeTangent(const double U) {
    ChMatrixNM<double, 9, 3> e_bar;
    ChMatrixNM<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT04(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT04::GetStaticGQTables() {
    return &static_tables_MT04;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT04 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT04::ChMaterialBeamANCF_MT04(double rho,        // material density
...
void ChMaterialBeamANCF_MT04::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
