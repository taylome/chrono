...
#ifndef CHELEMENTBEAMANCF3243TR05GQ322_H
#define CHELEMENTBEAMANCF3243TR05GQ322_H
...
class ChApi ChMaterialBeamANCF_3243_TR05_GQ322 {
  public:
    /// Construct an isotropic material.
    ChMaterialBeamANCF_3243_TR05_GQ322(double rho,        ///< material density
...
class ChApi ChElementBeamANCF_3243_TR05_GQ322 : public ChElementBeam, public ChLoadableU, public ChLoadableUVW {
  public:
    ...
    ChElementBeamANCF_3243_TR05_GQ322();
    ~ChElementBeamANCF_3243_TR05_GQ322() {}
    ...
    void SetMaterial(std::shared_ptr<ChMaterialBeamANCF_3243_TR05_GQ322> beam_mat) { m_material = beam_mat; }
    ...
    /// Return the material.
    std::shared_ptr<ChMaterialBeamANCF_3243_TR05_GQ322> GetMaterial() const { return m_material; }
  ...
  private:
    ...
    // Calculate the current 3x8 matrix of nodal coordinates.
    void CalcCoordMatrix(ChMatrixNM<double, 3, 8>& ebar);

    // Calculate the current 24x1 vector of nodal coordinate time derivatives.
    void CalcCoordDerivVector(ChVectorN<double, 24>& edot);

    // Calculate the current 3x8 matrix of nodal coordinate time derivatives.
    void CalcCoordDerivMatrix(ChMatrixNM<double, 3, 8>& ebardot);
    ...
    // Calculate the 3x24 Sparse & Repetitive Derivative of the Normalized Shape Function Matrix with respect to xi
    void Calc_Sxi_xi(ChMatrixNM<double, 3, 24>& Sxi_xi, double xi, double eta, double zeta);

    // Calculate the 8x1 Compact Vector of the Derivative of the Normalized Shape Functions with respect to xi
    void Calc_Sxi_xi_compact(ChVectorN<double, 8>& Sxi_xi_compact, double xi, double eta, double zeta);

    // Calculate the 3x24 Sparse & Repetitive Derivative of the Normalized Shape Function Matrix with respect to eta
    void Calc_Sxi_eta(ChMatrixNM<double, 3, 24>& Sxi_eta, double xi, double eta, double zeta);

    // Calculate the 8x1 Compact Vector of the Derivative of the Normalized Shape Functions with respect to eta
    void Calc_Sxi_eta_compact(ChVectorN<double, 8>& Sxi_eta_compact, double xi, double eta, double zeta);

    // Calculate the 3x24 Sparse & Repetitive Derivative of the Normalized Shape Function Matrix with respect to zeta
    void Calc_Sxi_zeta(ChMatrixNM<double, 3, 24>& Sxi_zeta, double xi, double eta, double zeta);

    // Calculate the 8x1 Compact Vector of the Derivative of the Normalized Shape Functions with respect to zeta
    void Calc_Sxi_zeta_compact(ChVectorN<double, 8>& Sxi_zeta_compact, double xi, double eta, double zeta);
    ...
    std::shared_ptr<ChMaterialBeamANCF_3243_TR05_GQ322> m_material;  ///< beam material
    StrainFormulation m_strain_form;                           ///< Strain formulation
    ChMatrixNM<double, 3, 8> m_ebar0;  ///< Element Position Coordinate Vector for the Reference Configuration
    ChMatrixNMc<double, 8, 36> m_SD_precompute_D0;  ///< Precomputed corrected normalized shape function derivative
                                                    ///< matrices for no Poisson Effect
    ChVectorN<double, 12> m_GQWeight_det_J_0xi_D0;  ///< Precomputed Gauss-Quadrature Weight & Element Jacobian scale
                                                    ///< factors for no Poisson Effect
    ChMatrixNMc<double, 8, 9> m_SD_precompute_Dv;  ///< Precomputed corrected normalized shape function derivative
                                                    ///< matrices for Poisson Effect on the beam axis only
    ChVectorN<double, 3> m_GQWeight_det_J_0xi_Dv;   ///< Precomputed Gauss-Quadrature Weight & Element Jacobian scale
