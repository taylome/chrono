...
void ChElementBeamANCF_MT30::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
    ...
    for (unsigned int i = 0; i < 9; i++) {
        for (unsigned int j = 0; j < 9; j++) {
            JacobianMatrixLiu(3 * i, 3 * j) += m_K13Compact(i, j) + Mfactor * m_MassMatrix(i, j);
            JacobianMatrixLiu(3 * i + 1, 3 * j + 1) += m_K13Compact(i, j) + Mfactor * m_MassMatrix(i, j);
            JacobianMatrixLiu(3 * i + 2, 3 * j + 2) += m_K13Compact(i, j) + Mfactor * m_MassMatrix(i, j);
        }
    }
