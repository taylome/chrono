...
ChElementBrickANCF_3843_TR07::ChElementBrickANCF_3843_TR07()
    : m_gravity_on(false), m_lenX(0), m_lenY(0), m_lenZ(0), m_Alpha(0), m_damping_enabled(false) {
    m_nodes.resize(8);

    m_SD_precompute_D.resize(32, 648);
    m_GQWeight_det_J_0xi_D.resize(216,1);
    m_SD_precompute_col_ordered.resize(32, 648);
    m_F_Transpose_CombinedBlock_col_ordered.resize(648, 3);
    m_F_Transpose_CombinedBlockDamping_col_ordered.resize(648, 6);
...
void ChElementBrickANCF_3843_TR07::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementBrickANCF_3843_TR07::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementBrickANCF_3843_TR07::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBrickANCF_3843_TR07::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBrickANCF_3843_TR07::ComputeKRMmatricesGlobal(ChMatrixRef H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
...
void ChElementBrickANCF_3843_TR07::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBrickANCF_3843_TR07::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBrickANCF_3843_TR07::ComputeNodalMass() {
...
void ChElementBrickANCF_3843_TR07::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    unsigned int GQ_idx_xi_eta = 5;    // 6 Point Gauss-Quadrature;
    unsigned int GQ_idx_zeta = 5;      // 6 Point Gauss-Quadrature;
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi_eta].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_xi_eta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_zeta].size(); it_zeta++) {
                ...
                m_SD_precompute_D.block(0, 3 * index, 32, 3) = Sxi_D * J_0xi.inverse();
    ...
    for (auto i = 0; i < 216; i++) {
        ...
        m_SD_precompute_col_ordered.col(i + 216) = m_SD_precompute_D.col(3 * i + 1);
        m_SD_precompute_col_ordered.col(i + 432) = m_SD_precompute_D.col(3 * i + 2);
    }
}
...
void ChElementBrickANCF_3843_TR07::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementBrickANCF_3843_TR07::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBrickANCF_3843_TR07::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>  E0_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0));
    E0_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1));
    E0_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2));
    E0_Block.array() -= 1;
    E0_Block *= 0.5;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E_BlockDamping = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 3));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 4));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 5));
    E0_Block += m_Alpha * E_BlockDamping;
    E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>  E1_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0));
    E1_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1));
    E1_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2));
    E1_Block.array() -= 1;
    E1_Block *= 0.5;
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 3));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 4));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 5));
    E1_Block += m_Alpha * E_BlockDamping;
    E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>  E2_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0));
    E2_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1));
    E2_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2));
    E2_Block.array() -= 1;
    E2_Block *= 0.5;
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 3));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 4));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 5));
    E2_Block += m_Alpha * E_BlockDamping;
    E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>  E3_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0));
    E3_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1));
    E3_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2));
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 3));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).cwiseProduct(
    ...
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 5));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 3));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 4));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).cwiseProduct(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>  E4_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0));
    E4_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1));
    E4_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2));
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 3));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).cwiseProduct(
    ...
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 5));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 3));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 4));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).cwiseProduct(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>  E5_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0));
    E5_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1));
    E5_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2));
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 3));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).cwiseProduct(
    ...
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 0) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 0) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).cwiseProduct(m_SPK2_4_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).cwiseProduct(m_SPK2_4_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 2) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 2) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).cwiseProduct(m_SPK2_4_Block);

    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 0) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 0) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 2) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 2) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).cwiseProduct(m_SPK2_3_Block);

    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 0) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 0) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).cwiseProduct(m_SPK2_2_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).cwiseProduct(m_SPK2_2_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 2) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 2) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).cwiseProduct(m_SPK2_2_Block);
...
void ChElementBrickANCF_3843_TR07::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    VectorNIP E0_Block = m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0));
    E0_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1));
    E0_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2));
    E0_Block.array() -= 1;
    E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E0_Block *= 0.5;

    VectorNIP E1_Block = m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0));
    E1_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1));
    E1_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2));
    E1_Block.array() -= 1;
    E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E1_Block *= 0.5;

    VectorNIP E2_Block = m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0));
    E2_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1));
    E2_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2));
    E2_Block.array() -= 1;
    E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E2_Block *= 0.5;

    VectorNIP E3_Block = m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0));
    E3_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1));
    E3_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2));
    E3_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    VectorNIP E4_Block = m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0));
    E4_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1));
    E4_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2));
    E4_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    VectorNIP E5_Block = m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0));
    E5_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1));
    E5_Block += m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2));
    E5_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    ...
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 0) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 0) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0).cwiseProduct(m_SPK2_4_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1).cwiseProduct(m_SPK2_4_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 2) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(0, 2) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2).cwiseProduct(m_SPK2_4_Block);

    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 0) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 0) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 2) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(216, 2) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2).cwiseProduct(m_SPK2_3_Block);

    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 0) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 0) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0).cwiseProduct(m_SPK2_2_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1).cwiseProduct(m_SPK2_2_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 2) =
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block<216, 1>(432, 2) +=
        m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2).cwiseProduct(m_SPK2_2_Block);
...
void ChElementBrickANCF_3843_TR07::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
    double Kfactor,
    double Rfactor) {
...
void ChElementBrickANCF_3843_TR07::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
    ...
    for (auto i = 0; i < 32; i++) {
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 216).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 432).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 648).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 864).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 1080).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 0).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 0).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 216).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 432).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 648).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 864).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 1080).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 1).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 1).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 216).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 432).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 648).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 864).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(432, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 1080).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(0, 2).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<216, 1>(216, 2).transpose());
    }

    //ChMatrixNMc<double, 648, 3> scaled_F_Transpose_col_ordered =
    //    (Kfactor + m_Alpha * Rfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<648, 3>(0, 0) +
    //    (m_Alpha * Kfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<648, 3>(0, 3);
    ...
    scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<648, 3>(0, 0) +
        (m_Alpha * Kfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<648, 3>(0, 3);
    ...
    for (auto i = 0; i < 32; i++) {
        S_scaled_SD_precompute_col_ordered.block<1, 216>(i, 0).noalias() =
            m_SPK2_0_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 0)) +
            m_SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 216)) +
            m_SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 432));

        S_scaled_SD_precompute_col_ordered.block<1, 216>(i, 216).noalias() =
            m_SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 0)) +
            m_SPK2_1_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 216)) +
            m_SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 432));

        S_scaled_SD_precompute_col_ordered.block<1, 216>(i, 432).noalias() =
            m_SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 0)) +
            m_SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 216)) +
            m_SPK2_2_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 216>(i, 432));
    }
    ...
    ChMatrixNM<double, 32, 32> Jacobian_CompactPart =
...
void ChElementBrickANCF_3843_TR07::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    // ChMatrixNM<double, 48, 180> partial_epsilon_partial_e_Transpose;
    //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 48, 432> partial_epsilon_partial_e_Transpose;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> partial_epsilon_partial_e_Transpose;
    partial_epsilon_partial_e_Transpose.resize(96, 1296);
    ...
    for (auto i = 0; i < 32; i++) {
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 216).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 432).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 648).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 864).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>(3 * i, 1080).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 0).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 0).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 216).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 432).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 648).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 864).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 1, 1080).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 1).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 1).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 216).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 432).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 648).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 864).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 432).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(432, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 216>((3 * i) + 2, 1080).noalias() = m_SD_precompute_col_ordered.block<1, 216>(i, 216).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(0, 2).transpose()) +
            m_SD_precompute_col_ordered.block<1, 216>(i, 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<216, 1>(216, 2).transpose());
    }
...
void ChElementBrickANCF_3843_TR07::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR07::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR07::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR07::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR07::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR07::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR07::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR07::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta) {
...
void ChElementBrickANCF_3843_TR07::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
...
void ChElementBrickANCF_3843_TR07::CalcCoordVector(Vector3N& e) {
...
void ChElementBrickANCF_3843_TR07::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementBrickANCF_3843_TR07::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementBrickANCF_3843_TR07::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
    ebar.row(3) = m_nodes[0]->GetDDD().eigen();
...
void ChElementBrickANCF_3843_TR07::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementBrickANCF_3843_TR07::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementBrickANCF_3843_TR07::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementBrickANCF_3843_TR07::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementBrickANCF_3843_TR07::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBrickANCF_3843_TR07::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
void ChElementBrickANCF_3843_TR07::EvaluateSectionFrame(const double xi,
                                                        const double eta,
                                                        const double zeta,
                                                        ChVector<>& point,
                                                        ChQuaternion<>& rot) {
...
void ChElementBrickANCF_3843_TR07::EvaluateSectionPoint(const double u, const double v, const double w, ChVector<>& point) {
    Matrix3x3N Sxi;
    Calc_Sxi(Sxi, u, v, w);
...
void ChElementBrickANCF_3843_TR07::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBrickANCF_3843_TR07::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBrickANCF_3843_TR07::LoadableStateIncrement(const unsigned int off_x,
                                                          ChState& x_new,
                                                          const ChState& x,
...
void ChElementBrickANCF_3843_TR07::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBrickANCF_3843_TR07::ComputeNF(
...
double ChElementBrickANCF_3843_TR07::GetDensity() {
...
ChQuadratureTables static_tables_3843_TR07(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBrickANCF_3843_TR07::GetStaticGQTables() {
    return &static_tables_3843_TR07;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBrickANCF_3843_TR07 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBrickANCF_3843_TR07::ChMaterialBrickANCF_3843_TR07(double rho,  // material density
...
ChMaterialBrickANCF_3843_TR07::ChMaterialBrickANCF_3843_TR07(
...
// Poisson effect
void ChMaterialBrickANCF_3843_TR07::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G) {
