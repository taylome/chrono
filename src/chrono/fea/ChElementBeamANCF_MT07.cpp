...
//// MIKE
//// Add function to manually set the initial configuration of the element (What update functions should this call?
///Right now, if I change the material, I don't think the mass matrix gets updated / Should the Mass Matrix be stored as
///a sparse matrix? If so, make this change in a later revision / Check with Radu on function calls (e.g. calculated
///det_J_0xi).  If this can be improved, make this change in a later revision / Ask Radu about Heap vs. Stack calls with
///Matrices.
////
//// What should be done with ChVector<> ChElementBeamANCF_MT07::EvaluateBeamSectionStrains()... It doesn't seems to
///make sense for this element since there are no input arguments /   In reality, there should be a function that
///returns the entire stress and strain tensors (which version(s)) is more of a question. / There is another block of
///base class functions that are commented out in the current Chrono implementation.  What needs to be done with these?
////
//// When is the right time to add new content, like the ability to apply a torque to an element (or different
///materials)?  Currently only applied forces appear to be supported
////
//// For ChElementBeamANCF_MT07::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being
///asked for /   Can the 1D versions just call the 3D version to eliminate code duplication?
////
//// What is "EvaluateSectionVelNorm(double U, ChVector<>& Result)"?  Is it even needed?
////
//// Figure out what is missing to enable irrlicht animations (missing "beam section" functions...How does this affect
///the entire interface for the element?)
...
ChElementBeamANCF_MT07::ChElementBeamANCF_MT07()
...
void ChElementBeamANCF_MT07::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT07::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT07::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT07::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT07::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
    ...
    // ChMatrixNM<double, 27, 27> JacobianMatrix_Analytic;
...
void ChElementBeamANCF_MT07::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT07::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT07::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_eta_zeta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_eta_zeta].size(); it_zeta++) {
                ...
                m_SD_precompute_D0.block(9 * index, 0, 9, 3) = Sxi_D * J_0xi.inverse();
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
        ...
        m_SD_precompute_Dv.block(9 * it_xi, 0, 9, 3) = Sxi_D * J_0xi.inverse();
        m_GQWeight_det_J_0xi_Dv(it_xi) = -J_0xi.determinant() * GQ_weight;
    }
}

/// This class computes and adds corresponding masses to ElementGeneric member m_TotalMass
void ChElementBeamANCF_MT07::ComputeNodalMass() {
...
void ChElementBeamANCF_MT07::SetAlphaDamp(double a) {
    m_Alpha = a;
...
void ChElementBeamANCF_MT07::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    ChMatrixNM<double, 9, 3> e_bar_dot;
...
void ChElementBeamANCF_MT07::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
                                                          ChMatrixNM<double, 9, 3>& e_bar,
                                                          ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ...
    for (unsigned int index = 0; index < m_GQWeight_det_J_0xi_D0.size(); index++) {
        ComputeInternalForcesSingleGQPnt(Qi, index, D0, e_bar, e_bar_dot);
        Fi += Qi;
    }

    // Calculate the portion of the Selective Reduced Integration that account for the Poisson effect, but only on the
    // beam axis
    if (GetStrainFormulation() == ChElementBeamANCF_MT07::StrainFormulation::CMPoisson) {
        for (unsigned int index = 0; index < m_GQWeight_det_J_0xi_Dv.size(); index++) {
            ComputeInternalForcesSingleGQPnt(Qi, index, Dv, e_bar, e_bar_dot);
            Fi += Qi;
        }
    }
}

void ChElementBeamANCF_MT07::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi,
                                                              unsigned int index,
                                                              ChVectorN<double, 6>& D0,
                                                              ChMatrixNM<double, 9, 3>& e_bar,
                                                              ChMatrixNM<double, 9, 3>& e_bar_dot) {
    // ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized
    // configuration ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;  // Matrix of normalized shape function derivatives corrected for a potentially
                                         // non-straight reference configuration
    ChMatrix33<double> F;  // Deformation Gradient tensor (non-symmetric tensor)
    ChMatrix33<double> E;  // Green-Lagrange strain tensor (symmetric tensor) - potentially combined with some damping
                           // contributions for speed
    ChMatrix33<double> S;                   // 2nd Piola-Kirchoff stress tensor (symmetric tensor)
    ChMatrix33<double> P_transpose_scaled;  // Transpose of the 1st Piola-Kirchoff stress tensor (non-symmetric tensor)
                                            // Scaled by the determinate of the element Jacobian
    ...
    // Calc_Sxi_D(Sxi_D, xi, eta, zeta);
    // J_0xi.noalias() = m_e0_bar.transpose()*Sxi_D;
    Sxi_D_0xi.noalias() = m_SD_precompute_D0.block(
        9 * index, 0, 9, 3);  // Faster to use this than to reference m_SD_precompute_D0 directly in the calculations
    ...
    S(0, 0) = D0(0) * E(0, 0);
    S(1, 0) = 2.0 * D0(5) * E(0, 1);
    S(2, 0) = 2.0 * D0(4) * E(0, 2);
    S(0, 1) = S(1, 0);
    S(1, 1) = D0(1) * E(1, 1);
    S(2, 1) = 2.0 * D0(3) * E(1, 2);
    S(0, 2) = S(2, 0);
    S(1, 2) = S(2, 1);
    S(2, 2) = D0(2) * E(2, 2);

    // Calculate the transpose of the 1st Piola Kirchoff Stress Tensor
    // Scale the PK1 Stress Tensor by J_0xi.determinant() so that this multiplication is already included when
    // calculating Qi
    ...
    QiCompact.noalias() = Sxi_D_0xi * P_transpose_scaled;
    // QiCompact.noalias() = -J_0xi.determinant()*F*S*Sxi_D_0xi.transpose();  //slower

    // A faster way to execute the following assignment of matrix terms into the generalized force output vector
    // for (unsigned int i = 0; i < 9; i++) {
    //    Qi.segment(3 * i, 3).noalias() = QiCompact.row(i);
    //}
    Qi << QiCompact.transpose().array();
    //for (auto i = 0; i < 27; i++) {
    //    Qi(i) = QiCompact.data()[i];
    //}
}
...
void ChElementBeamANCF_MT07::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi,
    ...
    ChMatrix33<double> E;  // Green-Lagrange strain tensor (symmetric tensor) - potentially combined with some damping
                           // contributions for speed
    ChVectorN<double, 3> S;  // 2nd Piola-Kirchoff stress tensor (symmetric tensor) - Diagonal in this special case due
                             // to the structure of the 6x6 stiffness matrix
    ChMatrix33<double> P_transpose_scaled;  // Transpose of the 1st Piola-Kirchoff stress tensor (non-symmetric tensor)
                                            // Scaled by the determinate of the element Jacobian
    ChMatrix33<double> I_3x3;            // 3x3 identity matrix
    ChMatrixNM<double, 9, 3> QiCompact;  // The compact result of the final generalized internal force computation
...
// -----------------------------------------------------------------------------
//static void ReshapeMat2Vec(ChMatrixNM<double, 9, 3>& M, ChVectorN<double, 27>& V) {
//    for (auto i = 0; i < 27; i++) {
//        V(i) = M.data()[i];
//    }
//}
...
void ChElementBeamANCF_MT07::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac,
                                                                double Kfactor,
                                                                double Rfactor,
                                                                unsigned int index,
                                                                ChVectorN<double, 6>& D0,
                                                                ChMatrixNM<double, 9, 3>& e_bar,
                                                                ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ChMatrixNM<double, 9, 3> Sxi_D_0xi = m_SD_precompute_D0.block(
        9 * index, 0, 9, 3);  // Faster to use this than to reference m_SD_precompute_D0 directly in the calculations
    ChMatrix33<double> F = e_bar.transpose() * Sxi_D_0xi;
    ChVectorN<double, 3> SxiF1e = F.col(0);
    ChVectorN<double, 3> SxiF2e = F.col(1);
    ChVectorN<double, 3> SxiF3e = F.col(2);

    ChVectorN<double, 6> epsilon;
    epsilon << 0.5 * (SxiF1e.transpose() * SxiF1e - 1), 0.5 * (SxiF2e.transpose() * SxiF2e - 1),
        0.5 * (SxiF3e.transpose() * SxiF3e - 1), SxiF2e.transpose() * SxiF3e, SxiF1e.transpose() * SxiF3e,
        SxiF1e.transpose() * SxiF2e;

    ChMatrixNM<double, 9, 3> SxiF1_t_SxiF1e_compact = Sxi_D_0xi.col(0) * SxiF1e.transpose();
    ChVectorN<double, 27> SxiF1_t_SxiF1e;
    SxiF1_t_SxiF1e << SxiF1_t_SxiF1e_compact.transpose().array();
    //ReshapeMat2Vec(SxiF1_t_SxiF1e_compact, SxiF1_t_SxiF1e);

    ChMatrixNM<double, 9, 3> SxiF2_t_SxiF1e_compact = Sxi_D_0xi.col(1) * SxiF1e.transpose();
    ChVectorN<double, 27> SxiF2_t_SxiF1e;
    SxiF2_t_SxiF1e << SxiF2_t_SxiF1e_compact.transpose().array();

    ChMatrixNM<double, 9, 3> SxiF3_t_SxiF1e_compact = Sxi_D_0xi.col(2) * SxiF1e.transpose();
    ChVectorN<double, 27> SxiF3_t_SxiF1e;
    SxiF3_t_SxiF1e << SxiF3_t_SxiF1e_compact.transpose().array();

    ChMatrixNM<double, 9, 3> SxiF1_t_SxiF2e_compact = Sxi_D_0xi.col(0) * SxiF2e.transpose();
    ChVectorN<double, 27> SxiF1_t_SxiF2e;
    SxiF1_t_SxiF2e << SxiF1_t_SxiF2e_compact.transpose().array();

    ChMatrixNM<double, 9, 3> SxiF2_t_SxiF2e_compact = Sxi_D_0xi.col(1) * SxiF2e.transpose();
    ChVectorN<double, 27> SxiF2_t_SxiF2e;
    SxiF2_t_SxiF2e << SxiF2_t_SxiF2e_compact.transpose().array();

    ChMatrixNM<double, 9, 3> SxiF3_t_SxiF2e_compact = Sxi_D_0xi.col(2) * SxiF2e.transpose();
    ChVectorN<double, 27> SxiF3_t_SxiF2e;
    SxiF3_t_SxiF2e << SxiF3_t_SxiF2e_compact.transpose().array();

    ChMatrixNM<double, 9, 3> SxiF1_t_SxiF3e_compact = Sxi_D_0xi.col(0) * SxiF3e.transpose();
    ChVectorN<double, 27> SxiF1_t_SxiF3e;
    SxiF1_t_SxiF3e << SxiF1_t_SxiF3e_compact.transpose().array();

    ChMatrixNM<double, 9, 3> SxiF2_t_SxiF3e_compact = Sxi_D_0xi.col(1) * SxiF3e.transpose();
    ChVectorN<double, 27> SxiF2_t_SxiF3e;
    SxiF2_t_SxiF3e << SxiF2_t_SxiF3e_compact.transpose().array();

    ChMatrixNM<double, 9, 3> SxiF3_t_SxiF3e_compact = Sxi_D_0xi.col(2) * SxiF3e.transpose();
    ChVectorN<double, 27> SxiF3_t_SxiF3e;
    SxiF3_t_SxiF3e << SxiF3_t_SxiF3e_compact.transpose().array();

    ChMatrixNM<double, 9, 9> SxiF1_t_SxiF1_compact = Sxi_D_0xi.col(0) * Sxi_D_0xi.col(0).transpose();
    ChMatrixNM<double, 9, 9> SxiF2_t_SxiF2_compact = Sxi_D_0xi.col(1) * Sxi_D_0xi.col(1).transpose();
    ChMatrixNM<double, 9, 9> SxiF3_t_SxiF3_compact = Sxi_D_0xi.col(2) * Sxi_D_0xi.col(2).transpose();
    ChMatrixNM<double, 9, 9> SxiF1_t_SxiF2_compact = Sxi_D_0xi.col(0) * Sxi_D_0xi.col(1).transpose();
    ChMatrixNM<double, 9, 9> SxiF1_t_SxiF3_compact = Sxi_D_0xi.col(0) * Sxi_D_0xi.col(2).transpose();
    ChMatrixNM<double, 9, 9> SxiF2_t_SxiF3_compact = Sxi_D_0xi.col(1) * Sxi_D_0xi.col(2).transpose();

    ChVectorN<double, 27> SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e = SxiF1_t_SxiF2e + SxiF2_t_SxiF1e;
    ChVectorN<double, 27> SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e = SxiF1_t_SxiF3e + SxiF3_t_SxiF1e;
    ChVectorN<double, 27> SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e = SxiF2_t_SxiF3e + SxiF3_t_SxiF2e;
    ...
    if (m_damping_enabled) {  // If linear Kelvin-Voigt viscoelastic material model is enabled
        ...
        epsilon_dot(4) = 2.0 * E_dot(0, 2);
        epsilon_dot(5) = 2.0 * E_dot(0, 1);
        ChVectorN<double, 3> SxiF1edot = F_dot.col(0);
        ChVectorN<double, 3> SxiF2edot = F_dot.col(1);
        ChVectorN<double, 3> SxiF3edot = F_dot.col(2);

        ChMatrixNM<double, 9, 3> SxiF1_t_SxiF1edot_compact = Sxi_D_0xi.col(0) * SxiF1edot.transpose();
        ChVectorN<double, 27> SxiF1_t_SxiF1edot;
        SxiF1_t_SxiF1edot << SxiF1_t_SxiF1edot_compact.transpose().array();

        ChMatrixNM<double, 9, 3> SxiF2_t_SxiF1edot_compact = Sxi_D_0xi.col(1) * SxiF1edot.transpose();
        ChVectorN<double, 27> SxiF2_t_SxiF1edot;
        SxiF2_t_SxiF1edot << SxiF2_t_SxiF1edot_compact.transpose().array();

        ChMatrixNM<double, 9, 3> SxiF3_t_SxiF1edot_compact = Sxi_D_0xi.col(2) * SxiF1edot.transpose();
        ChVectorN<double, 27> SxiF3_t_SxiF1edot;
        SxiF3_t_SxiF1edot << SxiF3_t_SxiF1edot_compact.transpose().array();

        ChMatrixNM<double, 9, 3> SxiF1_t_SxiF2edot_compact = Sxi_D_0xi.col(0) * SxiF2edot.transpose();
        ChVectorN<double, 27> SxiF1_t_SxiF2edot;
        SxiF1_t_SxiF2edot << SxiF1_t_SxiF2edot_compact.transpose().array();

        ChMatrixNM<double, 9, 3> SxiF2_t_SxiF2edot_compact = Sxi_D_0xi.col(1) * SxiF2edot.transpose();
        ChVectorN<double, 27> SxiF2_t_SxiF2edot;
        SxiF2_t_SxiF2edot << SxiF2_t_SxiF2edot_compact.transpose().array();

        ChMatrixNM<double, 9, 3> SxiF3_t_SxiF2edot_compact = Sxi_D_0xi.col(2) * SxiF2edot.transpose();
        ChVectorN<double, 27> SxiF3_t_SxiF2edot;
        SxiF3_t_SxiF2edot << SxiF3_t_SxiF2edot_compact.transpose().array();

        ChMatrixNM<double, 9, 3> SxiF1_t_SxiF3edot_compact = Sxi_D_0xi.col(0) * SxiF3edot.transpose();
        ChVectorN<double, 27> SxiF1_t_SxiF3edot;
        SxiF1_t_SxiF3edot << SxiF1_t_SxiF3edot_compact.transpose().array();

        ChMatrixNM<double, 9, 3> SxiF2_t_SxiF3edot_compact = Sxi_D_0xi.col(1) * SxiF3edot.transpose();
        ChVectorN<double, 27> SxiF2_t_SxiF3edot;
        SxiF2_t_SxiF3edot << SxiF2_t_SxiF3edot_compact.transpose().array();

        ChMatrixNM<double, 9, 3> SxiF3_t_SxiF3edot_compact = Sxi_D_0xi.col(2) * SxiF3edot.transpose();
        ChVectorN<double, 27> SxiF3_t_SxiF3edot;
        SxiF3_t_SxiF3edot << SxiF3_t_SxiF3edot_compact.transpose().array();

        ChVectorN<double, 27> SxiF1_t_SxiF2edot_plus_SxiF2_t_SxiF1edot = SxiF1_t_SxiF2edot + SxiF2_t_SxiF1edot;
        ChVectorN<double, 27> SxiF1_t_SxiF3edot_plus_SxiF3_t_SxiF1edot = SxiF1_t_SxiF3edot + SxiF3_t_SxiF1edot;
        ChVectorN<double, 27> SxiF2_t_SxiF3edot_plus_SxiF3_t_SxiF2edot = SxiF2_t_SxiF3edot + SxiF3_t_SxiF2edot;

        ChMatrixNM<double, 27, 27> Jac_GQ_Point =
            (D0(0) * (Kfactor + m_Alpha * Rfactor)) * (SxiF1_t_SxiF1e * SxiF1_t_SxiF1e.transpose()) +
            (D0(1) * (Kfactor + m_Alpha * Rfactor)) * (SxiF2_t_SxiF2e * SxiF2_t_SxiF2e.transpose()) +
            (D0(2) * (Kfactor + m_Alpha * Rfactor)) * (SxiF3_t_SxiF3e * SxiF3_t_SxiF3e.transpose()) +
            (D0(3) * (Kfactor + m_Alpha * Rfactor)) *
                (SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e * SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e.transpose()) +
            (D0(4) * (Kfactor + m_Alpha * Rfactor)) *
                (SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e * SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e.transpose()) +
            (D0(5) * (Kfactor + m_Alpha * Rfactor)) *
                (SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e * SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e.transpose()) +
            (D0(0) * m_Alpha * Kfactor) * (SxiF1_t_SxiF1e * SxiF1_t_SxiF1edot.transpose()) +
            (D0(1) * m_Alpha * Kfactor) * (SxiF2_t_SxiF2e * SxiF2_t_SxiF2edot.transpose()) +
            (D0(2) * m_Alpha * Kfactor) * (SxiF3_t_SxiF3e * SxiF3_t_SxiF3edot.transpose()) +
            (D0(3) * m_Alpha * Kfactor) *
                (SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e * SxiF2_t_SxiF3edot_plus_SxiF3_t_SxiF2edot.transpose()) +
            (D0(4) * m_Alpha * Kfactor) *
                (SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e * SxiF1_t_SxiF3edot_plus_SxiF3_t_SxiF1edot.transpose()) +
            (D0(5) * m_Alpha * Kfactor) *
                (SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e * SxiF1_t_SxiF2edot_plus_SxiF2_t_SxiF1edot.transpose());

        ChMatrixNM<double, 9, 9> Jac_GQ_Point_Part2Compact =
            (D0(0) * (Kfactor * epsilon(0) + m_Alpha * Rfactor * epsilon_dot(0))) * SxiF1_t_SxiF1_compact +
            (D0(1) * (Kfactor * epsilon(1) + m_Alpha * Rfactor * epsilon_dot(1))) * SxiF2_t_SxiF2_compact +
            (D0(2) * (Kfactor * epsilon(2) + m_Alpha * Rfactor * epsilon_dot(2))) * SxiF3_t_SxiF3_compact +
            (D0(3) * (Kfactor * epsilon(3) + m_Alpha * Rfactor * epsilon_dot(3))) *
                (SxiF2_t_SxiF3_compact + SxiF2_t_SxiF3_compact.transpose()) +
            (D0(4) * (Kfactor * epsilon(4) + m_Alpha * Rfactor * epsilon_dot(4))) *
                (SxiF1_t_SxiF3_compact + SxiF1_t_SxiF3_compact.transpose()) +
            (D0(5) * (Kfactor * epsilon(5) + m_Alpha * Rfactor * epsilon_dot(5))) *
                (SxiF1_t_SxiF2_compact + SxiF1_t_SxiF2_compact.transpose());

        // In MATLAB notation:
        // Jac_GQ_Point(1:3:end,1:3:end) = Jac_GQ_Point(1:3:end,1:3:end) + Jac_GQ_Point_Part2Compact;
        // Jac_GQ_Point(2:3:end,2:3:end) = Jac_GQ_Point(2:3:end,2:3:end) + Jac_GQ_Point_Part2Compact;
        // Jac_GQ_Point(3:3:end,3:3:end) = Jac_GQ_Point(3:3:end,3:3:end) + Jac_GQ_Point_Part2Compact;
        for (unsigned int i = 0; i < 9; i++) {
            for (unsigned int j = 0; j < 9; j++) {
                Jac_GQ_Point(3 * i, 3 * j) += Jac_GQ_Point_Part2Compact(i, j);
                Jac_GQ_Point(3 * i + 1, 3 * j + 1) += Jac_GQ_Point_Part2Compact(i, j);
                Jac_GQ_Point(3 * i + 2, 3 * j + 2) += Jac_GQ_Point_Part2Compact(i, j);
            }
        }
        Jac += m_GQWeight_det_J_0xi_D0(index) * Jac_GQ_Point;
    } else {
        ChMatrixNM<double, 27, 27> Jac_GQ_Point =
            D0(0) * (SxiF1_t_SxiF1e * SxiF1_t_SxiF1e.transpose()) +
            D0(1) * (SxiF2_t_SxiF2e * SxiF2_t_SxiF2e.transpose()) +
            D0(2) * (SxiF3_t_SxiF3e * SxiF3_t_SxiF3e.transpose()) +
            D0(3) * (SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e * SxiF2_t_SxiF3e_plus_SxiF3_t_SxiF2e.transpose()) +
            D0(4) * (SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e * SxiF1_t_SxiF3e_plus_SxiF3_t_SxiF1e.transpose()) +
            D0(5) * (SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e * SxiF1_t_SxiF2e_plus_SxiF2_t_SxiF1e.transpose());

        ChMatrixNM<double, 9, 9> Jac_GQ_Point_Part2Compact =
            D0(0) * 0.5 * epsilon(0) * SxiF1_t_SxiF1_compact + D0(1) * 0.5 * epsilon(1) * SxiF2_t_SxiF2_compact +
            D0(2) * 0.5 * epsilon(2) * SxiF3_t_SxiF3_compact +
            D0(3) * epsilon(3) * (SxiF2_t_SxiF3_compact + SxiF2_t_SxiF3_compact.transpose()) +
            D0(4) * epsilon(4) * (SxiF1_t_SxiF3_compact + SxiF1_t_SxiF3_compact.transpose()) +
            D0(5) * epsilon(5) * (SxiF1_t_SxiF2_compact + SxiF1_t_SxiF2_compact.transpose());
        ...
        Jac += Kfactor * m_GQWeight_det_J_0xi_D0(index) * Jac_GQ_Point;
    }
}
...
void ChElementBeamANCF_MT07::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac,
                                                                ...
                                                                ChMatrixNM<double, 9, 3>& e_bar,
                                                                ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ChMatrixNM<double, 9, 3> Sxi_D_0xi = m_SD_precompute_Dv.block(
        9 * index, 0, 9, 3);  // Faster to use this than to reference m_SD_precompute_D0 directly in the calculations
    ChMatrix33<double> F = e_bar.transpose() * Sxi_D_0xi;
    ChVectorN<double, 3> SxiF1e = F.col(0);
    ChVectorN<double, 3> SxiF2e = F.col(1);
    ...
    ChVectorN<double, 3> epsilon;
    epsilon << 0.5 * (SxiF1e.transpose() * SxiF1e - 1), 0.5 * (SxiF2e.transpose() * SxiF2e - 1),
        0.5 * (SxiF3e.transpose() * SxiF3e - 1);
    ...
    ChMatrixNM<double, 27, 27> SxiF1_t_SxiF1ee_t_SxiF2_t_SxiF2 = SxiF1_t_SxiF1e * SxiF2_t_SxiF2e.transpose();
    ChMatrixNM<double, 27, 27> SxiF1_t_SxiF1ee_t_SxiF3_t_SxiF3 = SxiF1_t_SxiF1e * SxiF3_t_SxiF3e.transpose();
    ChMatrixNM<double, 27, 27> SxiF2_t_SxiF2ee_t_SxiF3_t_SxiF3 = SxiF2_t_SxiF2e * SxiF3_t_SxiF3e.transpose();

    if (m_damping_enabled) {  // If linear Kelvin-Voigt viscoelastic material model is enabled
        ChMatrix33<double> F_dot;
        ChMatrix33<double> E_dot;
        ChVectorN<double, 3> epsilon_dot;
        F_dot.noalias() = e_bar_dot.transpose() * Sxi_D_0xi;
        E_dot.noalias() = 0.5 * (F_dot.transpose() * F + F.transpose() * F_dot);
        epsilon_dot(0) = E_dot(0, 0);
        epsilon_dot(1) = E_dot(1, 1);
        ...
        ChMatrixNM<double, 27, 27> Jac_GQ_Point =
            (Dv(0, 0) * (Kfactor + m_Alpha * Rfactor)) * (SxiF1_t_SxiF1e * SxiF1_t_SxiF1e.transpose()) +
            (Dv(0, 1) * (Kfactor + m_Alpha * Rfactor)) * (SxiF1_t_SxiF1ee_t_SxiF2_t_SxiF2) +
            (Dv(0, 2) * (Kfactor + m_Alpha * Rfactor)) * (SxiF1_t_SxiF1ee_t_SxiF3_t_SxiF3) +
            (Dv(1, 0) * (Kfactor + m_Alpha * Rfactor)) * (SxiF1_t_SxiF1ee_t_SxiF2_t_SxiF2.transpose()) +
            (Dv(1, 1) * (Kfactor + m_Alpha * Rfactor)) * (SxiF2_t_SxiF2e * SxiF2_t_SxiF2e.transpose()) +
            (Dv(1, 2) * (Kfactor + m_Alpha * Rfactor)) * (SxiF2_t_SxiF2ee_t_SxiF3_t_SxiF3) +
            (Dv(2, 0) * (Kfactor + m_Alpha * Rfactor)) * (SxiF1_t_SxiF1ee_t_SxiF3_t_SxiF3.transpose()) +
            (Dv(2, 1) * (Kfactor + m_Alpha * Rfactor)) * (SxiF2_t_SxiF2ee_t_SxiF3_t_SxiF3.transpose()) +
            (Dv(2, 2) * (Kfactor + m_Alpha * Rfactor)) * (SxiF3_t_SxiF3e * SxiF3_t_SxiF3e.transpose()) +
            (Dv(0, 0) * m_Alpha * Kfactor) * (SxiF1_t_SxiF1e * SxiF1_t_SxiF1edot.transpose()) +
            (Dv(0, 1) * m_Alpha * Kfactor) * (SxiF1_t_SxiF1e * SxiF2_t_SxiF2edot.transpose()) +
            (Dv(0, 2) * m_Alpha * Kfactor) * (SxiF1_t_SxiF1e * SxiF3_t_SxiF3edot.transpose()) +
            (Dv(1, 0) * m_Alpha * Kfactor) * (SxiF2_t_SxiF2e * SxiF1_t_SxiF1edot.transpose()) +
            (Dv(1, 1) * m_Alpha * Kfactor) * (SxiF2_t_SxiF2e * SxiF2_t_SxiF2edot.transpose()) +
            (Dv(1, 2) * m_Alpha * Kfactor) * (SxiF2_t_SxiF2e * SxiF3_t_SxiF3edot.transpose()) +
            (Dv(2, 0) * m_Alpha * Kfactor) * (SxiF3_t_SxiF3e * SxiF1_t_SxiF1edot.transpose()) +
            (Dv(2, 1) * m_Alpha * Kfactor) * (SxiF3_t_SxiF3e * SxiF2_t_SxiF2edot.transpose()) +
            (Dv(2, 2) * m_Alpha * Kfactor) * (SxiF3_t_SxiF3e * SxiF3_t_SxiF3edot.transpose());

        ChMatrixNM<double, 9, 9> Jac_GQ_Point_Part2Compact =
            (Dv(0, 0) * (Kfactor * epsilon(0) + m_Alpha * Rfactor * epsilon_dot(0))) * SxiF1_t_SxiF1_compact +
            (Dv(0, 1) * (Kfactor * epsilon(1) + m_Alpha * Rfactor * epsilon_dot(1))) * SxiF1_t_SxiF1_compact +
            (Dv(0, 2) * (Kfactor * epsilon(2) + m_Alpha * Rfactor * epsilon_dot(2))) * SxiF1_t_SxiF1_compact +
            (Dv(1, 0) * (Kfactor * epsilon(0) + m_Alpha * Rfactor * epsilon_dot(0))) * SxiF2_t_SxiF2_compact +
            (Dv(1, 1) * (Kfactor * epsilon(1) + m_Alpha * Rfactor * epsilon_dot(1))) * SxiF2_t_SxiF2_compact +
            (Dv(1, 2) * (Kfactor * epsilon(2) + m_Alpha * Rfactor * epsilon_dot(2))) * SxiF2_t_SxiF2_compact +
            (Dv(2, 0) * (Kfactor * epsilon(0) + m_Alpha * Rfactor * epsilon_dot(0))) * SxiF3_t_SxiF3_compact +
            (Dv(2, 1) * (Kfactor * epsilon(1) + m_Alpha * Rfactor * epsilon_dot(1))) * SxiF3_t_SxiF3_compact +
            (Dv(2, 2) * (Kfactor * epsilon(2) + m_Alpha * Rfactor * epsilon_dot(2))) * SxiF3_t_SxiF3_compact;
        ...
        Jac += m_GQWeight_det_J_0xi_Dv(index) * Jac_GQ_Point;
    } else {
        ChMatrixNM<double, 27, 27> Jac_GQ_Point =
            Dv(0, 0) * (SxiF1_t_SxiF1e * SxiF1_t_SxiF1e.transpose()) + Dv(0, 1) * (SxiF1_t_SxiF1ee_t_SxiF2_t_SxiF2) +
            Dv(0, 2) * (SxiF1_t_SxiF1ee_t_SxiF3_t_SxiF3) + Dv(1, 0) * (SxiF1_t_SxiF1ee_t_SxiF2_t_SxiF2.transpose()) +
            Dv(1, 1) * (SxiF2_t_SxiF2e * SxiF2_t_SxiF2e.transpose()) + Dv(1, 2) * (SxiF2_t_SxiF2ee_t_SxiF3_t_SxiF3) +
            Dv(2, 0) * (SxiF1_t_SxiF1ee_t_SxiF3_t_SxiF3.transpose()) +
            Dv(2, 1) * (SxiF2_t_SxiF2ee_t_SxiF3_t_SxiF3.transpose()) +
            Dv(2, 2) * (SxiF3_t_SxiF3e * SxiF3_t_SxiF3e.transpose());

        ChMatrixNM<double, 9, 9> Jac_GQ_Point_Part2Compact =
            Dv(0, 0) * epsilon(0) * SxiF1_t_SxiF1_compact + Dv(0, 1) * epsilon(1) * SxiF1_t_SxiF1_compact +
            Dv(0, 2) * epsilon(2) * SxiF1_t_SxiF1_compact + Dv(1, 0) * epsilon(0) * SxiF2_t_SxiF2_compact +
            Dv(1, 1) * epsilon(1) * SxiF2_t_SxiF2_compact + Dv(1, 2) * epsilon(2) * SxiF2_t_SxiF2_compact +
            Dv(2, 0) * epsilon(0) * SxiF3_t_SxiF3_compact + Dv(2, 1) * epsilon(1) * SxiF3_t_SxiF3_compact +
            Dv(2, 2) * epsilon(2) * SxiF3_t_SxiF3_compact;
        ...
        Jac += Kfactor * m_GQWeight_det_J_0xi_Dv(index) * Jac_GQ_Point;
    }
}

void ChElementBeamANCF_MT07::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix,
                                                      double Kfactor,
                                                      double Rfactor) {
...
void ChElementBeamANCF_MT07::ComputeInternalJacobiansAnalytic(ChMatrixNM<double, 27, 27>& JacobianMatrix,
                                                              double Kfactor,
                                                              double Rfactor) {
    ChMatrixNM<double, 9, 3> e_bar;
    ChMatrixNM<double, 9, 3> e_bar_dot;
    ...
    // Calculate the portion of the Selective Reduced Integration that does account for the Poisson effect
    for (unsigned int index = 0; index < m_GQWeight_det_J_0xi_D0.size(); index++) {
        ComputeInternalJacobianSingleGQPnt(JacobianMatrix, -Kfactor, -Rfactor, index, D0, e_bar, e_bar_dot);
    }
    ...
    if (GetStrainFormulation() == ChElementBeamANCF_MT07::StrainFormulation::CMPoisson) {
        for (unsigned int index = 0; index < m_GQWeight_det_J_0xi_Dv.size(); index++) {
            ComputeInternalJacobianSingleGQPnt(JacobianMatrix, -Kfactor, -Rfactor, index, Dv, e_bar, e_bar_dot);
        }
    }
}
...
void ChElementBeamANCF_MT07::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT07::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT07::Calc_Sxi_D(ChMatrixNM<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT07::CalcCoordMatrix(ChMatrixNM<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT07::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT07::CalcCoordDerivMatrix(ChMatrixNM<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT07::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT07::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT07::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNM<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
// ChVector<> ChElementBeamANCF_MT07::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT07::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_MT07::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    ChMatrixNM<double, 9, 3> e_bar;
    ChVectorN<double, 9> Sxi_compact;
    ChMatrixNM<double, 9, 3> Sxi_D;
...
// void ChElementBeamANCF_MT07::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT07::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT07::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT07::LoadableStateIncrement(const unsigned int off_x,
                                                    ChState& x_new,
                                                    const ChState& x,
...
// void ChElementBeamANCF_MT07::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT07::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT07::ComputeNF(
...
double ChElementBeamANCF_MT07::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT07::ComputeTangent(const double U) {
    ChMatrixNM<double, 9, 3> e_bar;
    ChMatrixNM<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT07(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT07::GetStaticGQTables() {
    return &static_tables_MT07;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT07 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT07::ChMaterialBeamANCF_MT07(double rho,        // material density
...
// Poisson effect
void ChMaterialBeamANCF_MT07::Calc_D0_Dv(const ChVector<>& E,
                                         const ChVector<>& nu,
                                         const ChVector<>& G,
                                         double k1,
                                         double k2) {
