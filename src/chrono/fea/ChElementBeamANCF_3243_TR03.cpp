...
ChElementBeamANCF_3243_TR03::ChElementBeamANCF_3243_TR03()
...
void ChElementBeamANCF_3243_TR03::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementBeamANCF_3243_TR03::SetDimensions(double lenX, double thicknessY, double thicknessZ) {
    m_lenX = lenX;
...
void ChElementBeamANCF_3243_TR03::SetMaterial(std::shared_ptr<ChMaterialBeamANCF> beam_mat) {
    m_material = beam_mat;
}

// Set the value for the single term structural damping coefficient.

void ChElementBeamANCF_3243_TR03::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementBeamANCF_3243_TR03::GetGreenLagrangeStrain(const double xi, const double eta, const double zeta) {
...
ChMatrix33<> ChElementBeamANCF_3243_TR03::GetPK2Stress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBeamANCF_3243_TR03::GetVonMissesStress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
void ChElementBeamANCF_3243_TR03::SetupInitial(ChSystem* system) {
...
void ChElementBeamANCF_3243_TR03::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_3243_TR03::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementBeamANCF_3243_TR03::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}

// This class computes and adds corresponding masses to ElementGeneric member m_TotalMass

void ChElementBeamANCF_3243_TR03::ComputeNodalMass() {
    m_nodes[0]->m_TotalMass += m_MassMatrix(0, 0) + m_MassMatrix(0, 12);
    m_nodes[1]->m_TotalMass += m_MassMatrix(12, 0) + m_MassMatrix(12, 12);
}

// Compute the generalized internal force vector for the current nodal coordinates and set the value in the Fi vector.

void ChElementBeamANCF_3243_TR03::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBeamANCF_3243_TR03::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                           double Kfactor,
                                                           double Rfactor,
                                                           double Mfactor) {
...
void ChElementBeamANCF_3243_TR03::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementBeamANCF_3243_TR03::EvaluateSectionFrame(const double xi, ChVector<>& point, ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementBeamANCF_3243_TR03::EvaluateSectionPoint(const double xi, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR03::EvaluateSectionVel(const double xi, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR03::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_3243_TR03::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_3243_TR03::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementBeamANCF_3243_TR03::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_3243_TR03::ComputeNF(
...
double ChElementBeamANCF_3243_TR03::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at coordinate xi [-1 to 1].

ChVector<> ChElementBeamANCF_3243_TR03::ComputeTangent(const double xi) {
    VectorN Sxi_xi_compact;
    Calc_Sxi_xi_compact(Sxi_xi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR03::ComputeMassMatrixAndGravityForce() {
...
void ChElementBeamANCF_3243_TR03::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[NP - 1].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[NT - 1].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[NT - 1].size(); it_zeta++) {
                ...
                Vector3N Qi;
                ComputeInternalForcesSingleGQPntD0(Qi, xi, eta, zeta, D0, ebar, ebardot);
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[NP - 1].size(); it_xi++) {
        ...
        double xi = GQTable->Lroots[NP - 1][it_xi];
        Vector3N Qi;
        ComputeInternalForcesSingleGQPntDv(Qi, xi, 0, 0, Dv, ebar, ebardot);

        Fi.noalias() += GQ_weight * Qi;
    }
}

void ChElementBeamANCF_3243_TR03::ComputeInternalForcesSingleGQPntD0(Vector3N& Qi,
                                                                     double xi,
                                                                     double eta,
                                                                     double zeta,
    ...
    sigmaPK2_combined(3) = F.col(1).dot(F.col(2)) + m_Alpha * (F.col(1).dot(Fdot.col(2)) + Fdot.col(1).dot(F.col(2)));
    sigmaPK2_combined(4) = F.col(0).dot(F.col(2)) + m_Alpha * (F.col(0).dot(Fdot.col(2)) + Fdot.col(0).dot(F.col(2)));
    sigmaPK2_combined(5) = F.col(0).dot(F.col(1)) + m_Alpha * (F.col(0).dot(Fdot.col(1)) + Fdot.col(0).dot(F.col(1)));
    sigmaPK2_combined.array() *= D0.array() * -J_0xi.determinant();
    ...
    Qi.noalias() = depsilon_de * sigmaPK2_combined;
}

void ChElementBeamANCF_3243_TR03::ComputeInternalForcesSingleGQPntDv(Vector3N& Qi,
                                                                     double xi,
                                                                     double eta,
                                                                     double zeta,
                                                                     const ChMatrix33<>& Dv,
    ...
    // Calculate the partial derivative of the Green-Lagrange strain tensor in Voigt notation with respect to the
    // element position coordinates.
    ChMatrixNMc<double, 3 * NSF, 3> depsilon_de;
    ...
    // Calculate the generalized internal force integrand for a Linear Kelvin-Voigt Viscoelastic material model
    Qi.noalias() = depsilon_de * Dv * epsilon_combined;
}
...
void ChElementBeamANCF_3243_TR03::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR03::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR03::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR03::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR03::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
    VectorN Sxi_D_col;
...
void ChElementBeamANCF_3243_TR03::CalcCoordVector(Vector3N& e) {
...
void ChElementBeamANCF_3243_TR03::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementBeamANCF_3243_TR03::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementBeamANCF_3243_TR03::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementBeamANCF_3243_TR03::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementBeamANCF_3243_TR03::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBeamANCF_3243_TR03::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
ChQuadratureTables static_tables_3243_TR03(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_3243_TR03::GetStaticGQTables() {
    return &static_tables_3243_TR03;
}
