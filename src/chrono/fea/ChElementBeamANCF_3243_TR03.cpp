...
void ChElementBeamANCF_3243_TR03::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[NP - 1].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[NT - 1].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[NT - 1].size(); it_zeta++) {
                ...
                Vector3N QiPnt;
                ComputeInternalForcesSingleGQPntD0(QiPnt, xi, eta, zeta, D0, ebar, ebardot);
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[NP - 1].size(); it_xi++) {
        ...
        double xi = GQTable->Lroots[NP - 1][it_xi];
        Vector3N QiPnt;
        ComputeInternalForcesSingleGQPntDv(QiPnt, xi, 0, 0, Dv, ebar, ebardot);
...
void ChElementBeamANCF_3243_TR03::CalcCombinedCoordMatrix(Matrix6xN& ebar_ebardot) {
