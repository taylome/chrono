...
ChElementBeamANCF_3243_TR11S::ChElementBeamANCF_3243_TR11S()
...
void ChElementBeamANCF_3243_TR11S::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementBeamANCF_3243_TR11S::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_3243_TR11S::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_3243_TR11S::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_3243_TR11S::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                           double Kfactor,
                                                           double Rfactor,
                                                           double Mfactor) {
    ...
    ChMatrixNMc<double, 8, 3> e_bar_plus_e_bar_dot;
    ...
    for (unsigned int i = 0; i < 8; i++) {
        for (unsigned int s = i; s < 8; s++) {
            ChMatrixNM<double, 8, 8> Ccompact = m_Ccompact.block<8, 8>(8 * index, 0);
            index++;
...
void ChElementBeamANCF_3243_TR11S::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_3243_TR11S::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_3243_TR11S::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    unsigned int GQ_idx_xi = 4;        // 5 Point Gauss-Quadrature;
    unsigned int GQ_idx_eta_zeta = 2;  // 3 Point Gauss-Quadrature;

    ChMatrixNM<double, 8, 8> K1compact;
    K1compact.setZero();
    m_Ccompact.setZero();
    m_K1.setZero();
    ...
    ChMatrixNM<double, 48, 8> epsilion_matrices;
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_eta_zeta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_eta_zeta].size(); it_zeta++) {
                ...
                epsilion_matrices.block<8, 8>(0, 0) = 0.5 * Sxi_D.col(0) * Sxi_D.col(0).transpose();
                epsilion_matrices.block<8, 8>(8, 0) = 0.5 * Sxi_D.col(1) * Sxi_D.col(1).transpose();
                epsilion_matrices.block<8, 8>(16, 0) = 0.5 * Sxi_D.col(2) * Sxi_D.col(2).transpose();
                epsilion_matrices.block<8, 8>(24, 0) =
                    0.5 * Sxi_D.col(1) * Sxi_D.col(2).transpose() + 0.5 * Sxi_D.col(2) * Sxi_D.col(1).transpose();
                epsilion_matrices.block<8, 8>(32, 0) =
                    0.5 * Sxi_D.col(0) * Sxi_D.col(2).transpose() + 0.5 * Sxi_D.col(2) * Sxi_D.col(0).transpose();
                epsilion_matrices.block<8, 8>(40, 0) =
                    ...
                    for (unsigned int j = i; j < 8; j++) {
                        for (unsigned int k = 0; k < 6; k++) {
                            for (unsigned int l = 0; l < 6; l++) {
                                m_Ccompact.block<8, 8>(8 * index, 0) +=
                                    J_0xi.determinant() * GQ_weight * 2 *
                                    epsilion_matrices.block<1, 8>(i + 8 * k, 0).transpose() * D(k, l) *
                                    epsilion_matrices.block<8, 1>(8 * l, j).transpose();
                            }
                        }
                        index++;
                    }
                ...
                for (unsigned int k = 0; k < 6; k++) {
                    for (unsigned int l = 0; l < 3; l++) {
                        K1compact +=
                            J_0xi.determinant() * GQ_weight * D(k, l) * epsilion_matrices.block<8, 8>(8 * k, 0);
                    }
                }
            }
        }
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
        ...
        for (unsigned int k = 0; k < 6; k++) {
            for (unsigned int l = 0; l < 3; l++) {
                K1compact += J_0xi.determinant() * GQ_weight * D(k, l) * epsilion_matrices.block<8, 8>(8 * k, 0);
            }
        }
    }
...
void ChElementBeamANCF_3243_TR11S::ComputeNodalMass() {
...
void ChElementBeamANCF_3243_TR11S::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementBeamANCF_3243_TR11S::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ChMatrixNMc<double, 8, 3> e_bar;
    ChVectorN<double, 24> edot;
...
void ChElementBeamANCF_3243_TR11S::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
    ...
    for (unsigned int i = 0; i < 8; i++) {
        for (unsigned int j = i; j < 8; j++) {
            Block = -e_bar.transpose() * m_Ccompact.block<8, 8>(8 * index, 0) * e_bar;
    ...
    ChMatrixNM<double, 8, 3> e_bar_rowmajor = e_bar;
    Eigen::Map<ChVectorN<double, 24>> e(e_bar_rowmajor.data(), e_bar_rowmajor.size());

    Fi = (m_K1 + m_K2) * e + m_2Alpha * m_K2 * e_dot;
}

void ChElementBeamANCF_3243_TR11S::ComputeInternalJacobians(ChMatrixNM<double, 24, 24>& JacobianMatrix,
                                                           double Kfactor,
                                                           double Rfactor) {
...
void ChElementBeamANCF_3243_TR11S::Calc_Sxi(ChMatrixNM<double, 3, 24>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_3243_TR11S::Calc_Sxi_compact(ChVectorN<double, 8>& Sxi_compact,
                                                   double xi,
                                                   double eta,
                                                   double zeta) {
...
void ChElementBeamANCF_3243_TR11S::Calc_Sxi_D(ChMatrixNMc<double, 8, 3>& Sxi_D, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR11S::CalcCoordMatrix(ChMatrixNMc<double, 8, 3>& e) {
...
void ChElementBeamANCF_3243_TR11S::CalcCoordVector(ChVectorN<double, 24>& e) {
...
void ChElementBeamANCF_3243_TR11S::CalcCoordDerivMatrix(ChMatrixNMc<double, 8, 3>& edot) {
...
void ChElementBeamANCF_3243_TR11S::CalcCoordDerivVector(ChVectorN<double, 24>& edot) {
...
void ChElementBeamANCF_3243_TR11S::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_3243_TR11S::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNMc<double, 8, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
// ChVector<> ChElementBeamANCF_3243_TR11S::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_3243_TR11S::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_3243_TR11S::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    ChMatrixNMc<double, 8, 3> e_bar;
    ChVectorN<double, 8> Sxi_compact;
    ChMatrixNMc<double, 8, 3> Sxi_D;
...
// void ChElementBeamANCF_3243_TR11S::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_3243_TR11S::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_3243_TR11S::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_3243_TR11S::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
// void ChElementBeamANCF_3243_TR11S::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_3243_TR11S::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_3243_TR11S::ComputeNF(
...
double ChElementBeamANCF_3243_TR11S::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_3243_TR11S::ComputeTangent(const double U) {
    ChMatrixNMc<double, 8, 3> e_bar;
    ChMatrixNMc<double, 8, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_2_TR11S(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_3243_TR11S::GetStaticGQTables() {
    return &static_tables_2_TR11S;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_3243_TR11S methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_3243_TR11S::ChMaterialBeamANCF_3243_TR11S(
...
// Poisson effect
void ChMaterialBeamANCF_3243_TR11S::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G,
                                              double k1,
                                              double k2) {
