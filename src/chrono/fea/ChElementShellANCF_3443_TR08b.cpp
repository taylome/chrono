...
ChElementShellANCF_3443_TR08b::ChElementShellANCF_3443_TR08b()
...
void ChElementShellANCF_3443_TR08b::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementShellANCF_3443_TR08b::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementShellANCF_3443_TR08b::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementShellANCF_3443_TR08b::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3443_TR08b::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                           double Kfactor,
                                                           double Rfactor,
                                                           double Mfactor) {
...
void ChElementShellANCF_3443_TR08b::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementShellANCF_3443_TR08b::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi_eta].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_xi_eta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_zeta].size(); it_zeta++) {
                ...
                m_GravForce.noalias() += (GQ_weight * rho * det_J_0xi) * Sxi.transpose() * g_acc.eigen();
                m_MassMatrix.noalias() += (GQ_weight * rho * det_J_0xi) * Sxi_compact * Sxi_compact.transpose();
            }
        }
    }
}
...
void ChElementShellANCF_3443_TR08b::ComputeNodalMass() {
...
void ChElementShellANCF_3443_TR08b::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementShellANCF_3443_TR08b::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementShellANCF_3443_TR08b::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    if (m_gravity_on) {
        Fi.noalias() += m_GravForce;
    }
}

void ChElementShellANCF_3443_TR08b::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const MatrixNx6& ebar_ebardot) {
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0));
    E0_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1));
    E0_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2));
    E0_Block.array() -= 1;
    E0_Block *= 0.5;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E_BlockDamping = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 3));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 4));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 5));
    E0_Block.noalias() += m_Alpha * E_BlockDamping;
    E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E1_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0));
    E1_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1));
    E1_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
    ...
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 4));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 5));
    E1_Block.noalias() += m_Alpha * E_BlockDamping;
    E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E2_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
    E2_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
    E2_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(
    ...
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 4));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 5));
    E2_Block.noalias() += m_Alpha * E_BlockDamping;
    E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E3_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
    E3_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
    E3_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2));
    E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 3));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
    ...
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 5));
    E3_Block.noalias() += m_Alpha * E_BlockDamping;
    E3_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E4_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
    E4_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
    E4_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2));
    E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 3));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
    ...
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 5));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 3));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 4));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 5));
    E4_Block.noalias() += m_Alpha * E_BlockDamping;
    E4_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E5_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0));
    E5_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1));
    E5_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2));
    E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 3));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
    ...
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 5));
    E5_Block.noalias() += m_Alpha * E_BlockDamping;
    E5_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    const ChMatrixNM<double, 6, 6>& D = GetMaterial()->Get_D();
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> SPK2_0_Block = D(0, 0)*E0_Block + D(0, 1)*E1_Block + D(0, 2)*E2_Block + D(0, 3)*E3_Block + D(0, 4)*E4_Block + D(0, 5)*E5_Block;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> SPK2_1_Block = D(1, 0)*E0_Block + D(1, 1)*E1_Block + D(1, 2)*E2_Block + D(1, 3)*E3_Block + D(1, 4)*E4_Block + D(1, 5)*E5_Block;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> SPK2_2_Block = D(2, 0)*E0_Block + D(2, 1)*E1_Block + D(2, 2)*E2_Block + D(2, 3)*E3_Block + D(2, 4)*E4_Block + D(2, 5)*E5_Block;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> SPK2_3_Block = D(3, 0)*E0_Block + D(3, 1)*E1_Block + D(3, 2)*E2_Block + D(3, 3)*E3_Block + D(3, 4)*E4_Block + D(3, 5)*E5_Block;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> SPK2_4_Block = D(4, 0)*E0_Block + D(4, 1)*E1_Block + D(4, 2)*E2_Block + D(4, 3)*E3_Block + D(4, 4)*E4_Block + D(4, 5)*E5_Block;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> SPK2_5_Block = D(5, 0)*E0_Block + D(5, 1)*E1_Block + D(5, 2)*E2_Block + D(5, 3)*E3_Block + D(5, 4)*E4_Block + D(5, 5)*E5_Block;
...
void ChElementShellANCF_3443_TR08b::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E0_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0));
    E0_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1));
    E0_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2));
    E0_Block.array() -= 1;
    E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E0_Block *= 0.5;

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E1_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0));
    E1_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1));
    E1_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2));
    E1_Block.array() -= 1;
    E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E1_Block *= 0.5;

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E2_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0));
    E2_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1));
    E2_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2));
    E2_Block.array() -= 1;
    E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E2_Block *= 0.5;

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E3_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0));
    E3_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1));
    E3_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2));
    E3_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E4_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0));
    E4_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1));
    E4_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2));
    E4_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 72, 1> E5_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0));
    E5_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1));
    E5_Block.noalias() += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2));
    E5_Block.array() *= m_GQWeight_det_J_0xi_D.array();
...
void ChElementShellANCF_3443_TR08b::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
    double Kfactor,
    double Rfactor) {
...
void ChElementShellANCF_3443_TR08b::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
    ...
    for (auto i = 0; i < 16; i++) {
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 0).noalias() =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 72).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 144).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 216).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 288).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 360).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 0).noalias() =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 72).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 144).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 216).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 288).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 360).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 0).noalias() =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 72).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 144).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 216).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 288).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 360).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose()) +
    ...
    scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<216, 3>(0, 0) +
        (m_Alpha * Kfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<216, 3>(0, 3);
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0));
    E0_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1));
    E0_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
    ...
    E0_Block.array() -= 1;
    E0_Block *= 0.5;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E_BlockDamping = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 3));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 4));
    E_BlockDamping.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E1_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0));
    E1_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1));
    E1_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E2_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
    E2_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
    E2_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E3_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
    E3_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
    E3_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E4_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
    E4_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
    E4_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E5_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0));
    E5_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
        F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1));
    E5_Block.noalias() += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
    ...
    Jacobian_CompactPart.noalias() += Mfactor * m_MassMatrix;
...
void ChElementShellANCF_3443_TR08b::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ...
    for (auto i = 0; i < 16; i++) {
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 0).noalias() =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 72).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 144).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 216).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 288).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose()) +
                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 360).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose()) +
                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 0).noalias() =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 72).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 144).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 216).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 288).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 360).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 0).noalias() =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 72).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 144).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 216).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).transpose()) +
        ...
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 288).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 360).noalias() = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose()) +
...
void ChElementShellANCF_3443_TR08b::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443_TR08b::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {    
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443_TR08b::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443_TR08b::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443_TR08b::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443_TR08b::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443_TR08b::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443_TR08b::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
void ChElementShellANCF_3443_TR08b::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
...
void ChElementShellANCF_3443_TR08b::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3443_TR08b::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3443_TR08b::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementShellANCF_3443_TR08b::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
    ebar.row(3) = m_nodes[0]->GetDDD().eigen();
...
void ChElementShellANCF_3443_TR08b::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3443_TR08b::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3443_TR08b::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementShellANCF_3443_TR08b::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementShellANCF_3443_TR08b::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementShellANCF_3443_TR08b::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
void ChElementShellANCF_3443_TR08b::EvaluateSectionFrame(const double xi, const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    Matrix3x3N Sxi;
    Matrix3x3N Sxi_xi;
    Matrix3x3N Sxi_eta;
...
void ChElementShellANCF_3443_TR08b::EvaluateSectionPoint(const double u, const double v, ChVector<>& point) {
    Matrix3x3N Sxi;
    Calc_Sxi(Sxi, u, v, 0);
...
void ChElementShellANCF_3443_TR08b::EvaluateSectionVelNorm(double U, double V, ChVector<>& Result) {
    Matrix3x3N Sxi_zeta;
    Vector3N edot;
    ChVector<> r_zeta;
...
void ChElementShellANCF_3443_TR08b::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3443_TR08b::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3443_TR08b::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementShellANCF_3443_TR08b::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3443_TR08b::ComputeNF(
...
double ChElementShellANCF_3443_TR08b::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementShellANCF_3443_TR08b::ComputeNormal(const double U, const double V) {
    Matrix3x3N Sxi_zeta;
    Vector3N e;
    ChVector<> r_zeta;
...
ChQuadratureTables static_tables_3443_TR08b(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3443_TR08b::GetStaticGQTables() {
    return &static_tables_3443_TR08b;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialShellANCF_3443_TR08b methods
// ============================================================================

// Construct an isotropic material.
ChMaterialShellANCF_3443_TR08b::ChMaterialShellANCF_3443_TR08b(
...
// Poisson effect
void ChMaterialShellANCF_3443_TR08b::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G) {
