...
ChElementHexaANCF_3843_MR_V4::ChElementHexaANCF_3843_MR_V4()
...
void ChElementHexaANCF_3843_MR_V4::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementHexaANCF_3843_MR_V4::SetDimensions(double lenX, double lenY, double lenZ) {
    m_lenX = lenX;
    m_lenY = lenY;
    m_lenZ = lenZ;
...
void ChElementHexaANCF_3843_MR_V4::SetMaterial(std::shared_ptr<ChMaterialHexaANCF_MR> brick_mat) {
...
ChMatrix33<> ChElementHexaANCF_3843_MR_V4::GetGreenLagrangeStrain(const double xi, const double eta, const double zeta) {
...
ChMatrix33<> ChElementHexaANCF_3843_MR_V4::GetPK2Stress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementHexaANCF_3843_MR_V4::GetVonMissesStress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
void ChElementHexaANCF_3843_MR_V4::SetupInitial(ChSystem* system) {
...
void ChElementHexaANCF_3843_MR_V4::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementHexaANCF_3843_MR_V4::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementHexaANCF_3843_MR_V4::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementHexaANCF_3843_MR_V4::ComputeNodalMass() {
...
void ChElementHexaANCF_3843_MR_V4::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementHexaANCF_3843_MR_V4::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementHexaANCF_3843_MR_V4::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementHexaANCF_3843_MR_V4::EvaluateElementFrame(const double xi,
                                                  const double eta,
                                                  const double zeta,
                                                  ChVector<>& point,
                                                  ChQuaternion<>& rot) {
...
void ChElementHexaANCF_3843_MR_V4::EvaluateElementPoint(const double xi,
                                                  const double eta,
                                                  const double zeta,
                                                  ChVector<>& point) {
    VectorN Sxi_compact;
...
void ChElementHexaANCF_3843_MR_V4::EvaluateElementVel(double xi, double eta, const double zeta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta);
...
void ChElementHexaANCF_3843_MR_V4::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementHexaANCF_3843_MR_V4::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementHexaANCF_3843_MR_V4::LoadableStateIncrement(const unsigned int off_x,
                                                    ChState& x_new,
                                                    const ChState& x,
...
void ChElementHexaANCF_3843_MR_V4::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementHexaANCF_3843_MR_V4::ComputeNF(
...
double ChElementHexaANCF_3843_MR_V4::GetDensity() {
...
void ChElementHexaANCF_3843_MR_V4::ComputeMassMatrixAndGravityForce() {
...
void ChElementHexaANCF_3843_MR_V4::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementHexaANCF_3843_MR_V4::ComputeInternalForceDamping(ChVectorDynamic<>& Fi) {
    ...
    for (unsigned int GQpnt = 0; GQpnt < NIP; GQpnt++) {
        ...
        //Compact the multiplication below into S*[3x3] matrix.  Also move the switch for with and without damping to above this function call (i.e. one function with damping and one without)
        ...
        ChMatrixNMc<double, 3, 3> CombinedF = (SPK2_NLKV - scale2 * C) * F.transpose() + scale3 * F.transpose();
        CombinedF(0, 0) += scale4 * (F(1, 1)*F(2, 2) - F(1, 2)*F(2, 1));
        CombinedF(1, 0) += scale4 * (F(1, 2)*F(2, 0) - F(1, 0)*F(2, 2));
        CombinedF(2, 0) += scale4 * (F(1, 0)*F(2, 1) - F(1, 1)*F(2, 0));

        CombinedF(0, 1) += scale4 * (F(0, 2)*F(2, 1) - F(0, 1)*F(2, 2));
        CombinedF(1, 1) += scale4 * (F(0, 0)*F(2, 2) - F(0, 2)*F(2, 0));
        CombinedF(2, 1) += scale4 * (F(0, 1)*F(2, 0) - F(0, 0)*F(2, 1));

        CombinedF(0, 2) += scale4 * (F(0, 1)*F(1, 2) - F(0, 2)*F(1, 1));
        CombinedF(1, 2) += scale4 * (F(0, 2)*F(1, 0) - F(0, 0)*F(1, 2));
        CombinedF(2, 2) += scale4 * (F(0, 0)*F(1, 1) - F(0, 1)*F(1, 0));

        QiCompact.noalias() += Sbar_xi_D * CombinedF;
        //QiCompact.noalias() += Sbar_xi_D.col(0)*((scale3 + SPK2_NLKV_0)*F.col(0).transpose() + SPK2_NLKV_5 * F.col(1).transpose() + SPK2_NLKV_4 * F.col(2).transpose() - scale2 * C.row(0)*F.transpose() + scale4 * detF_S0)
        //    + Sbar_xi_D.col(1)*(SPK2_NLKV_5*F.col(0).transpose() + (scale3 + SPK2_NLKV_1) * F.col(1).transpose() + SPK2_NLKV_3 * F.col(2).transpose() - scale2 * C.row(1)*F.transpose() + scale4 * detF_S1)
        //    + Sbar_xi_D.col(2)*(SPK2_NLKV_4*F.col(0).transpose() + SPK2_NLKV_3 * F.col(1).transpose() + (scale3 + SPK2_NLKV_2) * F.col(2).transpose() - scale2 * C.row(2)*F.transpose() + scale4 * detF_S2);
    }
...
void ChElementHexaANCF_3843_MR_V4::ComputeInternalForceNoDamping(ChVectorDynamic<>& Fi) {
    assert(Fi.size() == 3 * NSF);
    ...
    for (unsigned int GQpnt = 0; GQpnt < NIP; GQpnt++) {
        ...
        ChMatrixNMc<double, 3, 3> CombinedF = scale3*F.transpose() - scale2 * C *F.transpose();
        CombinedF(0, 0) += scale4 * (F(1, 1)*F(2, 2) - F(1, 2)*F(2, 1));
        CombinedF(1, 0) += scale4 * (F(1, 2)*F(2, 0) - F(1, 0)*F(2, 2));
        CombinedF(2, 0) += scale4 * (F(1, 0)*F(2, 1) - F(1, 1)*F(2, 0));
        ...
        //ChMatrixNMc<double, 3, 3> CombinedF;
        //CombinedF << scale4 * (F(1, 1)*F(2, 2) - F(1, 2)*F(2, 1)), scale4 * (F(1, 2)*F(2, 0) - F(1, 0)*F(2, 2)), scale4 * (F(1, 0)*F(2, 1) - F(1, 1)*F(2, 0)),
        //    scale4 * (F(0, 2)*F(2, 1) - F(0, 1)*F(2, 2)), scale4 * (F(0, 0)*F(2, 2) - F(0, 2)*F(2, 0)), scale4 * (F(0, 1)*F(2, 0) - F(0, 0)*F(2, 1)),
        //    scale4 * (F(0, 1)*F(1, 2) - F(0, 2)*F(1, 1)), scale4 * (F(0, 2)*F(1, 0) - F(0, 0)*F(1, 2)), scale4 * (F(0, 0)*F(1, 1) - F(0, 1)*F(1, 0));
        //CombinedF.noalias() += scale3 * F.transpose() - scale2 * C *F.transpose();


        //Compact the multiplication below into S*[3x3] matrix.  Also move the switch for with and without damping to above this function call (i.e. one function with damping and one without)

        //--------------------------------------------------------------------------------
        // 2 term Mooney-Rivlin Material Only
        //--------------------------------------------------------------------------------
        //QiCompact.noalias() += Sbar_xi_D.col(0)*(scale3 * F.col(0).transpose() - scale2 * C.row(0)*F.transpose() + scale4 * detF_S0)
        //    + Sbar_xi_D.col(1)*(scale3 * F.col(1).transpose() - scale2 * C.row(1)*F.transpose() + scale4 * detF_S1)
        //    + Sbar_xi_D.col(2)*(scale3 * F.col(2).transpose() - scale2 * C.row(2)*F.transpose() + scale4 * detF_S2);
        QiCompact.noalias() += Sbar_xi_D * CombinedF;
    }
...
void ChElementHexaANCF_3843_MR_V4::ComputeInternalJacobianDamping(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementHexaANCF_3843_MR_V4::ComputeInternalJacobianNoDamping(ChMatrixRef H, double Kfactor, double Mfactor) {
    assert((H.rows() == 3 * NSF) && (H.cols() == 3 * NSF));
...
void ChElementHexaANCF_3843_MR_V4::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
    Sxi_compact(0) =
...
void ChElementHexaANCF_3843_MR_V4::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
void ChElementHexaANCF_3843_MR_V4::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
void ChElementHexaANCF_3843_MR_V4::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
void ChElementHexaANCF_3843_MR_V4::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
    VectorN Sxi_D_col;
...
void ChElementHexaANCF_3843_MR_V4::CalcCoordVector(Vector3N& e) {
...
void ChElementHexaANCF_3843_MR_V4::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementHexaANCF_3843_MR_V4::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementHexaANCF_3843_MR_V4::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementHexaANCF_3843_MR_V4::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementHexaANCF_3843_MR_V4::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementHexaANCF_3843_MR_V4::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
ChQuadratureTables static_tables_3843_MR_V4(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementHexaANCF_3843_MR_V4::GetStaticGQTables() {
    return &static_tables_3843_MR_V4;
}
