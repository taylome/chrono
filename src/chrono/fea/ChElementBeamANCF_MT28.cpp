...
// =============================================================================
//
// MT28 = MT27 with Dynamic Eigen Matrices
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT28::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT28::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT28::ChElementBeamANCF_MT28() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);

    m_MassMatrix.resize(9,9);
    m_SD_precompute.resize(9,60);
    
    m_F_Block.resize(3,60);
    m_Fdot_Block.resize(3, 60);
    m_SPK2.resize(96, 1);
    
    m_SD_precompute_D0.resize(9, 48);
    m_SD_precompute_Dv.resize(9, 12);
}
...
void ChElementBeamANCF_MT28::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT28::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT28::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT28::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT28::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
    assert((H.rows() == 27) && (H.cols() == 27));

#if true
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 9, 3> Sxi_D_0xi;
    Sxi_D_0xi.resize(9, 3);
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 9, 9> Jacobian_CompactPart = Mfactor * m_MassMatrix;
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 3, 27> partial_epsilon_partial_e_Dv;//partial derivate of the Green-Lagrange strain tensor in Voigt notation with respect to the nodal coordinates
    partial_epsilon_partial_e_Dv.resize(3, 27);
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 9, 3> partial_e_temp0;//temporary matrix for calculating the partial derivatives of the strains with respect to the nodal coordinates
    partial_e_temp0.resize(9, 3);
    Eigen::Map<ChRowVectorN<double, 27>> partial_e_tempReshaped0(partial_e_temp0.data(), 27);
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 9, 3> partial_e_temp1;//temporary matrix for calculating the partial derivatives of the strains with respect to the nodal coordinates
    partial_e_temp1.resize(9, 3);
    Eigen::Map<ChRowVectorN<double, 27>> partial_e_tempReshaped1(partial_e_temp1.data(), 27);
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 9, 3> partial_e_temp2;//temporary matrix for calculating the partial derivatives of the strains with respect to the nodal coordinates
    partial_e_temp2.resize(9, 3);
    Eigen::Map<ChRowVectorN<double, 27>> partial_e_tempReshaped2(partial_e_temp2.data(), 27);
    ...
    if (m_damping_enabled) {        //If linear Kelvin-Voigt viscoelastic material model is enabled 
        ...
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 3, 27> Scaled_Combined_partial_epsilon_partial_e_Dv;
        Scaled_Combined_partial_epsilon_partial_e_Dv.resize(3, 27);
        for (unsigned int index = 16; index < 20; index++) {
...
void ChElementBeamANCF_MT28::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT28::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT28::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_MT28::ComputeNodalMass() {
...
void ChElementBeamANCF_MT28::SetAlphaDamp(double a) {
    m_Alpha = a;
...
void ChElementBeamANCF_MT28::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBeamANCF_MT28::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const ChMatrixNMc<double, 9, 3>& e_bar, const ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 60, 3> P_transpose_scaled_Block; //1st tensor Piola-Kirchoff stress tensor (non-symmetric tensor) - Tiled across all Gauss-Quadrature points in a big matrix
    P_transpose_scaled_Block.resize(60, 3);
...
void ChElementBeamANCF_MT28::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi, const ChMatrixNMc<double, 9, 3>& e_bar) {
...
void ChElementBeamANCF_MT28::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
...
void ChElementBeamANCF_MT28::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
...
void ChElementBeamANCF_MT28::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
...
void ChElementBeamANCF_MT28::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT28::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT28::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, double GQ_weight, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot){
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNMc<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChVectorN<double, 6> epsilon;           //Green-Lagrange strain tensor in Voigt notation
...
void ChElementBeamANCF_MT28::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT28::ComputeInternalJacobiansAnalytic(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> e_bar_dot;
    ...
    //Calculate the portion of the Selective Reduced Integration that account for the Poisson effect, but only on the beam axis
    if (GetStrainFormulation() == ChElementBeamANCF_MT28::StrainFormulation::CMPoisson) {
...
void ChElementBeamANCF_MT28::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT28::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT28::Calc_Sxi_D(ChMatrixNMc<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT28::CalcCoordMatrix(ChMatrixNMc<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT28::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT28::CalcCoordDerivMatrix(ChMatrixNMc<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT28::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT28::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT28::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNMc<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT28::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT28::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_MT28::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
...
// void ChElementBeamANCF_MT28::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT28::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT28::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT28::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT28::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT28::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT28::ComputeNF(
...
double ChElementBeamANCF_MT28::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT28::ComputeTangent(const double U) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT28(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT28::GetStaticGQTables() {
    return &static_tables_MT28;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT28 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT28::ChMaterialBeamANCF_MT28(double rho,        // material density
...
void ChMaterialBeamANCF_MT28::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
