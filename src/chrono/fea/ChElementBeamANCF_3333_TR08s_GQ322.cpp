...
ChElementBeamANCF_3333_TR08S_GQ322::ChElementBeamANCF_3333_TR08S_GQ322()
...
void ChElementBeamANCF_3333_TR08S_GQ322::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_3333_TR08S_GQ322::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_3333_TR08S_GQ322::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_3333_TR08S_GQ322::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
    ...
    ChMatrixNM<double, 9, 9> MassMatrixCompactSquare;
...
void ChElementBeamANCF_3333_TR08S_GQ322::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    unsigned int GQ_idx_xi = 2;        // 3 Point Gauss-Quadrature;
    unsigned int GQ_idx_eta_zeta = 1;  // 2 Point Gauss-Quadrature;

    ChMatrixNMc<double, 9, 36> SD_precompute_D0;
    ChMatrixNMc<double, 9, 9> SD_precompute_Dv;
    ...
    for (auto i = 0; i < 12; i++) {
        m_SD_precompute_col_ordered.col(i) = SD_precompute_D0.col(3 * i);
        m_SD_precompute_col_ordered.col(i + 12) = SD_precompute_D0.col(3 * i + 1);
        m_SD_precompute_col_ordered.col(i + 24) = SD_precompute_D0.col(3 * i + 2);
    }
    for (auto i = 0; i < 3; i++) {
        m_SD_precompute_col_ordered.col(i + 36) = SD_precompute_Dv.col(3 * i);
        m_SD_precompute_col_ordered.col(i + 39) = SD_precompute_Dv.col(3 * i + 1);
        m_SD_precompute_col_ordered.col(i + 42) = SD_precompute_Dv.col(3 * i + 2);
    }
}

/// This class computes and adds corresponding masses to ElementGeneric member m_TotalMass
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeNodalMass() {
...
void ChElementBeamANCF_3333_TR08S_GQ322::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
    ...
    ChMatrixNMc<double, 45, 6> F_Transpose_CombinedBlockDamping_col_ordered =
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeInternalJacobianDamping(ChMatrixRef& H,
                                                            double Kfactor,
                                                            double Rfactor,
                                                            double Mfactor) {
    ChMatrixNM<double, 9, 6> ebar_ebardot;
    ...
    ChMatrixNM<double, 27, 81> partial_epsilon_partial_e_Transpose;
    //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 27, 108> partial_epsilon_partial_e_Transpose;
    ...
    for (auto i = 0; i < 3; i++) {
        ...
        scaled_F_Transpose_col_ordered.block<12, 1>(12, i).array() *= m_GQWeight_det_J_0xi_D0.array();
        scaled_F_Transpose_col_ordered.block<12, 1>(24, i).array() *= m_GQWeight_det_J_0xi_D0.array();
        scaled_F_Transpose_col_ordered.block<3, 1>(36, i).array() *= m_GQWeight_det_J_0xi_Dv.array();
        scaled_F_Transpose_col_ordered.block<3, 1>(39, i).array() *= m_GQWeight_det_J_0xi_Dv.array();
        scaled_F_Transpose_col_ordered.block<3, 1>(42, i).array() *= m_GQWeight_det_J_0xi_Dv.array();
    }

    ChMatrixNM<double, 27, 81> Scaled_Combined_partial_epsilon_partial_e_Transpose;
    ...
    ChMatrixNM<double, 27, 3> ChunkSDvA;
    ChMatrixNM<double, 27, 3> ChunkSDvB;
    ChMatrixNM<double, 27, 3> ChunkSDvC;

    for (auto i = 0; i < 9; i++) {
        ChunkSDvA.block<1, 3>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 36).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(36, 0).transpose());
        ChunkSDvB.block<1, 3>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 39).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(39, 0).transpose());
        ChunkSDvC.block<1, 3>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 42).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(42, 0).transpose());

        ChunkSDvA.block<1, 3>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 36).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(36, 1).transpose());
        ChunkSDvB.block<1, 3>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 39).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(39, 1).transpose());
        ChunkSDvC.block<1, 3>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 42).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(42, 1).transpose());

        ChunkSDvA.block<1, 3>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 36).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(36, 2).transpose());
        ChunkSDvB.block<1, 3>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 39).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(39, 2).transpose());
        ChunkSDvC.block<1, 3>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 3>(i, 42).cwiseProduct(
            scaled_F_Transpose_col_ordered.block<3, 1>(42, 2).transpose());
    }

    const ChMatrix33<double>& Dv = GetMaterial()->Get_Dv();

    Scaled_Combined_partial_epsilon_partial_e_Transpose.block<27, 3>(0, 72).noalias() =
        Dv(0, 0) * ChunkSDvA + Dv(0, 1) * ChunkSDvB + Dv(0, 2) * ChunkSDvC;
    Scaled_Combined_partial_epsilon_partial_e_Transpose.block<27, 3>(0, 75).noalias() =
        Dv(1, 0) * ChunkSDvA + Dv(1, 1) * ChunkSDvB + Dv(1, 2) * ChunkSDvC;
    Scaled_Combined_partial_epsilon_partial_e_Transpose.block<27, 3>(0, 78).noalias() =
    ...
    ChMatrixNM<double, 9, 45> S_scaled_SD_precompute_col_ordered;
    ...
    ChVectorN<double, 45> ScaledMassMatrix = Mfactor * m_MassMatrix;
    ...
    for (unsigned int i = 0; i < 9; i++) {
        for (unsigned int j = i; j < 9; j++) {
            ...
            //double d = Kfactor * m_SD_precompute_col_ordered.row(i).dot(S_scaled_SD_precompute_col_ordered.row(j));
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ChMatrixNM<double, 9, 3> e_bar;
    CalcCoordMatrix(e_bar);
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix,
                                                      double Kfactor,
                                                      double Rfactor) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_3333_TR08S_GQ322::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::Calc_Sxi_D(ChMatrixNMc<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_3333_TR08S_GQ322::CalcCoordMatrix(ChMatrixNMc<double, 9, 3>& e) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::CalcCoordMatrix(ChMatrixNM<double, 9, 3>& e) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::CalcCoordDerivMatrix(ChMatrixNMc<double, 9, 3>& edot) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::CalcCombinedCoordMatrix(ChMatrixNM<double, 9, 6>& ebar_ebardot) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_3333_TR08S_GQ322::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNMc<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
// ChVector<> ChElementBeamANCF_3333_TR08S_GQ322::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_3333_TR08S_GQ322::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::EvaluateSectionFrame(const double xi, ChVector<>& point, ChQuaternion<>& rot) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChVectorN<double, 9> Sxi_compact;
    ChMatrixNMc<double, 9, 3> Sxi_D;
...
// void ChElementBeamANCF_3333_TR08S_GQ322::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_3333_TR08S_GQ322::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::LoadableStateIncrement(const unsigned int off_x,
                                                    ChState& x_new,
                                                    const ChState& x,
...
// void ChElementBeamANCF_3333_TR08S_GQ322::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_3333_TR08S_GQ322::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_3333_TR08S_GQ322::ComputeNF(
...
double ChElementBeamANCF_3333_TR08S_GQ322::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_3333_TR08S_GQ322::ComputeTangent(const double U) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_TR08S_GQ322(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_3333_TR08S_GQ322::GetStaticGQTables() {
    return &static_tables_TR08S_GQ322;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_3333_TR08S_GQ322 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_3333_TR08S_GQ322::ChMaterialBeamANCF_3333_TR08S_GQ322(double rho,        // material density
...
// Poisson effect
void ChMaterialBeamANCF_3333_TR08S_GQ322::Calc_D0_Dv(const ChVector<>& E,
                                         const ChVector<>& nu,
                                         const ChVector<>& G,
                                         double k1,
                                         double k2) {
