...
ChElementShellANCF_3443_TR08S::ChElementShellANCF_3443_TR08S()
...
void ChElementShellANCF_3443_TR08S::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementShellANCF_3443_TR08S::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementShellANCF_3443_TR08S::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementShellANCF_3443_TR08S::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3443_TR08S::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                           double Kfactor,
                                                           double Rfactor,
                                                           double Mfactor) {
...
void ChElementShellANCF_3443_TR08S::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementShellANCF_3443_TR08S::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementShellANCF_3443_TR08S::ComputeNodalMass() {
...
void ChElementShellANCF_3443_TR08S::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi_eta].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_xi_eta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_zeta].size(); it_zeta++) {
                ...
                m_SD_precompute_col_ordered.col(index + 72) = SD_precompute_D.col(1);
                m_SD_precompute_col_ordered.col(index + 144) = SD_precompute_D.col(2);
...
void ChElementShellANCF_3443_TR08S::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementShellANCF_3443_TR08S::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3443_TR08S::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const MatrixNx6& ebar_ebardot) {
    ...
    //F_Transpose_CombinedBlockDamping_col_ordered.resize(216, 6);
    ...
    ChMatrixNMc<double, 216, 6>  F_Transpose_CombinedBlockDamping_col_ordered = m_SD_precompute_col_ordered.transpose() * ebar_ebardot;
...
void ChElementShellANCF_3443_TR08S::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    //F_Transpose_CombinedBlock_col_ordered.resize(216, 3);
    //// Calculate F is one big block and then split up afterwards to improve efficiency (hopefully)
    //F_Transpose_CombinedBlock_col_ordered.noalias() = m_SD_precompute_col_ordered.transpose() * e_bar;
    ChMatrixNMc<double, 216, 3>  F_Transpose_CombinedBlock_col_ordered = m_SD_precompute_col_ordered.transpose() * e_bar;
...
void ChElementShellANCF_3443_TR08S::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
    double Kfactor,
    double Rfactor) {
...
void ChElementShellANCF_3443_TR08S::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
    ...
    for (auto i = 0; i < 16; i++) {
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 0) =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).transpose()) +
                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose()) +
                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose()) +
                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 0) =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 0) =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).transpose());
    }
    ...
    for (auto i = 0; i < 16; i++) {
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 0) =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 0).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 0).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 0).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 0).transpose()) +
                                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 0).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 0).transpose()) +
                                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 0).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 0).transpose()) +
                                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 0).transpose());

        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 1).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 1).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 1).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 1).transpose()) +
                                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 1).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 1).transpose()) +
                                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 1).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 1).transpose()) +
                                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 1).transpose());

        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 0) =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 2).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 2).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 2).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 2).transpose()) +
                                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 2).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 2).transpose()) +
                                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 2).transpose());
        Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 2).transpose()) +
    ...
    for (auto i = 0; i < 16; i++) {
        S_scaled_SD_precompute_col_ordered.block<1, 72>(i, 0) =
            SPK2_0_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 0)) +
            SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 72)) +
            SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 144));

        S_scaled_SD_precompute_col_ordered.block<1, 72>(i, 72) =
            SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 0)) +
            SPK2_1_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 72)) +
            SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 144));

        S_scaled_SD_precompute_col_ordered.block<1, 72>(i, 144) =
            SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 0)) +
            SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 72)) +
            SPK2_2_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 144));
    }
    ...
    for (unsigned int i = 0; i < 16; i++) {
        for (unsigned int j = i; j < 16; j++) {
            double d = Kfactor * m_SD_precompute_col_ordered.block<1, 216>(i, 0) * S_scaled_SD_precompute_col_ordered.block<1, 216>(j, 0).transpose();
...
void ChElementShellANCF_3443_TR08S::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ...
    for (auto i = 0; i < 16; i++) {
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 0) =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose()) +
                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose()) +
                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose()) +
                                                                                m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 0) =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose());

        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 0) =  m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).transpose());
        partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose()) +
                                                                                      m_SD_precompute_col_ordered.block<1, 72>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).transpose());
    }
...
void ChElementShellANCF_3443_TR08S::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443_TR08S::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {    
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443_TR08S::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443_TR08S::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443_TR08S::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443_TR08S::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443_TR08S::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443_TR08S::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
void ChElementShellANCF_3443_TR08S::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
...
void ChElementShellANCF_3443_TR08S::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3443_TR08S::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3443_TR08S::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementShellANCF_3443_TR08S::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
    ebar.row(3) = m_nodes[0]->GetDDD().eigen();
...
void ChElementShellANCF_3443_TR08S::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3443_TR08S::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3443_TR08S::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementShellANCF_3443_TR08S::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementShellANCF_3443_TR08S::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementShellANCF_3443_TR08S::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
void ChElementShellANCF_3443_TR08S::EvaluateSectionFrame(const double xi, const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    Matrix3x3N Sxi;
    Matrix3x3N Sxi_xi;
    Matrix3x3N Sxi_eta;
...
void ChElementShellANCF_3443_TR08S::EvaluateSectionPoint(const double u, const double v, ChVector<>& point) {
    Matrix3x3N Sxi;
    Calc_Sxi(Sxi, u, v, 0);
...
void ChElementShellANCF_3443_TR08S::EvaluateSectionVelNorm(double U, double V, ChVector<>& Result) {
    Matrix3x3N Sxi_zeta;
    Vector3N edot;
    ChVector<> r_zeta;
...
void ChElementShellANCF_3443_TR08S::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3443_TR08S::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3443_TR08S::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementShellANCF_3443_TR08S::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3443_TR08S::ComputeNF(
...
double ChElementShellANCF_3443_TR08S::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementShellANCF_3443_TR08S::ComputeNormal(const double U, const double V) {
    Matrix3x3N Sxi_zeta;
    Vector3N e;
    ChVector<> r_zeta;
...
ChQuadratureTables static_tables_3443_TR08S(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3443_TR08S::GetStaticGQTables() {
    return &static_tables_3443_TR08S;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialShellANCF_3443_TR08S methods
// ============================================================================

// Construct an isotropic material.
ChMaterialShellANCF_3443_TR08S::ChMaterialShellANCF_3443_TR08S(
...
// Poisson effect
void ChMaterialShellANCF_3443_TR08S::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G) {
