...
// =============================================================================
//
// MT02 = MT01 + Reduced number of GQ points for the internal force calculation:
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT02::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT02::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT02::ChElementBeamANCF_MT02() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT02::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT02::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT02::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT02::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT02::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementBeamANCF_MT02::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT02::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT02::ComputeNodalMass() {
...
void ChElementBeamANCF_MT02::SetAlphaDamp(double a) {
    m_Alpha = a;
    if (std::abs(m_Alpha) > 1e-10)
        m_damping_enabled = true;
}

void ChElementBeamANCF_MT02::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ChVectorN<double, 27> e;
    ChVectorN<double, 27> e_dot;

    CalcCoordVector(e);
    CalcCoordDerivVector(e_dot);
    ComputeInternalForcesAtState(Fi, e, e_dot);
...
void ChElementBeamANCF_MT02::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, ChVectorN<double, 27>& e, ChVectorN<double, 27>& e_dot) {
    ...
    if (GetStrainFormulation() == ChElementBeamANCF_MT02::StrainFormulation::CMPoisson) {
        for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
...
void ChElementBeamANCF_MT02::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, double xi, double eta, double zeta, ChMatrixNM<double,6,6>& D, ChVectorN<double, 27>& e, ChVectorN<double, 27>& e_dot) {
    ...
    if (m_damping_enabled) {
        ...
        //Linear Hookean material model
        Qi = -depsilon_de_transpose()*D*(epsilon)*det_J_0xi;
    }
#endif
...
void ChElementBeamANCF_MT02::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
    ...
    ComputeInternalForcesAtState(FiOrignal, e, e_dot);
    for (unsigned int i = 0; i < 27; i++) {
        e(i) = e(i) + delta;
        ComputeInternalForcesAtState(FiDelta, e, e_dot);
...
void ChElementBeamANCF_MT02::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT02::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBeamANCF_MT02::Calc_Sxi_xi(ChMatrixNM<double, 3, 27>& Sxi_xi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT02::Calc_Sxi_xi_compact(ChVectorN<double, 9>& Sxi_xi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBeamANCF_MT02::Calc_Sxi_eta(ChMatrixNM<double, 3, 27>& Sxi_eta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT02::Calc_Sxi_eta_compact(ChVectorN<double, 9>& Sxi_eta_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBeamANCF_MT02::Calc_Sxi_zeta(ChMatrixNM<double, 3, 27>& Sxi_zeta, double xi, double eta, double zeta) {
...
// 9x1 Vector Form of the partial derivatives of Normalized Shape Functions with respect to eta
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT02::Calc_Sxi_zeta_compact(ChVectorN<double, 9>& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT02::CalcCoordMatrix(ChMatrixNM<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT02::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT02::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT02::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT02::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT02::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT02::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
//}
//
// void ChElementBeamANCF_MT02::EvaluateSectionFrame(const double u,
//                                             const double v,
//                                             ChVector<>& point,
//                                             ChQuaternion<>& rot) {
//    // this is not a corotational element, so just do:
...
//}
//
// void ChElementBeamANCF_MT02::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT02::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT02::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT02::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT02::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT02::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT02::ComputeNF(
...
double ChElementBeamANCF_MT02::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT02::ComputeTangent(const double U) {
    ChMatrixNM<double, 3, 27> Sxi_xi;
    ChVectorN<double, 27> e;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT02(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT02::GetStaticGQTables() {
    return &static_tables_MT02;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT02 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT02::ChMaterialBeamANCF_MT02(double rho,        // material density
...
void ChMaterialBeamANCF_MT02::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
