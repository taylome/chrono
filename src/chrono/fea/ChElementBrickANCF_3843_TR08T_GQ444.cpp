...
ChElementBrickANCF_3843_TR08T_GQ444::ChElementBrickANCF_3843_TR08T_GQ444()
    : m_gravity_on(false), m_lenX(0), m_lenY(0), m_lenZ(0), m_Alpha(0), m_damping_enabled(false) {
    m_nodes.resize(8);

    //m_GQWeight_det_J_0xi_D.resize(64,1);
    //m_SD_precompute_col_ordered.resize(32, 192);
...
void ChElementBrickANCF_3843_TR08T_GQ444::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementBrickANCF_3843_TR08T_GQ444::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementBrickANCF_3843_TR08T_GQ444::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBrickANCF_3843_TR08T_GQ444::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeKRMmatricesGlobal(ChMatrixRef H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeNodalMass() {
...
void ChElementBrickANCF_3843_TR08T_GQ444::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    unsigned int GQ_idx_xi_eta_zeta = 3;    // 4 Point Gauss-Quadrature;

    for (unsigned int it = 0; it < 16; it++) {
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> SD;
        SD.resize(32, 12); //4 GQ Point Tile Size
        m_SD_precompute_col_ordered.push_back(SD);

        ChVectorN<double, 4> GQWeight_det_J_0xi_D;
        m_GQWeight_det_J_0xi_D.push_back(GQWeight_det_J_0xi_D);
    }
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi_eta_zeta].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_xi_eta_zeta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_xi_eta_zeta].size(); it_zeta++) {
                ...
                double eta = GQTable->Lroots[GQ_idx_xi_eta_zeta][it_eta];
                double zeta = GQTable->Lroots[GQ_idx_xi_eta_zeta][it_zeta];
                auto index = it_eta + it_xi * GQTable->Lroots[GQ_idx_xi_eta_zeta].size();
                ChMatrix33<double>
                ...
                m_SD_precompute_col_ordered[index].col(it_zeta) = SD_precompute_D.col(0);
                m_SD_precompute_col_ordered[index].col(it_zeta + 4) = SD_precompute_D.col(1);
                m_SD_precompute_col_ordered[index].col(it_zeta + 8) = SD_precompute_D.col(2);

                m_GQWeight_det_J_0xi_D[index](it_zeta) = -J_0xi.determinant() * GQ_weight;
            }
        }
    }
}
...
void ChElementBrickANCF_3843_TR08T_GQ444::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
    ...
    for(unsigned int tile = 0; tile < 16; tile++){
        ...
        ChMatrixNMc<double, 12, 6> F_Transpose_CombinedBlockDamping_col_ordered = m_SD_precompute_col_ordered[tile].transpose() * ebar_ebardot;

        VectorNIP E0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0));
        E0_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1));
        E0_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2));
        E0_Block.array() -= 1;
        E0_Block *= 0.5;
        VectorNIP E_BlockDamping = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 5));
        E0_Block += m_Alpha * E_BlockDamping;
        E0_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();

        VectorNIP E1_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0));
        E1_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1));
        E1_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2));
        E1_Block.array() -= 1;
        E1_Block *= 0.5;
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 5));
        E1_Block += m_Alpha * E_BlockDamping;
        E1_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();

        VectorNIP E2_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0));
        E2_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1));
        E2_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2));
        E2_Block.array() -= 1;
        E2_Block *= 0.5;
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 5));
        E2_Block += m_Alpha * E_BlockDamping;
        E2_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();

        VectorNIP E3_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0));
        E3_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1));
        E3_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2));
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).cwiseProduct(
        ...
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 5));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).cwiseProduct(
        ...
        E3_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();

        VectorNIP E4_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0));
        E4_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1));
        E4_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2));
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).cwiseProduct(
        ...
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 5));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).cwiseProduct(
        ...
        E4_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();

        VectorNIP E5_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0));
        E5_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1));
        E5_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2));
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).cwiseProduct(
        ...
        E5_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();
        ...
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 0) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 0) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 1) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 1) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 2) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 2) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).cwiseProduct(SPK2_4_Block);

        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 0) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 0) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 1) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 1) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 2) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 2) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).cwiseProduct(SPK2_3_Block);

        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 0) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 0) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 1) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 1) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 2) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 2) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).cwiseProduct(SPK2_2_Block);

        // =============================================================================

        if (tile == 0) {
            QiCompact = m_SD_precompute_col_ordered[tile] * P_transpose_scaled_Block_col_ordered;
        }
        else {
            QiCompact += m_SD_precompute_col_ordered[tile] * P_transpose_scaled_Block_col_ordered;
        }
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    for (unsigned int tile = 0; tile < 16; tile++) {
        ...
        ChMatrixNMc<double, 12, 3> F_Transpose_CombinedBlock_col_ordered = m_SD_precompute_col_ordered[tile].transpose() * e_bar;

        VectorNIP E0_Block = F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0));
        E0_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1));
        E0_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2));
        E0_Block.array() -= 1;
        E0_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();
        E0_Block *= 0.5;

        VectorNIP E1_Block = F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0));
        E1_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1));
        E1_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2));
        E1_Block.array() -= 1;
        E1_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();
        E1_Block *= 0.5;

        VectorNIP E2_Block = F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0));
        E2_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1));
        E2_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2));
        E2_Block.array() -= 1;
        E2_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();
        E2_Block *= 0.5;

        VectorNIP E3_Block = F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0));
        E3_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1));
        E3_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2));
        E3_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();

        VectorNIP E4_Block = F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0));
        E4_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1));
        E4_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2));
        E4_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();

        VectorNIP E5_Block = F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0));
        E5_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1));
        E5_Block += F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2));
        E5_Block.array() *= m_GQWeight_det_J_0xi_D[tile].array();
        ...
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 0) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 0) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 1) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 1) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 2) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(0, 2) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2).cwiseProduct(SPK2_4_Block);

        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 0) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 0) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 1) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 1) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 2) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(4, 2) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2).cwiseProduct(SPK2_3_Block);

        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 0) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 0) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 1) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 1) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 2) =
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<4, 1>(8, 2) +=
            F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2).cwiseProduct(SPK2_2_Block);
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
    double Kfactor,
    double Rfactor) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
    ...
    for (unsigned int tile = 0; tile < 16; tile++) {
        ...
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 48, 128> partial_epsilon_partial_e_Transpose;
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> partial_epsilon_partial_e_Transpose;
        //partial_epsilon_partial_e_Transpose.resize(96, 24);
        ChMatrixNM<double, 96, 24> partial_epsilon_partial_e_Transpose;

        for (auto i = 0; i < 32; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(8, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<4, 1>(4, 2).transpose());
        }

        //ChMatrixNMc<double, 12, 3> scaled_F_Transpose_col_ordered =
        //    (Kfactor + m_Alpha * Rfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<12, 3>(0, 0) +
        //    (m_Alpha * Kfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<12, 3>(0, 3);
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> scaled_F_Transpose_col_ordered;
        //scaled_F_Transpose_col_ordered.resize(12, 3);
        //scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<12, 3>(0, 0) +
        //    (m_Alpha * Kfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<12, 3>(0, 3);
        ChMatrixNMc<double, 12, 3> scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<12, 3>(0, 0) +
            (m_Alpha * Kfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<12, 3>(0, 3);

        for (auto i = 0; i < 3; i++) {
            scaled_F_Transpose_col_ordered.block<4, 1>(0, i).array() *= m_GQWeight_det_J_0xi_D[tile].array();
            scaled_F_Transpose_col_ordered.block<4, 1>(4, i).array() *= m_GQWeight_det_J_0xi_D[tile].array();
            scaled_F_Transpose_col_ordered.block<4, 1>(8, i).array() *= m_GQWeight_det_J_0xi_D[tile].array();
        }

        // ChMatrixNM<double, 48, 8> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 48, 8> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        Scaled_Combined_partial_epsilon_partial_e_Transpose.resize(96, 24);


        for (auto i = 0; i < 32; i++) {
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 0).transpose());

            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 1).transpose());

            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(8, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<4, 1>(4, 2).transpose());
        }

        // ChMatrixNM<double, 48, 8> DScaled_Combined_partial_epsilon_partial_e_Transpose;
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 48, 8> DScaled_Combined_partial_epsilon_partial_e_Transpose;
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> DScaled_Combined_partial_epsilon_partial_e_Transpose;
        DScaled_Combined_partial_epsilon_partial_e_Transpose.resize(96, 24);

        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 0) = D(0, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 0) + D(0, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 4) + D(0, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 8) + D(0, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 12) + D(0, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 16) + D(0, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 20);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 4) = D(1, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 0) + D(1, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 4) + D(1, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 8) + D(1, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 12) + D(1, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 16) + D(1, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 20);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 8) = D(2, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 0) + D(2, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 4) + D(2, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 8) + D(2, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 12) + D(2, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 16) + D(2, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 20);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 12) = D(3, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 0) + D(3, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 4) + D(3, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 8) + D(3, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 12) + D(3, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 16) + D(3, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 20);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 16) = D(4, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 0) + D(4, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 4) + D(4, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 8) + D(4, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 12) + D(4, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 16) + D(4, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 20);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 20) = D(5, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 0) + D(5, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 4) + D(5, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 8) + D(5, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 12) + D(5, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 16) + D(5, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block<96, 4>(0, 20);
        ...
        S_scaled_SD_precompute_col_ordered.resize(32, 12);

        for (auto i = 0; i < 32; i++) {
            S_scaled_SD_precompute_col_ordered.block<1, 4>(i, 0).noalias() =
                SPK2_0_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0)) +
                SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4)) +
                SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8));

            S_scaled_SD_precompute_col_ordered.block<1, 4>(i, 4).noalias() =
                SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0)) +
                SPK2_1_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4)) +
                SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8));

            S_scaled_SD_precompute_col_ordered.block<1, 4>(i, 8).noalias() =
                SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0)) +
                SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4)) +
                SPK2_2_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8));
        }
        ...
        if (tile == 0) {
            ...
            for (unsigned int i = 0; i < 32; i++) {
                for (unsigned int j = i; j < 32; j++) {
                    //double d = Kfactor * m_SD_precompute_col_ordered[tile].block<1, 12>(i, 0) * S_scaled_SD_precompute_col_ordered.block<1, 12>(j, 0).transpose();
                    double d = Kfactor * m_SD_precompute_col_ordered[tile].row(i).dot(S_scaled_SD_precompute_col_ordered.row(j));
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ...
    for (unsigned int tile = 0; tile < 16; tile++) {
        // Calculate F is one big block and then split up afterwards to improve efficiency (hopefully)
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> F_Transpose_CombinedBlock_col_ordered = m_SD_precompute_col_ordered[tile].transpose() * e_bar;
        ChMatrixNMc<double, 12, 3> F_Transpose_CombinedBlock_col_ordered = m_SD_precompute_col_ordered[tile].transpose() * e_bar;

        // ChMatrixNM<double, 48, 180> partial_epsilon_partial_e_Transpose;
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 48, 8> partial_epsilon_partial_e_Transpose;
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> partial_epsilon_partial_e_Transpose;
        partial_epsilon_partial_e_Transpose.resize(96, 24);

        for (auto i = 0; i < 32; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>(3 * i, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 1, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 4).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 8).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 12).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 16).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 8).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(8, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 4>((3 * i) + 2, 20).noalias() = m_SD_precompute_col_ordered[tile].block<1, 4>(i, 4).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered[tile].block<1, 4>(i, 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<4, 1>(4, 2).transpose());
        }

        //ChMatrixNMc<double, 12, 3> scaled_F_Transpose_col_ordered =
        //    (Kfactor + m_Alpha * Rfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<12, 3>(0, 0) +
        //    (m_Alpha * Kfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<12, 3>(0, 3);
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor, 12, 3> scaled_F_Transpose_col_ordered;
        //scaled_F_Transpose_col_ordered.resize(12, 3);
        //scaled_F_Transpose_col_ordered = Kfactor * F_Transpose_CombinedBlock_col_ordered;
        ChMatrixNMc<double, 12, 3> scaled_F_Transpose_col_ordered = Kfactor * F_Transpose_CombinedBlock_col_ordered;
        ...
        // ChMatrixNM<double, 48, 8> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor, 96, 24> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> Scaled_Combined_partial_epsilon_partial_e_Transpose;
        Scaled_Combined_partial_epsilon_partial_e_Transpose.resize(96, 24);
...
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::CalcCoordVector(Vector3N& e) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
    ebar.row(3) = m_nodes[0]->GetDDD().eigen();
...
void ChElementBrickANCF_3843_TR08T_GQ444::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBrickANCF_3843_TR08T_GQ444::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::EvaluateSectionFrame(const double xi,
                                                        const double eta,
                                                        const double zeta,
                                                        ChVector<>& point,
                                                        ChQuaternion<>& rot) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::EvaluateSectionPoint(const double u, const double v, const double w, ChVector<>& point) {
    Matrix3x3N Sxi;
    Calc_Sxi(Sxi, u, v, w);
...
void ChElementBrickANCF_3843_TR08T_GQ444::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::LoadableStateIncrement(const unsigned int off_x,
                                                          ChState& x_new,
                                                          const ChState& x,
...
void ChElementBrickANCF_3843_TR08T_GQ444::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBrickANCF_3843_TR08T_GQ444::ComputeNF(
...
double ChElementBrickANCF_3843_TR08T_GQ444::GetDensity() {
...
ChQuadratureTables static_tables_3843_TR08T_GQ444(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBrickANCF_3843_TR08T_GQ444::GetStaticGQTables() {
    return &static_tables_3843_TR08T_GQ444;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBrickANCF_3843_TR08T_GQ444 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBrickANCF_3843_TR08T_GQ444::ChMaterialBrickANCF_3843_TR08T_GQ444(double rho,  // material density
...
ChMaterialBrickANCF_3843_TR08T_GQ444::ChMaterialBrickANCF_3843_TR08T_GQ444(
...
// Poisson effect
void ChMaterialBrickANCF_3843_TR08T_GQ444::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G) {
