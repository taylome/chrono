...
ChElementBrickANCF_3843_TR11::ChElementBrickANCF_3843_TR11()
    : m_gravity_on(false), m_lenX(0), m_lenY(0), m_lenZ(0), m_Alpha(0), m_damping_enabled(false) {
    m_nodes.resize(8);

    m_Ccompact.resize(32768, 32);  // N^3xN
    m_K1.resize(96, 96);           // 3Nx3N
    m_K2.resize(96, 96);           // 3Nx3N
...
void ChElementBrickANCF_3843_TR11::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementBrickANCF_3843_TR11::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBrickANCF_3843_TR11::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBrickANCF_3843_TR11::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBrickANCF_3843_TR11::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                            double Kfactor,
                                                            double Rfactor,
                                                            double Mfactor) {
    ...
    ChMatrixNMc<double, 32, 3> e_bar;
    ChMatrixNMc<double, 32, 3> e_bar_dot;
    ChMatrixNMc<double, 32, 3> e_bar_plus_e_bar_dot;
        ...
        for (unsigned int k = 0; k < 32; k++) {
            ...
            for (unsigned int s = 0; s < 32; s++) {
                d += e_bar_plus_e_bar_dot.row(s) * e_bar.transpose() *
                     m_Ccompact.block<1, 32>(k + 32 * s + 1024 * i, 0).transpose();

                Block -= Kfactor * e_bar.transpose() * m_Ccompact.block<32, 1>(32 * s + 1024 * i, k) *
    ...
    ChMatrixNM<double, 96, 96> JacobianMatrix;
    ...
    ChMatrixNM<double, 32, 32> M = Mfactor * m_MassMatrix;
    for (unsigned int i = 0; i < 32; i++) {
...
void ChElementBrickANCF_3843_TR11::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBrickANCF_3843_TR11::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBrickANCF_3843_TR11::ComputeNodalMass() {
...
void ChElementBrickANCF_3843_TR11::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    unsigned int GQ_idx_xi_eta = 6;  // 7 Point Gauss-Quadrature;
    unsigned int GQ_idx_zeta = 6;    // 7 Point Gauss-Quadrature;

    ChMatrixNM<double, 32, 32> K1compact;
    K1compact.setZero();
    m_Ccompact.setZero();
    m_K1.setZero();
    ...
    epsilion_matrices.resize(192, 32);  // 6*NxN
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi_eta].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_xi_eta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_zeta].size(); it_zeta++) {
                ...
                ChMatrixNMc<double, 32, 3> Sxi_D;  // Matrix of normalized shape function derivatives

                Calc_Sxi_D(Sxi_D, xi, eta, zeta);
                J_0xi.noalias() = m_ebar0 * Sxi_D;
                Sxi_D = Sxi_D * J_0xi.inverse();

                epsilion_matrices.block<32, 32>(0, 0) = 0.5 * Sxi_D.col(0) * Sxi_D.col(0).transpose();
                epsilion_matrices.block<32, 32>(32, 0) = 0.5 * Sxi_D.col(1) * Sxi_D.col(1).transpose();
                epsilion_matrices.block<32, 32>(64, 0) = 0.5 * Sxi_D.col(2) * Sxi_D.col(2).transpose();
                epsilion_matrices.block<32, 32>(96, 0) =
                    0.5 * Sxi_D.col(1) * Sxi_D.col(2).transpose() + 0.5 * Sxi_D.col(2) * Sxi_D.col(1).transpose();
                epsilion_matrices.block<32, 32>(128, 0) =
                    0.5 * Sxi_D.col(0) * Sxi_D.col(2).transpose() + 0.5 * Sxi_D.col(2) * Sxi_D.col(0).transpose();
                epsilion_matrices.block<32, 32>(160, 0) =
                    ...
                    for (unsigned int j = 0; j < 32; j++) {
                        for (unsigned int k = 0; k < 6; k++) {
                            for (unsigned int l = 0; l < 6; l++) {
                                m_Ccompact.block<32, 32>(32 * j + 1024 * i, 0) +=
                                    J_0xi.determinant() * GQ_weight * 2 *
                                    epsilion_matrices.block<1, 32>(i + 32 * k, 0).transpose() * D(k, l) *
                                    epsilion_matrices.block<32, 1>(32 * l, j).transpose();
                            }
                        }
                    }
                }
                ...
                for (unsigned int k = 0; k < 6; k++) {
                    for (unsigned int l = 0; l < 3; l++) {
                        K1compact +=
                            J_0xi.determinant() * GQ_weight * D(k, l) * epsilion_matrices.block<32, 32>(32 * k, 0);
                    }
                }
            }
        }
...
void ChElementBrickANCF_3843_TR11::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementBrickANCF_3843_TR11::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBrickANCF_3843_TR11::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi,
    ...
    for (unsigned int i = 0; i < 32; i++) {
        for (unsigned int j = 0; j < 32; j++) {
            if (i <= j) {
                Block = -ebar.transpose() * m_Ccompact.block<32, 32>(32 * j + 1024 * i, 0) * ebar;
...
void ChElementBrickANCF_3843_TR11::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
                                                            double Kfactor,
                                                            double Rfactor) {
...
void ChElementBrickANCF_3843_TR11::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR11::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
    Sxi_compact(0) =
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR11::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR11::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR11::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR11::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBrickANCF_3843_TR11::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBrickANCF_3843_TR11::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta) {
...
void ChElementBrickANCF_3843_TR11::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
...
void ChElementBrickANCF_3843_TR11::CalcCoordVector(Vector3N& e) {
...
void ChElementBrickANCF_3843_TR11::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementBrickANCF_3843_TR11::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementBrickANCF_3843_TR11::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementBrickANCF_3843_TR11::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementBrickANCF_3843_TR11::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementBrickANCF_3843_TR11::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBrickANCF_3843_TR11::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
void ChElementBrickANCF_3843_TR11::EvaluateSectionFrame(const double xi,
                                                        const double eta,
                                                        const double zeta,
                                                        ChVector<>& point,
                                                        ChQuaternion<>& rot) {
...
void ChElementBrickANCF_3843_TR11::EvaluateSectionPoint(const double u,
                                                        const double v,
                                                        const double w,
                                                        ChVector<>& point) {
    Matrix3x3N Sxi;
...
void ChElementBrickANCF_3843_TR11::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBrickANCF_3843_TR11::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBrickANCF_3843_TR11::LoadableStateIncrement(const unsigned int off_x,
                                                          ChState& x_new,
                                                          const ChState& x,
...
void ChElementBrickANCF_3843_TR11::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBrickANCF_3843_TR11::ComputeNF(
...
double ChElementBrickANCF_3843_TR11::GetDensity() {
...
ChQuadratureTables static_tables_3843_TR11(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBrickANCF_3843_TR11::GetStaticGQTables() {
    return &static_tables_3843_TR11;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBrickANCF_3843_TR11 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBrickANCF_3843_TR11::ChMaterialBrickANCF_3843_TR11(double rho,  // material density
...
ChMaterialBrickANCF_3843_TR11::ChMaterialBrickANCF_3843_TR11(
...
// Poisson effect
void ChMaterialBrickANCF_3843_TR11::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G) {
