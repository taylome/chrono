...
// =============================================================================
// Higher order ANCF shell element with 8 nodes. Description of this element (3833_TR11B) and its internal forces may be
...
//
// =============================================================================
// TR11B = Garcia-Vallejo Based Pre-Integration Approach
...
// Constructor
// ------------------------------------------------------------------------------
ChElementShellANCF_3833_TR11B::ChElementShellANCF_3833_TR11B()
...
void ChElementShellANCF_3833_TR11B::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementShellANCF_3833_TR11B::AddLayer(double thickness,
                                             double theta,
...
void ChElementShellANCF_3833_TR11B::SetDimensions(double lenX, double lenY) {
    m_lenX = lenX;
    m_lenY = lenY;
}
...
void ChElementShellANCF_3833_TR11B::SetMidsurfaceOffset(const double offset) {
    m_midsurfoffset = offset;
}

// Set the value for the single term structural damping coefficient.

void ChElementShellANCF_3833_TR11B::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementShellANCF_3833_TR11B::GetGreenLagrangeStrain(const double xi,
                                                                   const double eta,
                                                                   const double zeta) {
...
ChMatrix33<> ChElementShellANCF_3833_TR11B::GetPK2Stress(const double layer,
                                                         const double xi,
                                                         const double eta,
...
double ChElementShellANCF_3833_TR11B::GetVonMissesStress(const double layer,
                                                         const double xi,
                                                         const double eta,
                                                         const double layer_zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
...
void ChElementShellANCF_3833_TR11B::SetupInitial(ChSystem* system) {
...
void ChElementShellANCF_3833_TR11B::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3833_TR11B::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementShellANCF_3833_TR11B::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementShellANCF_3833_TR11B::ComputeNodalMass() {
...
void ChElementShellANCF_3833_TR11B::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    Fi.noalias() = m_K2 * (e + (2 * m_Alpha) * edot) + m_K1 * e;
}

// Calculate the global matrix H as a linear combination of K, R, and M:
//   H = Mfactor * [M] + Kfactor * [K] + Rfactor * [R]

void ChElementShellANCF_3833_TR11B::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                             double Kfactor,
                                                             double Rfactor,
                                                             double Mfactor) {
    ...
    for (unsigned int m = 0; m < NSF; m++) {
        for (unsigned int n = m; n < NSF; n++) {
            ...
            for (unsigned int k = 0; k < NSF; k++) {
                ...
                Block(1, 1) += G(k);
                Block(2, 2) += G(k);
                H.block<3, 3>(3 * m, 3 * k).noalias() += Kfactor * Block;
            }
            ...
            if (m != n) {
                ...
                for (unsigned int k = 0; k < NSF; k++) {
                    ...
                    Block(1, 1) += G(k);
                    Block(2, 2) += G(k);
                    H.block<3, 3>(3 * n, 3 * k).noalias() += Kfactor * Block;
                }
            }
        }
    }
...
void ChElementShellANCF_3833_TR11B::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementShellANCF_3833_TR11B::EvaluateSectionFrame(const double xi,
                                                         const double eta,
                                                         ChVector<>& point,
                                                         ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementShellANCF_3833_TR11B::EvaluateSectionPoint(const double xi, const double eta, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR11B::EvaluateSectionVelNorm(const double xi, const double eta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR11B::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3833_TR11B::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3833_TR11B::LoadableStateIncrement(const unsigned int off_x,
                                                           ChState& x_new,
                                                           const ChState& x,
...
void ChElementShellANCF_3833_TR11B::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3833_TR11B::ComputeNF(
...
double ChElementShellANCF_3833_TR11B::GetDensity() {
...
ChVector<> ChElementShellANCF_3833_TR11B::ComputeNormal(const double xi, const double eta) {
...
void ChElementShellANCF_3833_TR11B::ComputeMassMatrixAndGravityForce() {
...
void ChElementShellANCF_3833_TR11B::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementShellANCF_3833_TR11B::Calc_Sxi_compact(VectorN& Sxi_compact,
                                                     double xi,
                                                     double eta,
                                                     double zeta,
                                                     double thickness,
...
void ChElementShellANCF_3833_TR11B::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact,
                                                        double xi,
                                                        double eta,
                                                        double zeta,
                                                        double thickness,
...
void ChElementShellANCF_3833_TR11B::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta,
                                                         double thickness,
...
void ChElementShellANCF_3833_TR11B::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                          double xi,
                                                          double eta,
                                                          double zeta,
                                                          double thickness,
...
void ChElementShellANCF_3833_TR11B::Calc_Sxi_D(MatrixNx3c& Sxi_D,
                                               double xi,
                                               double eta,
                                               double zeta,
                                               double thickness,
...
void ChElementShellANCF_3833_TR11B::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3833_TR11B::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3833_TR11B::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3833_TR11B::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3833_TR11B::CalcCombinedCoordMatrix(Matrix6xN& ebar_ebardot) {
...
void ChElementShellANCF_3833_TR11B::Calc_J_0xi(ChMatrix33<double>& J_0xi,
                                               double xi,
                                               double eta,
                                               double zeta,
                                               double thickness,
...
double ChElementShellANCF_3833_TR11B::Calc_det_J_0xi(double xi,
                                                     double eta,
                                                     double zeta,
                                                     double thickness,
                                                     double zoffset) {
...
void ChElementShellANCF_3833_TR11B::RotateReorderStiffnessMatrix(ChMatrixNM<double, 6, 6>& D, double theta) {
...
ChQuadratureTables static_tables_3833_TR11B(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3833_TR11B::GetStaticGQTables() {
    return &static_tables_3833_TR11B;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3833_TR11B::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)

ChElementShellANCF_3833_TR11B::Layer::Layer(double thickness,
                                            double theta,
                                            std::shared_ptr<ChMaterialShellANCF> material)
