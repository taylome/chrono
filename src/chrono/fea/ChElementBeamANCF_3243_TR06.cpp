...
ChElementBeamANCF_3243_TR06::ChElementBeamANCF_3243_TR06()
...
void ChElementBeamANCF_3243_TR06::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementBeamANCF_3243_TR06::SetDimensions(double lenX, double thicknessY, double thicknessZ) {
    m_lenX = lenX;
...
void ChElementBeamANCF_3243_TR06::SetMaterial(std::shared_ptr<ChMaterialBeamANCF> beam_mat) {
    m_material = beam_mat;
}

// Set the value for the single term structural damping coefficient.

void ChElementBeamANCF_3243_TR06::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementBeamANCF_3243_TR06::GetGreenLagrangeStrain(const double xi, const double eta, const double zeta) {
...
ChMatrix33<> ChElementBeamANCF_3243_TR06::GetPK2Stress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBeamANCF_3243_TR06::GetVonMissesStress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
void ChElementBeamANCF_3243_TR06::SetupInitial(ChSystem* system) {
...
void ChElementBeamANCF_3243_TR06::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_3243_TR06::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementBeamANCF_3243_TR06::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementBeamANCF_3243_TR06::ComputeNodalMass() {
    m_nodes[0]->m_TotalMass += m_MassMatrix(0, 0) + m_MassMatrix(0, 4);
    m_nodes[1]->m_TotalMass += m_MassMatrix(4, 0) + m_MassMatrix(4, 4);
}

// Compute the generalized internal force vector for the current nodal coordinates and set the value in the Fi vector.

void ChElementBeamANCF_3243_TR06::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    for (unsigned int GQpnt = 0; GQpnt < NIP_D0; GQpnt++) {
        ...
        // derivative of the Green-Lagrange strain tensor in Voigt notation and then multiply it by the elasticity
        // matrix (diagonal) time the element Jacobian to get the scaled combined second Piola-Kirchhoff tensor in Voigt
        // notation
        ChVectorN<double, 6> sigmaPK2_combined;
        sigmaPK2_combined(0) = 0.5 * (F.col(0).dot(F.col(0)) - 1) + m_Alpha * F.col(0).dot(Fdot.col(0));
        sigmaPK2_combined(1) = 0.5 * (F.col(1).dot(F.col(1)) - 1) + m_Alpha * F.col(1).dot(Fdot.col(1));
        sigmaPK2_combined(2) = 0.5 * (F.col(2).dot(F.col(2)) - 1) + m_Alpha * F.col(2).dot(Fdot.col(2));
        sigmaPK2_combined(3) =
        ...
        sigmaPK2_combined.array() *= D0.array() * m_kGQ_D0(GQpnt);
...
void ChElementBeamANCF_3243_TR06::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                           double Kfactor,
                                                           double Rfactor,
                                                           double Mfactor) {
    ...
    MatrixNxN H_CompactPart = Mfactor * m_MassMatrix;
    ...
    for (unsigned int GQpnt = 0; GQpnt < NIP_D0; GQpnt++) {
        ...
        sigmaPK2_combined(5) =
            F.col(0).dot(F.col(1)) + m_Alpha * (F.col(0).dot(Fdot.col(1)) + Fdot.col(0).dot(F.col(1)));
        sigmaPK2_combined.array() *= D0.array() * (Kfactor * m_kGQ_D0(GQpnt));
        ...
        H_CompactPart.noalias() -= Sbar_xi_D * SPK2 * Sbar_xi_D.transpose();
        ...
        ChMatrixNMc<double, 3, 3> Fcombined =
            -m_kGQ_D0(GQpnt) * ((Kfactor + m_Alpha * Rfactor) * F + (Kfactor * m_Alpha) * Fdot);
        ...
        PartialSigma0Compact = D0(0) * Sbar_xi_D.col(0) * Fcombined.col(0).transpose();
        PartialSigma1Compact = D0(1) * Sbar_xi_D.col(1) * Fcombined.col(1).transpose();
        PartialSigma2Compact = D0(2) * Sbar_xi_D.col(2) * Fcombined.col(2).transpose();
        PartialSigma3Compact =
            D0(3) * (Sbar_xi_D.col(2) * Fcombined.col(1).transpose() + Sbar_xi_D.col(1) * Fcombined.col(2).transpose());
        PartialSigma4Compact =
            D0(4) * (Sbar_xi_D.col(2) * Fcombined.col(0).transpose() + Sbar_xi_D.col(0) * Fcombined.col(2).transpose());
        PartialSigma5Compact =
            D0(5) * (Sbar_xi_D.col(1) * Fcombined.col(0).transpose() + Sbar_xi_D.col(0) * Fcombined.col(1).transpose());

        H.noalias() += PartialEpsilon.transpose() * PartialSigmaCombined;
    }
    ...
    for (unsigned int GQpnt = 0; GQpnt < NIP_Dv; GQpnt++) {
        ...
        epsilon_combined(1) = 0.5 * (F.col(1).dot(F.col(1)) - 1) + m_Alpha * F.col(1).dot(Fdot.col(1));
        epsilon_combined(2) = 0.5 * (F.col(2).dot(F.col(2)) - 1) + m_Alpha * F.col(2).dot(Fdot.col(2));
        epsilon_combined *= Kfactor * m_kGQ_Dv(GQpnt);

        // 2nd Piola Kirchhoff Stress tensor (Just the diagonal terms are non-zero)
        ChVectorN<double, 3> SPK2 = Dv * epsilon_combined;

        H_CompactPart.noalias() -= Sbar_xi_D * SPK2.asDiagonal() * Sbar_xi_D.transpose();

        Matrix3x3N PartialEpsilon;
        Eigen::Map<MatrixNx3> PartialEpsilon0Compact(PartialEpsilon.row(0).data(), NSF, 3);
        Eigen::Map<MatrixNx3> PartialEpsilon1Compact(PartialEpsilon.row(1).data(), NSF, 3);
        Eigen::Map<MatrixNx3> PartialEpsilon2Compact(PartialEpsilon.row(2).data(), NSF, 3);
        ...
        ChMatrixNMc<double, 3, 3> Fcombined =
            -m_kGQ_Dv(GQpnt) * ((Kfactor + m_Alpha * Rfactor) * F + (Kfactor * m_Alpha) * Fdot);

        Matrix3x3N PartialSigmaCombined;
        Eigen::Map<MatrixNx3> PartialSigma0Compact(PartialSigmaCombined.row(0).data(), NSF, 3);
        Eigen::Map<MatrixNx3> PartialSigma1Compact(PartialSigmaCombined.row(1).data(), NSF, 3);
        Eigen::Map<MatrixNx3> PartialSigma2Compact(PartialSigmaCombined.row(2).data(), NSF, 3);

        PartialSigma0Compact = Sbar_xi_D.col(0) * (Dv(0, 0) * Fcombined.col(0).transpose()) +
                               Sbar_xi_D.col(1) * (Dv(0, 1) * Fcombined.col(1).transpose()) +
                               Sbar_xi_D.col(2) * (Dv(0, 2) * Fcombined.col(2).transpose());
        PartialSigma1Compact = Sbar_xi_D.col(0) * (Dv(1, 0) * Fcombined.col(0).transpose()) +
                               Sbar_xi_D.col(1) * (Dv(1, 1) * Fcombined.col(1).transpose()) +
                               Sbar_xi_D.col(2) * (Dv(1, 2) * Fcombined.col(2).transpose());
        PartialSigma2Compact = Sbar_xi_D.col(0) * (Dv(2, 0) * Fcombined.col(0).transpose()) +
                               Sbar_xi_D.col(1) * (Dv(2, 1) * Fcombined.col(1).transpose()) +
                               Sbar_xi_D.col(2) * (Dv(2, 2) * Fcombined.col(2).transpose());
    ...
    for (unsigned int i = 0; i < NSF; i++) {
        for (unsigned int j = 0; j < NSF; j++) {
            H(3 * i, 3 * j) += H_CompactPart(i, j);
            H(3 * i + 1, 3 * j + 1) += H_CompactPart(i, j);
            H(3 * i + 2, 3 * j + 2) += H_CompactPart(i, j);
        }
    }
}

// Compute the generalized force vector due to gravity using the efficient ANCF specific method
void ChElementBeamANCF_3243_TR06::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementBeamANCF_3243_TR06::EvaluateSectionFrame(const double xi, ChVector<>& point, ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementBeamANCF_3243_TR06::EvaluateSectionPoint(const double xi, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR06::EvaluateSectionVel(const double xi, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR06::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_3243_TR06::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_3243_TR06::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementBeamANCF_3243_TR06::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_3243_TR06::ComputeNF(
...
double ChElementBeamANCF_3243_TR06::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at coordinate xi [-1 to 1].

ChVector<> ChElementBeamANCF_3243_TR06::ComputeTangent(const double xi) {
    VectorN Sxi_xi_compact;
    Calc_Sxi_xi_compact(Sxi_xi_compact, xi, 0, 0);
...
void ChElementBeamANCF_3243_TR06::ComputeMassMatrixAndGravityForce() {
...
void ChElementBeamANCF_3243_TR06::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[NP - 1].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[NT - 1].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[NT - 1].size(); it_zeta++) {
                ...
                m_SD_D0.block<NSF, 3>(0, 3 * index).noalias() = Sxi_D * J_0xi.inverse();
            }
        }
    }
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[NP - 1].size(); it_xi++) {
        ...
        // Adjust the shape function derivative matrix to account for a potentially deformed reference state
        m_SD_Dv.block<NSF, 3>(0, 3 * it_xi).noalias() = Sxi_D * J_0xi.inverse();
    }
}
...
void ChElementBeamANCF_3243_TR06::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR06::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR06::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR06::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_3243_TR06::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
    VectorN Sxi_D_col;
...
void ChElementBeamANCF_3243_TR06::CalcCoordVector(Vector3N& e) {
...
void ChElementBeamANCF_3243_TR06::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementBeamANCF_3243_TR06::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementBeamANCF_3243_TR06::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementBeamANCF_3243_TR06::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementBeamANCF_3243_TR06::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementBeamANCF_3243_TR06::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
ChQuadratureTables static_tables_3243_TR06(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_3243_TR06::GetStaticGQTables() {
    return &static_tables_3243_TR06;
}
