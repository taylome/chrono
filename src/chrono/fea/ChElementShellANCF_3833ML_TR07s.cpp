...
ChElementShellANCF_3833ML_TR07S::ChElementShellANCF_3833ML_TR07S()
...
void ChElementShellANCF_3833ML_TR07S::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementShellANCF_3833ML_TR07S::AddLayer(double thickness, double theta, std::shared_ptr<ChMaterialShellANCF_3833ML_TR07S> material) {
...
void ChElementShellANCF_3833ML_TR07S::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementShellANCF_3833ML_TR07S::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementShellANCF_3833ML_TR07S::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3833ML_TR07S::ComputeKRMmatricesGlobal(ChMatrixRef H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
...
void ChElementShellANCF_3833ML_TR07S::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementShellANCF_3833ML_TR07S::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementShellANCF_3833ML_TR07S::ComputeNodalMass() {
...
void ChElementShellANCF_3833ML_TR07S::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    unsigned int GQ_idx_xi_eta_zeta = 3;        // 4 Point Gauss-Quadrature;

    m_numLayerGQPnts = (GQ_idx_xi_eta_zeta + 1) * (GQ_idx_xi_eta_zeta + 1) * (GQ_idx_xi_eta_zeta + 1);
...
void ChElementShellANCF_3833ML_TR07S::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementShellANCF_3833ML_TR07S::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3833ML_TR07S::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const MatrixNx6& ebar_ebardot) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        VectorNIP E0_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0));
        E0_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1));
        E0_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2));
        E0_Block.array() -= 1;
        E0_Block *= 0.5;
        VectorNIP E_BlockDamping = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 5));
        E0_Block += m_Alpha * E_BlockDamping;
        E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E1_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0));
        E1_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1));
        E1_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2));
        E1_Block.array() -= 1;
        E1_Block *= 0.5;
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 5));
        E1_Block += m_Alpha * E_BlockDamping;
        E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E2_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0));
        E2_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1));
        E2_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2));
        E2_Block.array() -= 1;
        E2_Block *= 0.5;
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 5));
        E2_Block += m_Alpha * E_BlockDamping;
        E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E3_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0));
        E3_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1));
        E3_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2));
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(
        ...
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 5));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(
        ...
        VectorNIP E4_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0));
        E4_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1));
        E4_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2));
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(
        ...
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 5));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 4));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(
        ...
        VectorNIP E5_Block = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0));
        E5_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1));
        E5_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2));
        E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 3));
        E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(
        ...
        m_SPK2_0_Block.block<64, 1>(kl * 64, 0) = SPK2_0_Block;
        m_SPK2_1_Block.block<64, 1>(kl * 64, 0) = SPK2_1_Block;
        m_SPK2_2_Block.block<64, 1>(kl * 64, 0) = SPK2_2_Block;
        m_SPK2_3_Block.block<64, 1>(kl * 64, 0) = SPK2_3_Block;
        m_SPK2_4_Block.block<64, 1>(kl * 64, 0) = SPK2_4_Block;
        m_SPK2_5_Block.block<64, 1>(kl * 64, 0) = SPK2_5_Block;

        //m_SPK2_0_Block.block<64, 1>(kl*m_numLayerGQPnts, 0) = D(0, 0)*E0_Block + D(0, 1)*E1_Block + D(0, 2)*E2_Block + D(0, 3)*E3_Block + D(0, 4)*E4_Block + D(0, 5)*E5_Block;
        //m_SPK2_1_Block.block<64, 1>(kl*m_numLayerGQPnts, 0) = D(1, 0)*E0_Block + D(1, 1)*E1_Block + D(1, 2)*E2_Block + D(1, 3)*E3_Block + D(1, 4)*E4_Block + D(1, 5)*E5_Block;
        //m_SPK2_2_Block.block<64, 1>(kl*m_numLayerGQPnts, 0) = D(2, 0)*E0_Block + D(2, 1)*E1_Block + D(2, 2)*E2_Block + D(2, 3)*E3_Block + D(2, 4)*E4_Block + D(2, 5)*E5_Block;
        //m_SPK2_3_Block.block<64, 1>(kl*m_numLayerGQPnts, 0) = D(3, 0)*E0_Block + D(3, 1)*E1_Block + D(3, 2)*E2_Block + D(3, 3)*E3_Block + D(3, 4)*E4_Block + D(3, 5)*E5_Block;
        //m_SPK2_4_Block.block<64, 1>(kl*m_numLayerGQPnts, 0) = D(4, 0)*E0_Block + D(4, 1)*E1_Block + D(4, 2)*E2_Block + D(4, 3)*E3_Block + D(4, 4)*E4_Block + D(4, 5)*E5_Block;
        //m_SPK2_5_Block.block<64, 1>(kl*m_numLayerGQPnts, 0) = D(5, 0)*E0_Block + D(5, 1)*E1_Block + D(5, 2)*E2_Block + D(5, 3)*E3_Block + D(5, 4)*E4_Block + D(5, 5)*E5_Block;
        ...
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 0) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 0) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 1) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 1) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 2) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 2) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(SPK2_4_Block);

        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 0) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 0) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 1) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 1) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 2) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 2) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(SPK2_3_Block);

        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 0) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 0) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 1) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 1) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 2) =
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 2) +=
            m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(SPK2_2_Block);

        // =============================================================================

        QiCompact += m_SD_precompute_col_ordered.block<24, 192>(0, 192 * kl) * P_transpose_scaled_Block_col_ordered;
    }
...
void ChElementShellANCF_3833ML_TR07S::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        VectorNIP E0_Block = m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 0));
        E0_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 1));
        E0_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 2));
        E0_Block.array() -= 1;
        E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E0_Block *= 0.5;

        VectorNIP E1_Block = m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0));
        E1_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1));
        E1_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2));
        E1_Block.array() -= 1;
        E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E1_Block *= 0.5;

        VectorNIP E2_Block = m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0));
        E2_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1));
        E2_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2));
        E2_Block.array() -= 1;
        E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E2_Block *= 0.5;

        VectorNIP E3_Block = m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0));
        E3_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1));
        E3_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2));
        E3_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E4_Block = m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0));
        E4_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1));
        E4_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2));
        E4_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E5_Block = m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0));
        E5_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1));
        E5_Block += m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2));
        E5_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        ...
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 0) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 0) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 1) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 1) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 2) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(SPK2_0_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(0, 2) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(SPK2_4_Block);

        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 0) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 0) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 1) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 1) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 2) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(SPK2_5_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(64, 2) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(SPK2_3_Block);

        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 0) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 0).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 0) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 1) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 1).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 1) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 2) =
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl, 2).cwiseProduct(SPK2_4_Block) +
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<64, 1>(128, 2) +=
            m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2).cwiseProduct(SPK2_2_Block);
...
void ChElementShellANCF_3833ML_TR07S::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
                                                           double Kfactor,
                                                           double Rfactor) {
...
void ChElementShellANCF_3833ML_TR07S::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (auto i = 0; i < 24; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(192 * kl + 64, 2).transpose());
        }
        ...
        ChMatrixNMc<double, 192, 3> scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<192, 3>(192 * kl + 0, 0) +
            (m_Alpha * Kfactor) * m_F_Transpose_CombinedBlockDamping_col_ordered.block<192, 3>(192 * kl + 0, 3);
        ...
        for (auto i = 0; i < 24; i++) {
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 0).transpose());

            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 1).transpose());

            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(128, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<64, 1>(64, 2).transpose());
        }
        ...
        for (auto i = 0; i < 24; i++) {
            S_scaled_SD_precompute_col_ordered.block<1, 64>(i, 0).noalias() =
                m_SPK2_0_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0)) +
                m_SPK2_5_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64)) +
                m_SPK2_4_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128));

            S_scaled_SD_precompute_col_ordered.block<1, 64>(i, 64).noalias() =
                m_SPK2_5_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0)) +
                m_SPK2_1_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64)) +
                m_SPK2_3_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128));

            S_scaled_SD_precompute_col_ordered.block<1, 64>(i, 128).noalias() =
                m_SPK2_4_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0)) +
                m_SPK2_3_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64)) +
                m_SPK2_2_Block.block<64, 1>(kl * 64, 0).transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128));
        }
        ...
        for (unsigned int i = 0; i < 24; i++) {
            for (unsigned int j = i; j < 24; j++) {
                double d = Kfactor * m_SD_precompute_col_ordered.block<1, 192>(i, 192 * kl) * S_scaled_SD_precompute_col_ordered.block<1, 192>(j, 0).transpose();
...
void ChElementShellANCF_3833ML_TR07S::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (auto i = 0; i < 24; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block<64, 1>(192 * kl + 64, 2).transpose());
        }

        ChMatrixNMc<double, 192, 3> scaled_F_Transpose_col_ordered = Kfactor * m_F_Transpose_CombinedBlock_col_ordered.block<192, 3>(192 * kl + 0, 0);
...
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta, double thickness, double zoffset) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta, thickness, zoffset);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR07S::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR07S::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3833ML_TR07S::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3833ML_TR07S::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementShellANCF_3833ML_TR07S::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
...
void ChElementShellANCF_3833ML_TR07S::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3833ML_TR07S::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3833ML_TR07S::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementShellANCF_3833ML_TR07S::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementShellANCF_3833ML_TR07S::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta, double thickness, double zoffset) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta, thickness, zoffset);
...
double ChElementShellANCF_3833ML_TR07S::Calc_det_J_0xi(double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR07S::EvaluateSectionFrame(const double xi, const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    Matrix3x3N Sxi;
    Matrix3x3N Sxi_xi;
    Matrix3x3N Sxi_eta;
...
void ChElementShellANCF_3833ML_TR07S::EvaluateSectionPoint(const double u, const double v, ChVector<>& point) {
    Matrix3x3N Sxi;
...
void ChElementShellANCF_3833ML_TR07S::EvaluateSectionVelNorm(double U, double V, ChVector<>& Result) {
    Matrix3x3N Sxi_zeta;
    Vector3N edot;
    ChVector<> r_zeta;
...
void ChElementShellANCF_3833ML_TR07S::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3833ML_TR07S::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3833ML_TR07S::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementShellANCF_3833ML_TR07S::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3833ML_TR07S::ComputeNF(
...
double ChElementShellANCF_3833ML_TR07S::GetDensity() {
...
ChVector<> ChElementShellANCF_3833ML_TR07S::ComputeNormal(const double U, const double V) {
    Matrix3x3N Sxi_zeta;
    Vector3N e;
    ChVector<> r_zeta;
...
ChQuadratureTables static_tables_3833ML_TR07S(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3833ML_TR07S::GetStaticGQTables() {
    return &static_tables_3833ML_TR07S;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3833ML_TR07S::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)
ChElementShellANCF_3833ML_TR07S::Layer::Layer(ChElementShellANCF_3833ML_TR07S* element,
    double thickness,
    double theta,
    std::shared_ptr<ChMaterialShellANCF_3833ML_TR07S> material)
    : m_element(element), m_thickness(thickness), m_theta(theta), m_material(material) {}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialShellANCF_3833ML_TR07S methods
// ============================================================================

// Construct an isotropic material.
ChMaterialShellANCF_3833ML_TR07S::ChMaterialShellANCF_3833ML_TR07S(
...
// Poisson effect
void ChMaterialShellANCF_3833ML_TR07S::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G) {
...
void ChMaterialShellANCF_3833ML_TR07S::Get_Rotated_D(ChMatrixNM<double, 6, 6>& D, double theta) {
