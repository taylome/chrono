...
// =============================================================================
//
// MT26 = MT25 with caching of Fblock & Strains with Analytic Jacobian again
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT26::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT26::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT26::ChElementBeamANCF_MT26() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT26::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT26::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT26::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT26::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT26::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
    ...
    if (m_damping_enabled) {        //If linear Kelvin-Voigt viscoelastic material model is enabled 
        ...
        for (unsigned int index = 0; index < 16; index++) {
            ...
            Fcol0transpose = m_F_Block.block(0, 3 * index, 3, 1).transpose();
            Fcol1transpose = m_F_Block.block(0, (3 * index) + 1, 3, 1).transpose();
            Fcol2transpose = m_F_Block.block(0, (3 * index) + 2, 3, 1).transpose();
            ...
            Fdotcol0transpose = m_Fdot_Block.block(0, 3 * index, 3, 1).transpose();
            Fdotcol1transpose = m_Fdot_Block.block(0, (3 * index) + 1, 3, 1).transpose();
            Fdotcol2transpose = m_Fdot_Block.block(0, (3 * index) + 2, 3, 1).transpose();
        ...
        for (unsigned int index = 16; index < 20; index++) {
            SDCol0 = m_SD_precompute.block(0, 3 * index, 9, 1);
            SDCol1 = m_SD_precompute.block(0, (3 * index) + 1, 9, 1);
            SDCol2 = m_SD_precompute.block(0, (3 * index) + 2, 9, 1);
...
void ChElementBeamANCF_MT26::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT26::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT26::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_MT26::ComputeNodalMass() {
...
void ChElementBeamANCF_MT26::SetAlphaDamp(double a) {
    m_Alpha = a;
    if (std::abs(m_Alpha) > 1e-10)
        m_damping_enabled = true;
}

void ChElementBeamANCF_MT26::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBeamANCF_MT26::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const ChMatrixNMc<double, 9, 3>& e_bar, const ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ...
    for (unsigned int index = 0; index < 16; index++) {
        ...
        m_SPK2((6 * index) + 3) = m_GQWeight_det_J_0xi(index)*D0(3)*(FCol1.dot(FCol2) + 0.5*m_Alpha*(FCol1.dot(FdotCol2)+ FCol2.dot(FdotCol1))); //Smat(1, 2) = Smat(2, 1)
        m_SPK2((6 * index) + 4) = m_GQWeight_det_J_0xi(index)*D0(4)*(FCol0.dot(FCol2) + 0.5*m_Alpha*(FCol0.dot(FdotCol2) + FCol2.dot(FdotCol0))); //Smat(0, 2) = Smat(2, 0)
        m_SPK2((6 * index) + 5) = m_GQWeight_det_J_0xi(index)*D0(5)*(FCol0.dot(FCol1) + 0.5*m_Alpha*(FCol0.dot(FdotCol1) + FCol1.dot(FdotCol0))); //Smat(0, 1) = Smat(1, 0)
...
void ChElementBeamANCF_MT26::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi, const ChMatrixNMc<double, 9, 3>& e_bar) {
...
void ChElementBeamANCF_MT26::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
...
void ChElementBeamANCF_MT26::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
...
void ChElementBeamANCF_MT26::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
...
void ChElementBeamANCF_MT26::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT26::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT26::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, double GQ_weight, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot){
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNMc<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChVectorN<double, 6> epsilon;           //Green-Lagrange strain tensor in Voigt notation
...
void ChElementBeamANCF_MT26::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT26::ComputeInternalJacobiansAnalytic(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> e_bar_dot;
    ...
    //Calculate the portion of the Selective Reduced Integration that account for the Poisson effect, but only on the beam axis
    if (GetStrainFormulation() == ChElementBeamANCF_MT26::StrainFormulation::CMPoisson) {
...
void ChElementBeamANCF_MT26::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT26::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT26::Calc_Sxi_D(ChMatrixNMc<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT26::CalcCoordMatrix(ChMatrixNMc<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT26::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT26::CalcCoordDerivMatrix(ChMatrixNMc<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT26::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT26::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT26::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNMc<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT26::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT26::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_MT26::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
...
// void ChElementBeamANCF_MT26::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT26::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT26::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT26::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT26::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT26::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT26::ComputeNF(
...
double ChElementBeamANCF_MT26::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT26::ComputeTangent(const double U) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT26(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT26::GetStaticGQTables() {
    return &static_tables_MT26;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT26 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT26::ChMaterialBeamANCF_MT26(double rho,        // material density
...
void ChMaterialBeamANCF_MT26::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
