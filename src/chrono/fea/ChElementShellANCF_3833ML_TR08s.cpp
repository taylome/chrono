...
ChElementShellANCF_3833ML_TR08S::ChElementShellANCF_3833ML_TR08S()
...
void ChElementShellANCF_3833ML_TR08S::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementShellANCF_3833ML_TR08S::AddLayer(double thickness, double theta, std::shared_ptr<ChMaterialShellANCF_3833ML_TR08S> material) {
...
void ChElementShellANCF_3833ML_TR08S::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementShellANCF_3833ML_TR08S::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementShellANCF_3833ML_TR08S::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3833ML_TR08S::ComputeKRMmatricesGlobal(ChMatrixRef H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
...
void ChElementShellANCF_3833ML_TR08S::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementShellANCF_3833ML_TR08S::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementShellANCF_3833ML_TR08S::ComputeNodalMass() {
...
void ChElementShellANCF_3833ML_TR08S::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementShellANCF_3833ML_TR08S::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementShellANCF_3833ML_TR08S::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3833ML_TR08S::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const MatrixNx6& ebar_ebardot) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ChMatrixNMc<double, 192, 6> F_Transpose_CombinedBlockDamping_col_ordered = m_SD_precompute_col_ordered.block<24, 192>(0, 192 * kl).transpose() * ebar_ebardot;

        VectorNIP  E0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 0).cwiseProduct(
        ...
        E0_Block.array() -= 1;
        E0_Block *= 0.5;
        VectorNIP E_BlockDamping = F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 0).cwiseProduct(
        ...
        VectorNIP  E1_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 0).cwiseProduct(
        ...
        VectorNIP  E2_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 0).cwiseProduct(
        ...
        VectorNIP  E3_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 0));
        ...
        VectorNIP  E4_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 0));
        ...
        VectorNIP  E5_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 0));
...
void ChElementShellANCF_3833ML_TR08S::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ChMatrixNMc<double, 192, 3>  F_Transpose_CombinedBlock_col_ordered = m_SD_precompute_col_ordered.block<24, 192>(0, 192 * kl).transpose() * e_bar;

        VectorNIP  E0_Block = F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 0).cwiseProduct(
        ...
        VectorNIP  E1_Block = F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 0).cwiseProduct(
        ...
        VectorNIP  E2_Block = F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 0).cwiseProduct(
        ...
        VectorNIP  E3_Block = F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 0));
        ...
        VectorNIP  E4_Block = F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 0));
        ...
        VectorNIP  E5_Block = F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 0));
...
void ChElementShellANCF_3833ML_TR08S::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
                                                           double Kfactor,
                                                           double Rfactor) {
...
void ChElementShellANCF_3833ML_TR08S::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (auto i = 0; i < 24; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<64, 1>(64, 2).transpose());
        }
        ...
        ChMatrixNMc<double, 192, 3> scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<192, 3>(0, 0) +
            (m_Alpha * Kfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<192, 3>(0, 3);
        ...
        for (auto i = 0; i < 24; i++) {
            S_scaled_SD_precompute_col_ordered.block<1, 64>(i, 0).noalias() =
                SPK2_0_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0)) +
                SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64)) +
                SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128));

            S_scaled_SD_precompute_col_ordered.block<1, 64>(i, 64).noalias() =
                SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0)) +
                SPK2_1_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64)) +
                SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128));

            S_scaled_SD_precompute_col_ordered.block<1, 64>(i, 128).noalias() =
                SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0)) +
                SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64)) +
                SPK2_2_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128));
        }
...
void ChElementShellANCF_3833ML_TR08S::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (auto i = 0; i < 24; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>(3 * i, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 1, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 0).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 64).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 128).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 192).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 256).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 128).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(128, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 64>((3 * i) + 2, 320).noalias() = m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 64).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 64>(i, 192 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<64, 1>(64, 2).transpose());
        }
        ...
        //Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> scaled_F_Transpose_col_ordered;
        //scaled_F_Transpose_col_ordered.resize(192, 3);
        ChMatrixNMc<double, 192, 3> scaled_F_Transpose_col_ordered = Kfactor * F_Transpose_CombinedBlock_col_ordered;
...
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta, double thickness, double zoffset) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta, thickness, zoffset);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR08S::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR08S::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3833ML_TR08S::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3833ML_TR08S::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementShellANCF_3833ML_TR08S::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
...
void ChElementShellANCF_3833ML_TR08S::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3833ML_TR08S::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3833ML_TR08S::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementShellANCF_3833ML_TR08S::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementShellANCF_3833ML_TR08S::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta, double thickness, double zoffset) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta, thickness, zoffset);
...
double ChElementShellANCF_3833ML_TR08S::Calc_det_J_0xi(double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR08S::EvaluateSectionFrame(const double xi, const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    Matrix3x3N Sxi;
    Matrix3x3N Sxi_xi;
    Matrix3x3N Sxi_eta;
...
void ChElementShellANCF_3833ML_TR08S::EvaluateSectionPoint(const double u, const double v, ChVector<>& point) {
    Matrix3x3N Sxi;
...
void ChElementShellANCF_3833ML_TR08S::EvaluateSectionVelNorm(double U, double V, ChVector<>& Result) {
    Matrix3x3N Sxi_zeta;
    Vector3N edot;
    ChVector<> r_zeta;
...
void ChElementShellANCF_3833ML_TR08S::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3833ML_TR08S::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3833ML_TR08S::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementShellANCF_3833ML_TR08S::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3833ML_TR08S::ComputeNF(
...
double ChElementShellANCF_3833ML_TR08S::GetDensity() {
...
ChVector<> ChElementShellANCF_3833ML_TR08S::ComputeNormal(const double U, const double V) {
    Matrix3x3N Sxi_zeta;
    Vector3N e;
    ChVector<> r_zeta;
...
ChQuadratureTables static_tables_3833ML_TR08S(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3833ML_TR08S::GetStaticGQTables() {
    return &static_tables_3833ML_TR08S;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3833ML_TR08S::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)
ChElementShellANCF_3833ML_TR08S::Layer::Layer(ChElementShellANCF_3833ML_TR08S* element,
    double thickness,
    double theta,
    std::shared_ptr<ChMaterialShellANCF_3833ML_TR08S> material)
    : m_element(element), m_thickness(thickness), m_theta(theta), m_material(material) {}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialShellANCF_3833ML_TR08S methods
// ============================================================================

// Construct an isotropic material.
ChMaterialShellANCF_3833ML_TR08S::ChMaterialShellANCF_3833ML_TR08S(
...
// Poisson effect
void ChMaterialShellANCF_3833ML_TR08S::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G) {
...
void ChMaterialShellANCF_3833ML_TR08S::Get_Rotated_D(ChMatrixNM<double, 6, 6>& D, double theta) {
