...
// =============================================================================
//
// MT24 = MT23 + More internal force calculation tweaks
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT24::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT24::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT24::ChElementBeamANCF_MT24() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT24::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT24::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT24::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT24::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT24::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
    ...
    if (GetStrainFormulation() == ChElementBeamANCF_MT24::StrainFormulation::CMPoisson) {
...
void ChElementBeamANCF_MT24::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT24::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT24::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementBeamANCF_MT24::ComputeNodalMass() {
...
void ChElementBeamANCF_MT24::SetAlphaDamp(double a) {
    m_Alpha = a;
...
void ChElementBeamANCF_MT24::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ChMatrixNMc<double, 9, 3> e_bar;
    CalcCoordMatrix(e_bar);
...
void ChElementBeamANCF_MT24::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const ChMatrixNMc<double, 9, 3>& e_bar, const ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ...
    ChMatrixNMc<double, 3, 60> F_Block = e_bar.transpose()*m_SD_precompute;
    ChMatrixNMc<double, 60, 3> P_transpose_scaled_Block;
    ChMatrixNMc<double, 3, 60> Fdot_Block = e_bar_dot.transpose()*m_SD_precompute;

    ChMatrix33<double> F;                   //Deformation Gradient tensor (non-symmetric tensor)
    ChMatrix33<double> F_dot;               //Time Derivative of the Deformation Gradient tensor (non-symmetric tensor)
    ...
    ChMatrixNM<double, 3, 3> Smat;          //2nd Piola-Kirchoff stress tensor (symmetric tensor)
    ChVectorN<double, 3> Sdiag;             //2nd Piola-Kirchoff stress tensor (symmetric tensor) - Diagonal in this special case due to the structure of the 6x6 stiffness matrix

    for (unsigned int index = 0; index < 16; index++) {
        F.noalias() = F_Block.block(0, 3 * index, 3, 3);
        F_dot.noalias() = Fdot_Block.block(0, 3 * index, 3, 3);
        ...
        Smat(0, 0) = D0(0)*E(0, 0);
        Smat(0, 1) = 2.0*D0(5)*E(0, 1);
        Smat(0, 2) = 2.0*D0(4)*E(0, 2);
        Smat(1, 0) = Smat(0, 1);
        Smat(1, 1) = D0(1)*E(1, 1);
        Smat(1, 2) = 2.0*D0(3)*E(1, 2);
        Smat(2, 0) = Smat(0, 2);
        Smat(2, 1) = Smat(1, 2);
        Smat(2, 2) = D0(2)*E(2, 2);

        P_transpose_scaled_Block.block(3 * index, 0, 3, 3).noalias() = m_GQWeight_det_J_0xi(index)*Smat*F.transpose();
    }
    for (unsigned int index = 16; index < 20; index++) {
        ...
        //Carry out the multiplication with the specific provided diagonal 6x6 stiffness tensor
        // (without explicitly forming the matrix multiplication in Voigt notation)
        Sdiag.noalias() = Dv*E.diagonal();

        P_transpose_scaled_Block.block(3 * index, 0, 3, 3).noalias() = m_GQWeight_det_J_0xi(index)*Sdiag.asDiagonal()*F.transpose();
    }
...
void ChElementBeamANCF_MT24::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi, const ChMatrixNMc<double, 9, 3>& e_bar) {
    ...
    ChMatrixNMc<double, 3, 60> F_Block = e_bar.transpose()*m_SD_precompute;
    ChMatrixNM<double, 60, 3> P_transpose_scaled_Block;
    ...
    ChVectorN<double, 3> FCol0;
    ChVectorN<double, 3> FCol1;
    ChVectorN<double, 3> FCol2;
    ChRowVectorN<double, 3> P_transpose_row0;
    ChRowVectorN<double, 3> P_transpose_row1;
    ChRowVectorN<double, 3> P_transpose_row2;
    ChVectorN<double, 3> Ediag;

    ChVectorN<double, 3> SCol0;
    ChVectorN<double, 3> SCol1;
    ChVectorN<double, 3> SCol2;
    ChMatrixNM<double, 3, 3> P_transpose_scaled;
    ...
    for (unsigned int index = 0; index < 16; index++) {
        ...
        ////E.noalias() = 0.5*F_Block.block(0, 3 * index, 3, 3).transpose()*F_Block.block(0, 3 * index, 3, 3); //still need to substract 0.5*I_3x3

        //////2nd PK2 Stress tensor is symmetric just like the Green-Lagrange Strain tensor
        //////Carry out the multiplication with the specific provided diagonal 6x6 stiffness tensor
        ////// (without explicitly forming the matrix multiplication in Voigt notation)
        ////Smat(0, 0) = D0(0)*(E(0, 0)-0.5);
        ////Smat(0, 1) = 2.0*D0(5)*E(0, 1);
        ////Smat(0, 2) = 2.0*D0(4)*E(0, 2);
        ////Smat(1, 0) = Smat(0, 1);
        ////Smat(1, 1) = D0(1)*(E(1, 1)-0.5);
        ////Smat(1, 2) = 2.0*D0(3)*E(1, 2);
        ////Smat(2, 0) = Smat(0, 2);
        ////Smat(2, 1) = Smat(1, 2);
        ////Smat(2, 2) = D0(2)*(E(2, 2)-0.5);

        ////P_transpose_scaled_Block.block(3 * index, 0, 3, 3).noalias() = m_GQWeight_det_J_0xi(index)*Smat*F.transpose();

        //--------------------------------------------------------------------------------

        //ChMatrixNM<double, 3, 3> Smat2;
        //F.noalias() = F_Block.block(0, 3 * index, 3, 3);
        //E.noalias() = 0.5*(F.transpose()*F - I_3x3);
        //Smat2(0, 0) = D0(0)*E(0, 0);
        //Smat2(0, 1) = 2.0*D0(5)*E(0, 1);
        //Smat2(0, 2) = 2.0*D0(4)*E(0, 2);
        //Smat2(1, 0) = Smat2(0, 1);
        //Smat2(1, 1) = D0(1)*E(1, 1);
        //Smat2(1, 2) = 2.0*D0(3)*E(1, 2);
        //Smat2(2, 0) = Smat2(0, 2);
        //Smat2(2, 1) = Smat2(1, 2);
        //Smat2(2, 2) = D0(2)*E(2, 2);
        //Smat2 *= m_GQWeight_det_J_0xi(index);
        //ChMatrixNM<double, 3, 3> P_transpose = Smat2*F.transpose();

#if false
        FCol0.noalias() = F_Block.block(0, 3 * index, 3, 1);
        FCol1.noalias() = F_Block.block(0, (3 * index) + 1, 3, 1);
        FCol2.noalias() = F_Block.block(0, (3 * index) + 2, 3, 1);

        //Smat(1, 2) = m_GQWeight_det_J_0xi(index)*D0(3)*(FCol1.transpose()*FCol2);
        //Smat(0, 2) = m_GQWeight_det_J_0xi(index)*D0(4)*(FCol0.transpose()*FCol2);
        //Smat(0, 1) = m_GQWeight_det_J_0xi(index)*D0(5)*(FCol0.transpose()*FCol1);
        //Smat(0, 0) = m_GQWeight_det_J_0xi(index)*D0(0)*(0.5*FCol0.transpose()*FCol0 - 0.5);
        //Smat(1, 1) = m_GQWeight_det_J_0xi(index)*D0(1)*(0.5*FCol1.transpose()*FCol1 - 0.5);
        //Smat(2, 2) = m_GQWeight_det_J_0xi(index)*D0(2)*(0.5*FCol2.transpose()*FCol2 - 0.5);
        SPK2(0) = m_GQWeight_det_J_0xi(index)*D0(0)*(0.5*FCol0.dot(FCol0) - 0.5); //SPK2(0)
        SPK2(1) = m_GQWeight_det_J_0xi(index)*D0(1)*(0.5*FCol1.dot(FCol1) - 0.5); //SPK2(1)
        SPK2(2) = m_GQWeight_det_J_0xi(index)*D0(2)*(0.5*FCol2.dot(FCol2) - 0.5); //SPK2(2)
        SPK2(3) = m_GQWeight_det_J_0xi(index)*D0(3)*FCol1.dot(FCol2); //SPK2(3)
        SPK2(4) = m_GQWeight_det_J_0xi(index)*D0(4)*FCol0.dot(FCol2); //SPK2(4)
        SPK2(5) = m_GQWeight_det_J_0xi(index)*D0(5)*FCol0.dot(FCol1); //SPK2(5)

        P_transpose_row0.noalias() = SPK2(0)*FCol0.transpose() + SPK2(5)*FCol1.transpose() + SPK2(4)*FCol2.transpose();
        P_transpose_row1.noalias() = SPK2(5)*FCol0.transpose() + SPK2(1)*FCol1.transpose() + SPK2(3)*FCol2.transpose();
        P_transpose_row2.noalias() = SPK2(4)*FCol0.transpose() + SPK2(3)*FCol1.transpose() + SPK2(2)*FCol2.transpose();
        P_transpose_scaled_Block.block(3 * index, 0, 1, 3).noalias() = P_transpose_row0;
        P_transpose_scaled_Block.block(3 * index + 1, 0, 1, 3).noalias() = P_transpose_row1;
        P_transpose_scaled_Block.block(3 * index + 2, 0, 1, 3).noalias() = P_transpose_row2;
#else
        FCol0 = F_Block.block(0, 3 * index, 3, 1);
        FCol1 = F_Block.block(0, (3 * index) + 1, 3, 1);
        FCol2 = F_Block.block(0, (3 * index) + 2, 3, 1);
        ...
        //SPK2(0) = m_GQWeight_det_J_0xi(index)*D0(0)*(0.5*FCol0.transpose()*FCol0 - 0.5); //SPK2(0)
        //SPK2(1) = m_GQWeight_det_J_0xi(index)*D0(1)*(0.5*FCol1.transpose()*FCol1 - 0.5); //SPK2(1)
        //SPK2(2) = m_GQWeight_det_J_0xi(index)*D0(2)*(0.5*FCol2.transpose()*FCol2 - 0.5); //SPK2(2)
        //SPK2(3) = m_GQWeight_det_J_0xi(index)*D0(3)*FCol1.transpose()*FCol2; //SPK2(3)
        //SPK2(4) = m_GQWeight_det_J_0xi(index)*D0(4)*FCol0.transpose()*FCol2; //SPK2(4)
        //SPK2(5) = m_GQWeight_det_J_0xi(index)*D0(5)*FCol0.transpose()*FCol1; //SPK2(5)
        ...
        P_transpose_scaled_Block.block(3 * index + 2, 0, 1, 3).noalias() = SPK2(4)*FCol0.transpose() + SPK2(3)*FCol1.transpose() + SPK2(2)*FCol2.transpose();

        //P_transpose_scaled_Block.block(3 * index, 0, 1, 3) = SPK2(0)*FCol0.transpose() + SPK2(5)*FCol1.transpose() + SPK2(4)*FCol2.transpose();
        //P_transpose_scaled_Block.block(3 * index + 1, 0, 1, 3) = SPK2(5)*FCol0.transpose() + SPK2(1)*FCol1.transpose() + SPK2(3)*FCol2.transpose();
        //P_transpose_scaled_Block.block(3 * index + 2, 0, 1, 3) = SPK2(4)*FCol0.transpose() + SPK2(3)*FCol1.transpose() + SPK2(2)*FCol2.transpose();
        ...
        ////Smat(0, 0) = m_GQWeight_det_J_0xi(index)*D0(0)*(0.5*FCol0.dot(FCol0) - 0.5); //SPK2(0)
        ////Smat(1, 1) = m_GQWeight_det_J_0xi(index)*D0(1)*(0.5*FCol1.dot(FCol1) - 0.5); //SPK2(1)
        ////Smat(2, 2) = m_GQWeight_det_J_0xi(index)*D0(2)*(0.5*FCol2.dot(FCol2) - 0.5); //SPK2(2)
        ////Smat(1, 2) = m_GQWeight_det_J_0xi(index)*D0(3)*FCol1.dot(FCol2); //SPK2(3)
        ////Smat(0, 2) = m_GQWeight_det_J_0xi(index)*D0(4)*FCol0.dot(FCol2); //SPK2(4)
        ////Smat(0, 1) = m_GQWeight_det_J_0xi(index)*D0(5)*FCol0.dot(FCol1); //SPK2(5)
        ////Smat(2, 1) = Smat(1, 2);
        ////Smat(2, 0) = Smat(0, 2);
        ////Smat(1, 0) = Smat(0, 1);

        //SCol0(0) = m_GQWeight_det_J_0xi(index)*D0(0)*(0.5*FCol0.dot(FCol0) - 0.5); //SPK2(0)
        //SCol0(1) = m_GQWeight_det_J_0xi(index)*D0(5)*FCol0.dot(FCol1); //SPK2(5)
        //SCol0(2) = m_GQWeight_det_J_0xi(index)*D0(4)*FCol0.dot(FCol2); //SPK2(4)

        //SCol1(0) = SCol0(1);
        //SCol1(1) = m_GQWeight_det_J_0xi(index)*D0(1)*(0.5*FCol1.dot(FCol1) - 0.5); //SPK2(1)
        //SCol1(2) = m_GQWeight_det_J_0xi(index)*D0(3)*FCol1.dot(FCol2); //SPK2(3)

        //SCol2(0) = SCol0(2);
        //SCol2(1) = SCol1(2);
        //SCol2(2) = m_GQWeight_det_J_0xi(index)*D0(2)*(0.5*FCol2.dot(FCol2) - 0.5); //SPK2(2)

        //P_transpose_scaled.noalias() = SCol0*FCol0.transpose() + SCol1*FCol1.transpose() + SCol2*FCol2.transpose();
        //P_transpose_scaled_Block.block(3 * index, 0, 3, 3).noalias() = P_transpose_scaled;
        ////P_transpose_scaled_Block.block(3 * index, 0, 3, 3).noalias() = SCol0*FCol0.transpose() + SCol1*FCol1.transpose() + SCol2*FCol2.transpose();

        //--------------------------------------------------------------------------------
        //FCol0 = F_Block.block(0, 3 * index, 3, 1);
        //FCol1 = F_Block.block(0, (3 * index) + 1, 3, 1);
        //FCol2 = F_Block.block(0, (3 * index) + 2, 3, 1);
        //P_transpose_scaled_Block.block(3 * index, 0, 1, 3).noalias() = m_GQWeight_det_J_0xi(index)*D0(0)*(0.5*FCol0.dot(FCol0) - 0.5)*FCol0.transpose() + m_GQWeight_det_J_0xi(index)*D0(5)*FCol0.dot(FCol1)*FCol1.transpose() + m_GQWeight_det_J_0xi(index)*D0(4)*FCol0.dot(FCol2)*FCol2.transpose();
        //P_transpose_scaled_Block.block(3 * index + 1, 0, 1, 3).noalias() = m_GQWeight_det_J_0xi(index)*D0(5)*FCol0.dot(FCol1)*FCol0.transpose() + m_GQWeight_det_J_0xi(index)*D0(1)*(0.5*FCol1.dot(FCol1) - 0.5)*FCol1.transpose() + m_GQWeight_det_J_0xi(index)*D0(3)*FCol1.dot(FCol2)*FCol2.transpose();
        //P_transpose_scaled_Block.block(3 * index + 2, 0, 1, 3).noalias() = m_GQWeight_det_J_0xi(index)*D0(4)*FCol0.dot(FCol2)*FCol0.transpose() + m_GQWeight_det_J_0xi(index)*D0(3)*FCol1.dot(FCol2)*FCol1.transpose() + m_GQWeight_det_J_0xi(index)*D0(2)*(0.5*FCol2.dot(FCol2) - 0.5)*FCol2.transpose();
    ...
    for (unsigned int index = 16; index < 20; index++) {
        ...
        ////Sdiag.noalias() = Dv*Ediag;

        //Ediag(0) = m_GQWeight_det_J_0xi(index)*(0.5*FCol0.dot(FCol0) - 0.5);
        //Ediag(1) = m_GQWeight_det_J_0xi(index)*(0.5*FCol1.dot(FCol1) - 0.5);
        //Ediag(2) = m_GQWeight_det_J_0xi(index)*(0.5*FCol2.dot(FCol2) - 0.5);
        //Sdiag = Ediag(0)*DvCol0 + Ediag(1)*DvCol1 + Ediag(2)*DvCol2;
        ////Sdiag.noalias() = Dv*Ediag;


        ////Ediag(0) = m_GQWeight_det_J_0xi(index)*(0.5*FCol0.transpose()*FCol0 - 0.5);
        ////Ediag(1) = m_GQWeight_det_J_0xi(index)*(0.5*FCol1.transpose()*FCol1 - 0.5);
        ////Ediag(2) = m_GQWeight_det_J_0xi(index)*(0.5*FCol2.transpose()*FCol2 - 0.5);
        ////Sdiag.noalias() = Dv*Ediag;

        ////Sdiag.noalias() = m_GQWeight_det_J_0xi(index)*(0.5*FCol0.dot(FCol0) - 0.5)*DvCol0 +
        ////    m_GQWeight_det_J_0xi(index)*(0.5*FCol1.dot(FCol1) - 0.5)*DvCol1 +
        ////    m_GQWeight_det_J_0xi(index)*(0.5*FCol2.dot(FCol2) - 0.5)*DvCol2;

        ////P_transpose_row0.noalias() = Sdiag(0)*FCol0.transpose();
        ////P_transpose_row1.noalias() = Sdiag(1)*FCol1.transpose();
        ////P_transpose_row2.noalias() = Sdiag(2)*FCol2.transpose();
        ////P_transpose_scaled_Block.block(3 * index, 0, 1, 3).noalias() = P_transpose_row0;
        ////P_transpose_scaled_Block.block(3 * index + 1, 0, 1, 3).noalias() = P_transpose_row1;
        ////P_transpose_scaled_Block.block(3 * index + 2, 0, 1, 3).noalias() = P_transpose_row2;

        //P_transpose_scaled_Block.block(3 * index, 0, 1, 3).noalias() = Sdiag(0)*FCol0.transpose();
        //P_transpose_scaled_Block.block(3 * index + 1, 0, 1, 3).noalias() = Sdiag(1)*FCol1.transpose();
        //P_transpose_scaled_Block.block(3 * index + 2, 0, 1, 3).noalias() = Sdiag(2)*FCol2.transpose
...
#else
    P_transpose_scaled_Block.block(48, 0, 12, 3).setZero();
#endif // true
...
void ChElementBeamANCF_MT24::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
...
void ChElementBeamANCF_MT24::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    //ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    //ChMatrixNM<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
...
void ChElementBeamANCF_MT24::ComputeInternalForcesSingleGQPnt(ChMatrixNM<double, 9, 3>& QiCompact, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
...
void ChElementBeamANCF_MT24::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChVectorN<double, 6>& D0, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT24::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, unsigned int index, ChMatrix33<double>& Dv, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot) {
...
void ChElementBeamANCF_MT24::ComputeInternalJacobianSingleGQPnt(ChMatrixNM<double, 27, 27>& Jac_Dense, ChMatrixNM<double, 9, 9>& Jac_Compact, double Kfactor, double Rfactor, double GQ_weight, double xi, double eta, double zeta, ChMatrixNM<double, 6, 6>& D, ChMatrixNMc<double, 9, 3>& e_bar, ChMatrixNMc<double, 9, 3>& e_bar_dot){
    ChMatrix33<double> J_0xi;               //Element Jacobian between the reference configuration and normalized configuration
    ChMatrixNMc<double, 9, 3> Sxi_D;         //Matrix of normalized shape function derivatives
    ChMatrixNMc<double, 9, 3> Sxi_D_0xi;     //Matrix of normalized shape function derivatives corrected for a potentially non-straight reference configuration
    ChVectorN<double, 6> epsilon;           //Green-Lagrange strain tensor in Voigt notation
...
void ChElementBeamANCF_MT24::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT24::ComputeInternalJacobiansAnalytic(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> e_bar_dot;
...
void ChElementBeamANCF_MT24::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT24::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT24::Calc_Sxi_D(ChMatrixNMc<double, 9, 3>& Sxi_D, double xi, double eta, double zeta) {
    Sxi_D(0, 0) = xi - 0.5;
...
void ChElementBeamANCF_MT24::CalcCoordMatrix(ChMatrixNMc<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT24::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT24::CalcCoordDerivMatrix(ChMatrixNMc<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT24::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT24::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT24::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrixNMc<double, 9, 3> Sxi_D;
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT24::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT24::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
void ChElementBeamANCF_MT24::EvaluateSectionFrame(const double eta, ChVector<>& point, ChQuaternion<>& rot) {
...
// void ChElementBeamANCF_MT24::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT24::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT24::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT24::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT24::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT24::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT24::ComputeNF(
...
double ChElementBeamANCF_MT24::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT24::ComputeTangent(const double U) {
    ChMatrixNMc<double, 9, 3> e_bar;
    ChMatrixNMc<double, 9, 3> Sxi_D;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT24(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT24::GetStaticGQTables() {
    return &static_tables_MT24;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT24 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT24::ChMaterialBeamANCF_MT24(double rho,        // material density
...
void ChMaterialBeamANCF_MT24::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
