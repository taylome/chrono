...
// =============================================================================
//
// MT03 = MT02 + Dense Math, PK1 Stress, but no pre-computation
...
////
//// What should be done with ChVector<> ChElementBeamANCF_MT03::EvaluateBeamSectionStrains()... It doesn't seems to make sense for this element since there are no input arguments
...
////
//// For ChElementBeamANCF_MT03::ComputeNF, Need to figure out the state update stuff and exactly what Jacobian is being asked for
...
ChElementBeamANCF_MT03::ChElementBeamANCF_MT03() : m_gravity_on(false), m_thicknessY(0), m_thicknessZ(0), m_lenX(0), m_Alpha(0), m_damping_enabled(false){
    m_nodes.resize(3);
}
...
void ChElementBeamANCF_MT03::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementBeamANCF_MT03::SetupInitial(ChSystem* system) {
...
// State update.
void ChElementBeamANCF_MT03::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementBeamANCF_MT03::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementBeamANCF_MT03::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementBeamANCF_MT03::ComputeMmatrixGlobal(ChMatrixRef M) {
    M = m_MassMatrix;
}
...
// -----------------------------------------------------------------------------
void ChElementBeamANCF_MT03::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementBeamANCF_MT03::ComputeNodalMass() {
...
void ChElementBeamANCF_MT03::SetAlphaDamp(double a) {
    m_Alpha = a;
    if (std::abs(m_Alpha) > 1e-10)
        m_damping_enabled = true;
}

void ChElementBeamANCF_MT03::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementBeamANCF_MT03::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    ...
    for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
        for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_eta_zeta].size(); it_eta++) {
            for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_eta_zeta].size(); it_zeta++) {
                ...
                ComputeInternalForcesSingleGQPnt(Qi, xi, eta, zeta, D0, e_bar, e_bar_dot);
    ...
    if (GetStrainFormulation() == ChElementBeamANCF_MT03::StrainFormulation::CMPoisson) {
        for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi].size(); it_xi++) {
            double GQ_weight = GQTable->Weight[GQ_idx_xi][it_xi] * 2 * 2;
            double xi = GQTable->Lroots[GQ_idx_xi][it_xi];
            ChVectorN<double, 27> Qi;
            ComputeInternalForcesSingleGQPnt(Qi, xi, 0, 0, Dv, e_bar, e_bar_dot);
...
void ChElementBeamANCF_MT03::ComputeInternalForcesSingleGQPnt(ChVectorN<double, 27>& Qi, double xi, double eta, double zeta, ChMatrixNM<double,6,6>& D, ChMatrixNM<double, 9, 3>& e_bar, ChMatrixNM<double, 9, 3>& e_bar_dot) {
    
    ChMatrixNM<double, 9, 3> Sxi_D_0xi;
    //ChMatrixNM<double, 9, 3> e_bar;
    ...
    ChMatrix33<double> S;
    ChMatrix33<double> P;
    ChMatrix33<double> I_3x3;
    Calc_Sxi_D_0xi(Sxi_D_0xi, xi, eta, zeta);

    I_3x3.setIdentity();
    F = e_bar.transpose()*Sxi_D_0xi;
    E = 0.5*(F.transpose()*F - I_3x3);
    epsilon(0) = E(0, 0);
    epsilon(1) = E(1, 1);
    epsilon(2) = E(2, 2);
    epsilon(3) = 2.0*E(1, 2);
    ...
    if (m_damping_enabled) {
        ...
        F_dot = e_bar_dot.transpose()*Sxi_D_0xi;
        E_dot = 0.5 * (F_dot.transpose()*F + F.transpose()*F_dot);
        ...
        sigma_PK2 = D*(epsilon + m_Alpha*epsilon_dot);
    }
    else {
        sigma_PK2 = D*epsilon;
    }
    ...
    for (unsigned int i = 0; i < 9; i++) {
        for (unsigned int j = 0; j < 3; j++) {
            Qi(3 * i + j) = P(j, 0)*Sxi_D_0xi(i, 0) + P(j, 1)*Sxi_D_0xi(i, 1) + P(j, 2)*Sxi_D_0xi(i, 2);
        }
    }

    Qi *= -Calc_det_J_0xi(xi, eta, zeta);
}
...
void ChElementBeamANCF_MT03::ComputeInternalJacobians(ChMatrixNM<double, 27, 27>& JacobianMatrix, double Kfactor, double Rfactor) {
...
void ChElementBeamANCF_MT03::Calc_Sxi(ChMatrixNM<double, 3, 27>& Sxi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT03::Calc_Sxi_compact(ChVectorN<double, 9>& Sxi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBeamANCF_MT03::Calc_Sxi_xi(ChMatrixNM<double, 3, 27>& Sxi_xi, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT03::Calc_Sxi_xi_compact(ChVectorN<double, 9>& Sxi_xi_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBeamANCF_MT03::Calc_Sxi_eta(ChMatrixNM<double, 3, 27>& Sxi_eta, double xi, double eta, double zeta) {
...
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT03::Calc_Sxi_eta_compact(ChVectorN<double, 9>& Sxi_eta_compact, double xi, double eta, double zeta) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementBeamANCF_MT03::Calc_Sxi_zeta(ChMatrixNM<double, 3, 27>& Sxi_zeta, double xi, double eta, double zeta) {
...
// 9x1 Vector Form of the partial derivatives of Normalized Shape Functions with respect to eta
// [s1; s2; s3; ...]
void ChElementBeamANCF_MT03::Calc_Sxi_zeta_compact(ChVectorN<double, 9>& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
void ChElementBeamANCF_MT03::Calc_Sxi_D_0xi(ChMatrixNM<double, 9, 3>& Sxi_D_0xi, double xi, double eta, double zeta) {
    ChVectorN<double, 9> Sxi_D_row0;
    ChVectorN<double, 9> Sxi_D_row1;
    ChVectorN<double, 9> Sxi_D_row2;
    ChMatrixNM<double, 3, 9>Sxi_D_Transpose;
    ChMatrix33<double> J_0xi;

    Calc_Sxi_xi_compact(Sxi_D_row0, xi, eta, zeta);
    Calc_Sxi_eta_compact(Sxi_D_row1, xi, eta, zeta);
    Calc_Sxi_zeta_compact(Sxi_D_row2, xi, eta, zeta);
    Calc_J_0xi(J_0xi, xi, eta, zeta);

    Sxi_D_Transpose.row(0) = Sxi_D_row0;
    Sxi_D_Transpose.row(1) = Sxi_D_row1;
    Sxi_D_Transpose.row(2) = Sxi_D_row2;

    Sxi_D_0xi.noalias() = Sxi_D_Transpose.transpose()*J_0xi.inverse();
}
...
void ChElementBeamANCF_MT03::CalcCoordMatrix(ChMatrixNM<double, 9, 3>& e) {
...
void ChElementBeamANCF_MT03::CalcCoordVector(ChVectorN<double, 27>& e) {
...
void ChElementBeamANCF_MT03::CalcCoordDerivMatrix(ChMatrixNM<double, 9, 3>& edot) {
...
void ChElementBeamANCF_MT03::CalcCoordDerivVector(ChVectorN<double, 27>& edot) {
...
void ChElementBeamANCF_MT03::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
...
double ChElementBeamANCF_MT03::Calc_det_J_0xi(double xi, double eta, double zeta) {
    ChMatrix33<double> J_0xi;
    Calc_J_0xi(J_0xi, xi, eta, zeta);
...
// -----------------------------------------------------------------------------
//ChVector<> ChElementBeamANCF_MT03::EvaluateBeamSectionStrains() {
...
//}
//
// void ChElementBeamANCF_MT03::EvaluateSectionDisplacement(const double u,
//                                                    const double v,
//                                                    ChVector<>& u_displ,
//                                                    ChVector<>& u_rotaz) {
...
//}
//
// void ChElementBeamANCF_MT03::EvaluateSectionFrame(const double u,
//                                             const double v,
//                                             ChVector<>& point,
//                                             ChQuaternion<>& rot) {
//    // this is not a corotational element, so just do:
...
//}
//
// void ChElementBeamANCF_MT03::EvaluateSectionPoint(const double u,
//                                             const double v,
//                                             ChVector<>& point) {
//    ChVector<> u_displ;
//
...
void ChElementBeamANCF_MT03::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementBeamANCF_MT03::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementBeamANCF_MT03::LoadableStateIncrement(const unsigned int off_x,
                                               ChState& x_new,
                                               const ChState& x,
...
//void ChElementBeamANCF_MT03::EvaluateSectionVelNorm(double U, ChVector<>& Result) {
//    ShapeVector N;
...
void ChElementBeamANCF_MT03::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementBeamANCF_MT03::ComputeNF(
...
double ChElementBeamANCF_MT03::GetDensity() {
    return GetMaterial()->Get_rho();
}

// Calculate tangent to the centerline at (U) coordinates.
ChVector<> ChElementBeamANCF_MT03::ComputeTangent(const double U) {
    ChMatrixNM<double, 3, 27> Sxi_xi;
    ChVectorN<double, 27> e;
    ChVector<> r_xi;
...
ChQuadratureTables static_tables_MT03(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementBeamANCF_MT03::GetStaticGQTables() {
    return &static_tables_MT03;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialBeamANCF_MT03 methods
// ============================================================================

// Construct an isotropic material.
ChMaterialBeamANCF_MT03::ChMaterialBeamANCF_MT03(double rho,        // material density
...
void ChMaterialBeamANCF_MT03::Calc_D0_Dv(const ChVector<>& E, const ChVector<>& nu, const ChVector<>& G, double k1, double k2) {
