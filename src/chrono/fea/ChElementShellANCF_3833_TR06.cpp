...
// =============================================================================
// Higher order ANCF shell element with 8 nodes. Description of this element (3833_TR06) and its internal forces may be
...
// Constructor
// ------------------------------------------------------------------------------
ChElementShellANCF_3833_TR06::ChElementShellANCF_3833_TR06()
...
void ChElementShellANCF_3833_TR06::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementShellANCF_3833_TR06::AddLayer(double thickness,
                                            double theta,
...
void ChElementShellANCF_3833_TR06::SetDimensions(double lenX, double lenY) {
    m_lenX = lenX;
    m_lenY = lenY;
}
...
void ChElementShellANCF_3833_TR06::SetMidsurfaceOffset(const double offset) {
    m_midsurfoffset = offset;
}

// Set the value for the single term structural damping coefficient.

void ChElementShellANCF_3833_TR06::SetAlphaDamp(double a) {
    m_Alpha = a;
...
ChMatrix33<> ChElementShellANCF_3833_TR06::GetGreenLagrangeStrain(const double xi,
                                                                  const double eta,
                                                                  const double zeta) {
...
ChMatrix33<> ChElementShellANCF_3833_TR06::GetPK2Stress(const double layer,
                                                        const double xi,
                                                        const double eta,
...
double ChElementShellANCF_3833_TR06::GetVonMissesStress(const double layer,
                                                        const double xi,
                                                        const double eta,
                                                        const double layer_zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
...
void ChElementShellANCF_3833_TR06::SetupInitial(ChSystem* system) {
...
void ChElementShellANCF_3833_TR06::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3833_TR06::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementShellANCF_3833_TR06::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementShellANCF_3833_TR06::ComputeNodalMass() {
    m_nodes[0]->m_TotalMass += m_MassMatrix(0, 0) + m_MassMatrix(0, 3) + m_MassMatrix(0, 6) + m_MassMatrix(0, 9) +
                               m_MassMatrix(0, 12) + m_MassMatrix(0, 15) + m_MassMatrix(0, 18) + m_MassMatrix(0, 21);
    m_nodes[1]->m_TotalMass += m_MassMatrix(3, 0) + m_MassMatrix(3, 3) + m_MassMatrix(3, 6) + m_MassMatrix(3, 9) +
                               m_MassMatrix(3, 12) + m_MassMatrix(3, 15) + m_MassMatrix(3, 18) + m_MassMatrix(3, 21);
    m_nodes[2]->m_TotalMass += m_MassMatrix(6, 0) + m_MassMatrix(6, 3) + m_MassMatrix(6, 6) + m_MassMatrix(6, 9) +
                               m_MassMatrix(6, 12) + m_MassMatrix(6, 15) + m_MassMatrix(6, 18) + m_MassMatrix(6, 21);
    m_nodes[3]->m_TotalMass += m_MassMatrix(9, 0) + m_MassMatrix(9, 3) + m_MassMatrix(9, 6) + m_MassMatrix(9, 9) +
                               m_MassMatrix(9, 12) + m_MassMatrix(9, 15) + m_MassMatrix(9, 18) + m_MassMatrix(9, 21);
    m_nodes[4]->m_TotalMass += m_MassMatrix(12, 0) + m_MassMatrix(12, 3) + m_MassMatrix(12, 6) + m_MassMatrix(12, 9) +
                               m_MassMatrix(12, 12) + m_MassMatrix(12, 15) + m_MassMatrix(12, 18) +
                               m_MassMatrix(12, 21);
    m_nodes[5]->m_TotalMass += m_MassMatrix(15, 0) + m_MassMatrix(15, 3) + m_MassMatrix(15, 6) + m_MassMatrix(15, 9) +
                               m_MassMatrix(15, 12) + m_MassMatrix(15, 15) + m_MassMatrix(15, 18) +
                               m_MassMatrix(15, 21);
    m_nodes[6]->m_TotalMass += m_MassMatrix(18, 0) + m_MassMatrix(18, 3) + m_MassMatrix(18, 6) + m_MassMatrix(18, 9) +
                               m_MassMatrix(56, 12) + m_MassMatrix(18, 15) + m_MassMatrix(18, 18) +
                               m_MassMatrix(18, 21);
    m_nodes[7]->m_TotalMass += m_MassMatrix(21, 0) + m_MassMatrix(21, 3) + m_MassMatrix(21, 6) + m_MassMatrix(21, 9) +
                               m_MassMatrix(0, 12) + m_MassMatrix(21, 15) + m_MassMatrix(21, 18) + m_MassMatrix(21, 21);
}

// Compute the generalized internal force vector for the current nodal coordinates and set the value in the Fi vector.

void ChElementShellANCF_3833_TR06::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (unsigned int GQpnt = 0; GQpnt < NIP; GQpnt++) {
            MatrixNx3c Sbar_xi_D = m_SD.block<NSF, 3>(0, 3 * (GQpnt + kl * NIP));
            ...
            // derivative of the Green-Lagrange strain tensor in Voigt notation and then multiply it by the element
            // Jacobian
            ChVectorN<double, 6> epsilon_combined;
            ...
            epsilon_combined *= m_kGQ(GQpnt + kl * NIP);
...
void ChElementShellANCF_3833_TR06::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                            double Kfactor,
                                                            double Rfactor,
                                                            double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (unsigned int GQpnt = 0; GQpnt < NIP; GQpnt++) {
            ...
            epsilon_combined(5) =
                F.col(0).dot(F.col(1)) + m_Alpha * (F.col(0).dot(Fdot.col(1)) + Fdot.col(0).dot(F.col(1)));
            epsilon_combined *= Kfactor * m_kGQ(GQpnt + kl * NIP);
...
void ChElementShellANCF_3833_TR06::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementShellANCF_3833_TR06::EvaluateSectionFrame(const double xi,
                                                        const double eta,
                                                        ChVector<>& point,
                                                        ChQuaternion<>& rot) {
    VectorN Sxi_compact;
...
void ChElementShellANCF_3833_TR06::EvaluateSectionPoint(const double xi, const double eta, ChVector<>& point) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR06::EvaluateSectionVelNorm(const double xi, const double eta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, 0, m_thicknessZ, m_midsurfoffset);
...
void ChElementShellANCF_3833_TR06::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3833_TR06::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3833_TR06::LoadableStateIncrement(const unsigned int off_x,
                                                          ChState& x_new,
                                                          const ChState& x,
...
void ChElementShellANCF_3833_TR06::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3833_TR06::ComputeNF(
...
double ChElementShellANCF_3833_TR06::GetDensity() {
...
ChVector<> ChElementShellANCF_3833_TR06::ComputeNormal(const double xi, const double eta) {
...
void ChElementShellANCF_3833_TR06::ComputeMassMatrixAndGravityForce() {
...
void ChElementShellANCF_3833_TR06::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementShellANCF_3833_TR06::Calc_Sxi_compact(VectorN& Sxi_compact,
                                                    double xi,
                                                    double eta,
                                                    double zeta,
                                                    double thickness,
...
void ChElementShellANCF_3833_TR06::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact,
                                                       double xi,
                                                       double eta,
                                                       double zeta,
                                                       double thickness,
...
void ChElementShellANCF_3833_TR06::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact,
                                                        double xi,
                                                        double eta,
                                                        double zeta,
                                                        double thickness,
...
void ChElementShellANCF_3833_TR06::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact,
                                                         double xi,
                                                         double eta,
                                                         double zeta,
                                                         double thickness,
...
void ChElementShellANCF_3833_TR06::Calc_Sxi_D(MatrixNx3c& Sxi_D,
                                              double xi,
                                              double eta,
                                              double zeta,
                                              double thickness,
...
void ChElementShellANCF_3833_TR06::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3833_TR06::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3833_TR06::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3833_TR06::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3833_TR06::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementShellANCF_3833_TR06::Calc_J_0xi(ChMatrix33<double>& J_0xi,
                                              double xi,
                                              double eta,
                                              double zeta,
                                              double thickness,
...
double ChElementShellANCF_3833_TR06::Calc_det_J_0xi(double xi,
                                                    double eta,
                                                    double zeta,
                                                    double thickness,
                                                    double zoffset) {
...
void ChElementShellANCF_3833_TR06::RotateReorderStiffnessMatrix(ChMatrixNM<double, 6, 6>& D, double theta) {
...
ChQuadratureTables static_tables_3833_TR06(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3833_TR06::GetStaticGQTables() {
    return &static_tables_3833_TR06;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3833_TR06::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)

ChElementShellANCF_3833_TR06::Layer::Layer(double thickness,
                                           double theta,
                                           std::shared_ptr<ChMaterialShellANCF> material)
