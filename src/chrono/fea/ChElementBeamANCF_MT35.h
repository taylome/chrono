...
#ifndef CHELEMENTBEAMANCFMT35_H
#define CHELEMENTBEAMANCFMT35_H
...
class ChApi ChMaterialBeamANCF_MT35 {
  public:
    /// Construct an isotropic material.
    ChMaterialBeamANCF_MT35(double rho,        ///< material density
...
class ChApi ChElementBeamANCF_MT35 : public ChElementBeam, public ChLoadableU, public ChLoadableUVW {
  public:
    ...
    void SetMaterial(std::shared_ptr<ChMaterialBeamANCF_MT35> beam_mat) { m_material = beam_mat; }
    ...
    /// Return the material.
    std::shared_ptr<ChMaterialBeamANCF_MT35> GetMaterial() const { return m_material; }
  ...
  public:
    ...
    void ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const ChMatrixNMc<double, 9, 6>& ebar_ebardot);
    void ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi, const ChMatrixNMc<double, 9, 3>& e_bar);
  ...
  private:
    ...
    void ComputeInternalJacobianNoDamping(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor);
    ...
    // Calculate the current 9x3 matrix of nodal coordinates.
    void CalcCombinedCoordMatrix(ChMatrixNMc<double, 9, 6>& ebar_ebardot);
    ...
    std::shared_ptr<ChMaterialBeamANCF_MT35> m_material;    ///< beam material
    ...
    ChMatrixNMc<double, 9, 60> m_SD_precompute;             ///< Precomputed corrected normalized shape function derivative matrices for no Poisson Effect followed by Poisson Effect on the beam axis only
    ChMatrixNMc<double, 9, 60> m_SD_precompute_col_ordered; ///< Precomputed corrected normalized shape function derivative matrices for no Poisson Effect followed by Poisson Effect on the beam axis only in column by column order
    ChMatrixNMc<double, 60, 3> m_F_Transpose_CombinedBlock_col_ordered;
    ChMatrixNMc<double, 60, 6> m_F_Transpose_CombinedBlockDamping_col_ordered;
    ChVectorN<double, 16> m_SPK2_0_D0_Block;
    ChVectorN<double, 16> m_SPK2_1_D0_Block;
    ChVectorN<double, 16> m_SPK2_2_D0_Block;
    ChVectorN<double, 16> m_SPK2_3_D0_Block;
    ChVectorN<double, 16> m_SPK2_4_D0_Block;
    ChVectorN<double, 16> m_SPK2_5_D0_Block;
    ChVectorN<double, 4> m_Sdiag_0_Dv_Block;
    ChVectorN<double, 4> m_Sdiag_1_Dv_Block;
    ChVectorN<double, 4> m_Sdiag_2_Dv_Block;
    ChVectorN<double, 20> m_GQWeight_det_J_0xi;             ///< Precomputed Gauss-Quadrature Weight & Element Jacobian scale factors for no Poisson Effect followed by Poisson Effect on the beam axis only

    ChMatrixNMc<double, 9, 48> m_SD_precompute_D0;          ///< Precomputed corrected normalized shape function derivative matrices for no Poisson Effect
    ChMatrixNMc<double, 9, 16> m_SD_precompute_D0_col0_block;          ///< Precomputed corrected normalized shape function derivative matrices for no Poisson Effect (matrix of just the 1st columns)
    ChMatrixNMc<double, 9, 16> m_SD_precompute_D0_col1_block;          ///< Precomputed corrected normalized shape function derivative matrices for no Poisson Effect (matrix of just the 2nd columns)
    ChMatrixNMc<double, 9, 16> m_SD_precompute_D0_col2_block;          ///< Precomputed corrected normalized shape function derivative matrices for no Poisson Effect (matrix of just the 3rd columns)    
    ...
    ChMatrixNMc<double, 9, 12> m_SD_precompute_Dv;           ///< Precomputed corrected normalized shape function derivative matrices for Poisson Effect on the beam axis only
    ChMatrixNMc<double, 9, 4> m_SD_precompute_Dv_col0_block;           ///< Precomputed corrected normalized shape function derivative matrices for Poisson Effect on the beam axis only (matrix of just the 1st columns)
	ChMatrixNMc<double, 9, 4> m_SD_precompute_Dv_col1_block;           ///< Precomputed corrected normalized shape function derivative matrices for Poisson Effect on the beam axis only (matrix of just the 2nd columns)
	ChMatrixNMc<double, 9, 4> m_SD_precompute_Dv_col2_block;           ///< Precomputed corrected normalized shape function derivative matrices for Poisson Effect on the beam axis only (matrix of just the 3rd columns)
