...
ChElementShellANCF_3443ML_TR08S::ChElementShellANCF_3443ML_TR08S()
...
void ChElementShellANCF_3443ML_TR08S::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementShellANCF_3443ML_TR08S::AddLayer(double thickness, double theta, std::shared_ptr<ChMaterialShellANCF_3443ML_TR08S> material) {
...
void ChElementShellANCF_3443ML_TR08S::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementShellANCF_3443ML_TR08S::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementShellANCF_3443ML_TR08S::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3443ML_TR08S::ComputeKRMmatricesGlobal(ChMatrixRef H,
                                                           double Kfactor,
                                                           double Rfactor,
                                                           double Mfactor) {
...
void ChElementShellANCF_3443ML_TR08S::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementShellANCF_3443ML_TR08S::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementShellANCF_3443ML_TR08S::ComputeNodalMass() {
...
void ChElementShellANCF_3443ML_TR08S::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementShellANCF_3443ML_TR08S::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementShellANCF_3443ML_TR08S::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3443ML_TR08S::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const MatrixNx6& ebar_ebardot) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ChMatrixNMc<double, 216, 6>  F_Transpose_CombinedBlockDamping_col_ordered = m_SD_precompute_col_ordered.block<16, 216>(0, 216 * kl).transpose() * ebar_ebardot;

        VectorNIP E0_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0));
        E0_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1));
        E0_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2));
        E0_Block.array() -= 1;
        E0_Block *= 0.5;
        VectorNIP E_BlockDamping = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 5));
        E0_Block += m_Alpha * E_BlockDamping;
        E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E1_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0));
        E1_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1));
        E1_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2));
        E1_Block.array() -= 1;
        E1_Block *= 0.5;
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 5));
        E1_Block += m_Alpha * E_BlockDamping;
        E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E2_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
        E2_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
        E2_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2));
        E2_Block.array() -= 1;
        E2_Block *= 0.5;
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 5));
        E2_Block += m_Alpha * E_BlockDamping;
        E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E3_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
        E3_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
        E3_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2));
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
        ...
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 5));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(
        ...
        VectorNIP E4_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0));
        E4_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1));
        E4_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2));
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(
        ...
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 5));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 4));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
        ...
        VectorNIP E5_Block = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0));
        E5_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1));
        E5_Block += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2));
        E_BlockDamping.noalias() = F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 3));
        E_BlockDamping += F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(
        ...
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 0) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 0) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 1) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 1) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 2) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 2) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(SPK2_4_Block);

        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 0) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 0) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 1) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 1) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 2) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 2) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(SPK2_3_Block);

        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 0) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 0) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 1) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 1) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 2) =
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 2) +=
            F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).cwiseProduct(SPK2_2_Block);
...
void ChElementShellANCF_3443ML_TR08S::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ChMatrixNMc<double, 216, 3>  F_Transpose_CombinedBlock_col_ordered = m_SD_precompute_col_ordered.block<16, 216>(0, 216 * kl).transpose() * e_bar;

        VectorNIP E0_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0));
        E0_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1));
        E0_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2));
        E0_Block.array() -= 1;
        E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E0_Block *= 0.5;

        VectorNIP E1_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0));
        E1_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1));
        E1_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2));
        E1_Block.array() -= 1;
        E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E1_Block *= 0.5;

        VectorNIP E2_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0));
        E2_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1));
        E2_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2));
        E2_Block.array() -= 1;
        E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        E2_Block *= 0.5;

        VectorNIP E3_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0));
        E3_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1));
        E3_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2));
        E3_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E4_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0));
        E4_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1));
        E4_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2));
        E4_Block.array() *= m_GQWeight_det_J_0xi_D.array();

        VectorNIP E5_Block = F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0));
        E5_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1));
        E5_Block += F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2));
        E5_Block.array() *= m_GQWeight_det_J_0xi_D.array();
        ...
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 0) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 0) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 1) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 1) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).cwiseProduct(SPK2_4_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 2) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(SPK2_0_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).cwiseProduct(SPK2_5_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(0, 2) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).cwiseProduct(SPK2_4_Block);

        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 0) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 0) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 1) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 1) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 2) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(SPK2_5_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).cwiseProduct(SPK2_1_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(72, 2) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).cwiseProduct(SPK2_3_Block);

        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 0) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 0) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 1) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 1) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).cwiseProduct(SPK2_2_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 2) =
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).cwiseProduct(SPK2_4_Block) +
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).cwiseProduct(SPK2_3_Block);
        P_transpose_scaled_Block_col_ordered.block<72, 1>(144, 2) +=
            F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).cwiseProduct(SPK2_2_Block);
...
void ChElementShellANCF_3443ML_TR08S::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
    double Kfactor,
    double Rfactor) {
...
void ChElementShellANCF_3443ML_TR08S::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (auto i = 0; i < 16; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(144, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlockDamping_col_ordered.block<72, 1>(72, 2).transpose());
        }
        ...
        //scaled_F_Transpose_col_ordered.noalias() = (Kfactor + m_Alpha * Rfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<216, 3>(0, 0) +
        //    (m_Alpha * Kfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<216, 3>(0, 3);
        ChMatrixNMc<double, 216, 3> scaled_F_Transpose_col_ordered = (Kfactor + m_Alpha * Rfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<216, 3>(0, 0) +
                (m_Alpha * Kfactor) * F_Transpose_CombinedBlockDamping_col_ordered.block<216, 3>(0, 3);
        ...
        for (auto i = 0; i < 16; i++) {
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 0).transpose()) +
            ...
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 0).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 0).transpose());

            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 1).transpose()) +
            ...
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 1).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 1).transpose());

            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(72, 2).transpose()) +
            ...
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(144, 2).transpose());
            Scaled_Combined_partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(scaled_F_Transpose_col_ordered.block<72, 1>(0, 2).transpose()) +
        ...
        for (auto i = 0; i < 16; i++) {
            S_scaled_SD_precompute_col_ordered.block<1, 72>(i, 0) =
                SPK2_0_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0)) +
                SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72)) +
                SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144));

            S_scaled_SD_precompute_col_ordered.block<1, 72>(i, 72) =
                SPK2_5_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0)) +
                SPK2_1_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72)) +
                SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144));

            S_scaled_SD_precompute_col_ordered.block<1, 72>(i, 144) =
                SPK2_4_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0)) +
                SPK2_3_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72)) +
                SPK2_2_Block.transpose().cwiseProduct(m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144));
        }
...
void ChElementShellANCF_3443ML_TR08S::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        for (auto i = 0; i < 16; i++) {
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 0).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>(3 * i, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 0).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 0).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 1).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 1, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 1).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 1).transpose());

            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 0) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 72) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 144) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 216) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 288) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 144).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(144, 2).transpose());
            partial_epsilon_partial_e_Transpose.block<1, 72>((3 * i) + 2, 360) = m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 72).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(0, 2).transpose()) +
                m_SD_precompute_col_ordered.block<1, 72>(i, 216 * kl + 0).cwiseProduct(F_Transpose_CombinedBlock_col_ordered.block<72, 1>(72, 2).transpose());
        }
        ...
        ChMatrixNMc<double, 216, 3> scaled_F_Transpose_col_ordered = Kfactor * F_Transpose_CombinedBlock_col_ordered;
...
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta, double thickness, double zoffset) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta, thickness, zoffset);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3443ML_TR08S::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3443ML_TR08S::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3443ML_TR08S::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3443ML_TR08S::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementShellANCF_3443ML_TR08S::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
    ebar.row(3) = m_nodes[0]->GetDDD().eigen();
...
void ChElementShellANCF_3443ML_TR08S::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3443ML_TR08S::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3443ML_TR08S::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementShellANCF_3443ML_TR08S::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementShellANCF_3443ML_TR08S::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta, double thickness, double zoffset) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta, thickness, zoffset);
...
double ChElementShellANCF_3443ML_TR08S::Calc_det_J_0xi(double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3443ML_TR08S::EvaluateSectionFrame(const double xi, const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    Matrix3x3N Sxi;
    Matrix3x3N Sxi_xi;
    Matrix3x3N Sxi_eta;
...
void ChElementShellANCF_3443ML_TR08S::EvaluateSectionPoint(const double u, const double v, ChVector<>& point) {
    Matrix3x3N Sxi;
...
void ChElementShellANCF_3443ML_TR08S::EvaluateSectionVelNorm(double U, double V, ChVector<>& Result) {
    Matrix3x3N Sxi_zeta;
    Vector3N edot;
    ChVector<> r_zeta;
...
void ChElementShellANCF_3443ML_TR08S::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3443ML_TR08S::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3443ML_TR08S::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementShellANCF_3443ML_TR08S::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3443ML_TR08S::ComputeNF(
...
double ChElementShellANCF_3443ML_TR08S::GetDensity() {
...
ChVector<> ChElementShellANCF_3443ML_TR08S::ComputeNormal(const double U, const double V) {
    Matrix3x3N Sxi_zeta;
    Vector3N e;
    ChVector<> r_zeta;
...
ChQuadratureTables static_tables_3443ML_TR08S(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3443ML_TR08S::GetStaticGQTables() {
    return &static_tables_3443ML_TR08S;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3443ML_TR08S::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)
ChElementShellANCF_3443ML_TR08S::Layer::Layer(ChElementShellANCF_3443ML_TR08S* element,
    double thickness,
    double theta,
    std::shared_ptr<ChMaterialShellANCF_3443ML_TR08S> material)
    : m_element(element), m_thickness(thickness), m_theta(theta), m_material(material) {}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialShellANCF_3443ML_TR08S methods
// ============================================================================

// Construct an isotropic material.
ChMaterialShellANCF_3443ML_TR08S::ChMaterialShellANCF_3443ML_TR08S(
...
// Poisson effect
void ChMaterialShellANCF_3443ML_TR08S::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G) {
...
void ChMaterialShellANCF_3443ML_TR08S::Get_Rotated_D(ChMatrixNM<double, 6, 6>& D, double theta) {
