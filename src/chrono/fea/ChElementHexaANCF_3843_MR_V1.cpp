...
ChElementHexaANCF_3843_MR_V1::ChElementHexaANCF_3843_MR_V1()
...
void ChElementHexaANCF_3843_MR_V1::SetNodes(std::shared_ptr<ChNodeFEAxyzDDD> nodeA,
...
void ChElementHexaANCF_3843_MR_V1::SetDimensions(double lenX, double lenY, double lenZ) {
    m_lenX = lenX;
    m_lenY = lenY;
    m_lenZ = lenZ;
...
void ChElementHexaANCF_3843_MR_V1::SetMaterial(std::shared_ptr<ChMaterialHexaANCF_MR> brick_mat) {
...
ChMatrix33<> ChElementHexaANCF_3843_MR_V1::GetGreenLagrangeStrain(const double xi, const double eta, const double zeta) {
...
ChMatrix33<> ChElementHexaANCF_3843_MR_V1::GetPK2Stress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementHexaANCF_3843_MR_V1::GetVonMissesStress(const double xi, const double eta, const double zeta) {
    MatrixNx3c Sxi_D;  // Matrix of normalized shape function derivatives
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
void ChElementHexaANCF_3843_MR_V1::SetupInitial(ChSystem* system) {
...
void ChElementHexaANCF_3843_MR_V1::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementHexaANCF_3843_MR_V1::Update() {
    ChElementGeneric::Update();
}

// Return the mass matrix in full sparse form.

void ChElementHexaANCF_3843_MR_V1::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
void ChElementHexaANCF_3843_MR_V1::ComputeNodalMass() {
...
void ChElementHexaANCF_3843_MR_V1::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementHexaANCF_3843_MR_V1::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, Matrix3xN& ebar, Matrix3xN& ebardot) {
    ...
    for (unsigned int GQpnt = 0; GQpnt < NIP; GQpnt++) {
        ...
        ChMatrix33<> C = F.transpose()*F;
        //ChMatrixNM<double, 3, 3> C = F.transpose()*F;

        //Calculate the 1st invariant of the Right Cauchy-Green deformation tensor: trace(C)
        double I1 = C(0, 0) + C(1, 1) + C(2, 2);
        //std::cout <<"I1: " << I1 << "  C(0, 0): " << C(0, 0) << "  C(1, 1): " << C(1, 1) << "  C(2, 2): " << C(2, 2) << std::endl;
        ...
        //std::cout << "I2: " << I2 << "  C(0, 1): " << C(0, 1) << "  C(0, 2): " << C(0, 2) << "  C(1, 2): " << C(1, 2) << std::endl;
        ...
        MatrixNx3 dI1_de_T = Sbar_xi_D.col(0)*(2 * F.col(0).transpose()) +
                             Sbar_xi_D.col(1)*(2 * F.col(1).transpose()) +
                             Sbar_xi_D.col(2)*(2 * F.col(2).transpose());

        MatrixNx3 dI2_de_T = Sbar_xi_D.col(0)*(2 * I1*F.col(0).transpose() -2*C.row(0)*F.transpose()) + 
                             Sbar_xi_D.col(1)*(2 * I1*F.col(1).transpose() -2*C.row(1)*F.transpose()) + 
                             Sbar_xi_D.col(2)*(2 * I1*F.col(2).transpose() -2*C.row(2)*F.transpose());
        ...
        MatrixNx3 ddetF_de_T = Sbar_xi_D.col(0)*detF_S0 + 
                               Sbar_xi_D.col(1)*detF_S1 + 
                               Sbar_xi_D.col(2)*detF_S2;
        MatrixNx3 dI1bar_de_T = std::pow(detF,-2.0/3.0)*dI1_de_T - 2.0 / 3.0 * std::pow(detF,-5.0/3.0)*I1*ddetF_de_T;
        MatrixNx3 dI2bar_de_T = std::pow(detF,-4.0/3.0)*dI2_de_T - 4.0 / 3.0 * std::pow(detF,-7.0/3.0)*I2*ddetF_de_T;

        if (mu == 0) {
        //--------------------------------------------------------------------------------
        // 2 term Mooney-Rivlin Material Only
        //--------------------------------------------------------------------------------
            QiCompact.noalias() += m_kGQ(GQpnt) * (c10 * dI1bar_de_T + c01 * dI2bar_de_T + k * (detF - 1)*ddetF_de_T);
        }
        else{
        //--------------------------------------------------------------------------------
        ...
        //--------------------------------------------------------------------------------
            ...
            ChMatrix33<> S_PK2NLKV = detF * mu*C.inverse()*Edot*C.inverse();

            QiCompact.noalias() += m_kGQ(GQpnt) * (c10 * dI1bar_de_T + c01 * dI2bar_de_T + k * (detF - 1)*ddetF_de_T);
            QiCompact.noalias() += m_kGQ(GQpnt) * S_PK2NLKV(0, 0)*Sbar_xi_D.col(0)*F.col(0).transpose();
            QiCompact.noalias() += m_kGQ(GQpnt) * S_PK2NLKV(1, 1)*Sbar_xi_D.col(1)*F.col(1).transpose();
            QiCompact.noalias() += m_kGQ(GQpnt) * S_PK2NLKV(2, 2)*Sbar_xi_D.col(2)*F.col(2).transpose();
            QiCompact.noalias() += m_kGQ(GQpnt) * S_PK2NLKV(1, 2)*(Sbar_xi_D.col(2)*F.col(1).transpose() + Sbar_xi_D.col(1)*F.col(2).transpose());
            QiCompact.noalias() += m_kGQ(GQpnt) * S_PK2NLKV(0, 2)*(Sbar_xi_D.col(2)*F.col(0).transpose() + Sbar_xi_D.col(0)*F.col(2).transpose());
            QiCompact.noalias() += m_kGQ(GQpnt) * S_PK2NLKV(0, 1)*(Sbar_xi_D.col(1)*F.col(0).transpose() + Sbar_xi_D.col(0)*F.col(1).transpose());
        }
    }
...
void ChElementHexaANCF_3843_MR_V1::ComputeKRMmatricesGlobal(ChMatrixRef H, double Kfactor, double Rfactor, double Mfactor) {
...
void ChElementHexaANCF_3843_MR_V1::ComputeGravityForces(ChVectorDynamic<>& Fg, const ChVector<>& G_acc) {
...
void ChElementHexaANCF_3843_MR_V1::EvaluateElementFrame(const double xi,
                                                  const double eta,
                                                  const double zeta,
                                                  ChVector<>& point,
                                                  ChQuaternion<>& rot) {
...
void ChElementHexaANCF_3843_MR_V1::EvaluateElementPoint(const double xi,
                                                  const double eta,
                                                  const double zeta,
                                                  ChVector<>& point) {
    VectorN Sxi_compact;
...
void ChElementHexaANCF_3843_MR_V1::EvaluateElementVel(double xi, double eta, const double zeta, ChVector<>& Result) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta);
...
void ChElementHexaANCF_3843_MR_V1::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementHexaANCF_3843_MR_V1::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementHexaANCF_3843_MR_V1::LoadableStateIncrement(const unsigned int off_x,
                                                    ChState& x_new,
                                                    const ChState& x,
...
void ChElementHexaANCF_3843_MR_V1::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementHexaANCF_3843_MR_V1::ComputeNF(
...
double ChElementHexaANCF_3843_MR_V1::GetDensity() {
...
void ChElementHexaANCF_3843_MR_V1::ComputeMassMatrixAndGravityForce() {
...
void ChElementHexaANCF_3843_MR_V1::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
...
void ChElementHexaANCF_3843_MR_V1::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta) {
    Sxi_compact(0) =
...
void ChElementHexaANCF_3843_MR_V1::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta) {
...
void ChElementHexaANCF_3843_MR_V1::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta) {
...
void ChElementHexaANCF_3843_MR_V1::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta) {
...
void ChElementHexaANCF_3843_MR_V1::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta) {
    VectorN Sxi_D_col;
...
void ChElementHexaANCF_3843_MR_V1::CalcCoordVector(Vector3N& e) {
...
void ChElementHexaANCF_3843_MR_V1::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementHexaANCF_3843_MR_V1::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementHexaANCF_3843_MR_V1::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementHexaANCF_3843_MR_V1::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementHexaANCF_3843_MR_V1::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta);
...
double ChElementHexaANCF_3843_MR_V1::Calc_det_J_0xi(double xi, double eta, double zeta) {
...
ChQuadratureTables static_tables_3843_MR_V1(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementHexaANCF_3843_MR_V1::GetStaticGQTables() {
    return &static_tables_3843_MR_V1;
}
