...
// =============================================================================
// TR07B = a Gerstmayr style implementation of the element with pre-calculation
...
//
// =============================================================================
// Multi-Layer Shell Implementation - All GQ Points at Once implementation
...
ChElementShellANCF_3833ML_TR07B::ChElementShellANCF_3833ML_TR07B()
...
void ChElementShellANCF_3833ML_TR07B::SetNodes(std::shared_ptr<ChNodeFEAxyzDD> nodeA,
...
void ChElementShellANCF_3833ML_TR07B::AddLayer(double thickness, double theta, std::shared_ptr<ChMaterialShellANCF_3833ML_TR07B> material) {
...
void ChElementShellANCF_3833ML_TR07B::SetupInitial(ChSystem* system) {
    // Initial positions and slopes of the element nodes
    // These values define the reference configuration of the element
    CalcCoordMatrix(m_ebar0);
...
// State update.
void ChElementShellANCF_3833ML_TR07B::Update() {
    ChElementGeneric::Update();
}

// Fill the D vector with the current field values at the element nodes.
void ChElementShellANCF_3833ML_TR07B::GetStateBlock(ChVectorDynamic<>& mD) {
...
void ChElementShellANCF_3833ML_TR07B::ComputeKRMmatricesGlobal(ChMatrixRef H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {
...
void ChElementShellANCF_3833ML_TR07B::ComputeMmatrixGlobal(ChMatrixRef M) {
    M.setZero();
...
// -----------------------------------------------------------------------------
void ChElementShellANCF_3833ML_TR07B::ComputeMassMatrixAndGravityForce(const ChVector<>& g_acc) {
...
void ChElementShellANCF_3833ML_TR07B::ComputeNodalMass() {
...
void ChElementShellANCF_3833ML_TR07B::PrecomputeInternalForceMatricesWeights() {
    ChQuadratureTables* GQTable = GetStaticGQTables();
    ...
    SD_precompute_D.resize(24, 3 * m_numGQPnts);
        ...
        for (unsigned int it_xi = 0; it_xi < GQTable->Lroots[GQ_idx_xi_eta_zeta].size(); it_xi++) {
            for (unsigned int it_eta = 0; it_eta < GQTable->Lroots[GQ_idx_xi_eta_zeta].size(); it_eta++) {
                for (unsigned int it_zeta = 0; it_zeta < GQTable->Lroots[GQ_idx_xi_eta_zeta].size(); it_zeta++) {
                    ...
                    SD_precompute_D.block(0, 3 * index, 24, 3) = Sxi_D * J_0xi.inverse();
...
void ChElementShellANCF_3833ML_TR07B::SetAlphaDamp(double a) {
    m_Alpha = a;
    m_2Alpha = 2 * a;
...
void ChElementShellANCF_3833ML_TR07B::ComputeInternalForces(ChVectorDynamic<>& Fi) {
...
void ChElementShellANCF_3833ML_TR07B::ComputeInternalForcesAtState(ChVectorDynamic<>& Fi, const MatrixNx6& ebar_ebardot) {
    ...
    E0_Block.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1));
    E0_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1));
    E0_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1));
    E0_Block.array() -= 1;
    E0_Block *= 0.5;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E_BlockDamping;
    E_BlockDamping.resize(m_numGQPnts, 1);
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 3, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 4, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 5, m_numGQPnts, 1));
    ...
    E1_Block.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1));
    E1_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1));
    E1_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1));
    E1_Block.array() -= 1;
    E1_Block *= 0.5;
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 3, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 4, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 5, m_numGQPnts, 1));
    ...
    E2_Block.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1));
    E2_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1));
    E2_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1));
    E2_Block.array() -= 1;
    E2_Block *= 0.5;
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 3, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 4, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 5, m_numGQPnts, 1));
    ...
    E3_Block.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1));
    E3_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1));
    E3_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1));
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 3, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
    ...
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 5, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 3, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 4, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
    ...
    E4_Block.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1));
    E4_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1));
    E4_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1));
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 3, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
    ...
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 5, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 3, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 4, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(
    ...
    E5_Block.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1));
    E5_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1));
    E5_Block += m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1));
    E_BlockDamping.noalias() = m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 3, m_numGQPnts, 1));
    E_BlockDamping += m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
    ...
    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ...
        m_SPK2_0_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) = D(0, 0)*E0_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(0, 1)*E1_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(0, 2)*E2_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(0, 3)*E3_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(0, 4)*E4_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(0, 5)*E5_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1);
        m_SPK2_1_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) = D(1, 0)*E0_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(1, 1)*E1_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(1, 2)*E2_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(1, 3)*E3_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(1, 4)*E4_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(1, 5)*E5_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1);
        m_SPK2_2_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) = D(2, 0)*E0_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(2, 1)*E1_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(2, 2)*E2_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(2, 3)*E3_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(2, 4)*E4_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(2, 5)*E5_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1);
        m_SPK2_3_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) = D(3, 0)*E0_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(3, 1)*E1_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(3, 2)*E2_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(3, 3)*E3_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(3, 4)*E4_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(3, 5)*E5_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1);
        m_SPK2_4_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) = D(4, 0)*E0_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(4, 1)*E1_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(4, 2)*E2_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(4, 3)*E3_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(4, 4)*E4_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(4, 5)*E5_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1);
        m_SPK2_5_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) = D(5, 0)*E0_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(5, 1)*E1_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(5, 2)*E2_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(5, 3)*E3_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(5, 4)*E4_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1) + D(5, 5)*E5_Block.block(kl*m_numLayerGQPnts, 0, m_numLayerGQPnts, 1);
    }
    ...
    P_transpose_scaled_Block_col_ordered.block(0, 0, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 0, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 1, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 1, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 2, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 2, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block);

    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);

    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_2_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_2_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlockDamping_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_2_Block);
...
void ChElementShellANCF_3833ML_TR07B::ComputeInternalForcesAtStateNoDamping(ChVectorDynamic<>& Fi,
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E0_Block;
    E0_Block.resize(m_numGQPnts, 1);
    E0_Block.noalias() = m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1));
    E0_Block += m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1));
    E0_Block += m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1));
    E0_Block.array() -= 1;
    E0_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E0_Block *= 0.5;
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E1_Block;
    E1_Block.resize(m_numGQPnts, 1);
    E1_Block.noalias() = m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1));
    E1_Block += m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1));
    E1_Block += m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1));
    E1_Block.array() -= 1;
    E1_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E1_Block *= 0.5;
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E2_Block;
    E2_Block.resize(m_numGQPnts, 1);
    E2_Block.noalias() = m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1));
    E2_Block += m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1));
    E2_Block += m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1));
    E2_Block.array() -= 1;
    E2_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    E2_Block *= 0.5;
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E3_Block;
    E3_Block.resize(m_numGQPnts, 1);
    E3_Block.noalias() = m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1));
    E3_Block += m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1));
    E3_Block += m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1));
    E3_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E4_Block;
    E4_Block.resize(m_numGQPnts, 1);
    E4_Block.noalias() = m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1));
    E4_Block += m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1));
    E4_Block += m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1));
    E4_Block.array() *= m_GQWeight_det_J_0xi_D.array();

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> E5_Block;
    E5_Block.resize(m_numGQPnts, 1);
    E5_Block.noalias() = m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1));
    E5_Block += m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1));
    E5_Block += m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1));
    E5_Block.array() *= m_GQWeight_det_J_0xi_D.array();
    ...
    P_transpose_scaled_Block_col_ordered.block(0, 0, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 0, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 1, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 1, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 2, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_0_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block);
    P_transpose_scaled_Block_col_ordered.block(0, 2, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block);

    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_5_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_1_Block);
    P_transpose_scaled_Block_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);

    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).cwiseProduct(m_SPK2_2_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).cwiseProduct(m_SPK2_2_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1) =
        m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_4_Block) +
        m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_3_Block);
    P_transpose_scaled_Block_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1) +=
        m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).cwiseProduct(m_SPK2_2_Block);
...
void ChElementShellANCF_3833ML_TR07B::ComputeInternalJacobians(Matrix3Nx3N& JacobianMatrix,
                                                           double Kfactor,
                                                           double Rfactor) {
...
void ChElementShellANCF_3833ML_TR07B::ComputeInternalJacobianDamping(ChMatrixRef& H,
    double Kfactor,
    double Rfactor,
    double Mfactor) {

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> partial_epsilon_partial_e_Transpose;
    partial_epsilon_partial_e_Transpose.resize(72, 6 * m_numGQPnts);
    ...
    Scaled_Combined_partial_epsilon_partial_e_Transpose.resize(72, 6 * m_numGQPnts);
    ...
    DScaled_Combined_partial_epsilon_partial_e_Transpose.resize(72, 6 * m_numGQPnts);

    for (size_t kl = 0; kl < m_numLayers; kl++) {
        ChMatrixNM<double, 6, 6> D;
        m_layers[kl].GetMaterial()->Get_Rotated_D(D, m_layers[kl].Get_theta());

        DScaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts, 72, m_numLayerGQPnts) = D(0, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts, 72, m_numLayerGQPnts) + D(0, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + m_numGQPnts, 72, m_numLayerGQPnts) + D(0, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 2 * m_numGQPnts, 72, m_numLayerGQPnts) + D(0, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 3 * m_numGQPnts, 72, m_numLayerGQPnts) + D(0, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 4 * m_numGQPnts, 72, m_numLayerGQPnts) + D(0, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 5 * m_numGQPnts, 72, m_numLayerGQPnts);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + m_numGQPnts, 72, m_numLayerGQPnts) = D(1, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts, 72, m_numLayerGQPnts) + D(1, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + m_numGQPnts, 72, m_numLayerGQPnts) + D(1, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 2 * m_numGQPnts, 72, m_numLayerGQPnts) + D(1, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 3 * m_numGQPnts, 72, m_numLayerGQPnts) + D(1, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 4 * m_numGQPnts, 72, m_numLayerGQPnts) + D(1, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 5 * m_numGQPnts, 72, m_numLayerGQPnts);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 2 * m_numGQPnts, 72, m_numLayerGQPnts) = D(2, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts, 72, m_numLayerGQPnts) + D(2, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + m_numGQPnts, 72, m_numLayerGQPnts) + D(2, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 2 * m_numGQPnts, 72, m_numLayerGQPnts) + D(2, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 3 * m_numGQPnts, 72, m_numLayerGQPnts) + D(2, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 4 * m_numGQPnts, 72, m_numLayerGQPnts) + D(2, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 5 * m_numGQPnts, 72, m_numLayerGQPnts);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 3 * m_numGQPnts, 72, m_numLayerGQPnts) = D(3, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts, 72, m_numLayerGQPnts) + D(3, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + m_numGQPnts, 72, m_numLayerGQPnts) + D(3, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 2 * m_numGQPnts, 72, m_numLayerGQPnts) + D(3, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 3 * m_numGQPnts, 72, m_numLayerGQPnts) + D(3, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 4 * m_numGQPnts, 72, m_numLayerGQPnts) + D(3, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 5 * m_numGQPnts, 72, m_numLayerGQPnts);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 4 * m_numGQPnts, 72, m_numLayerGQPnts) = D(4, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts, 72, m_numLayerGQPnts) + D(4, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + m_numGQPnts, 72, m_numLayerGQPnts) + D(4, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 2 * m_numGQPnts, 72, m_numLayerGQPnts) + D(4, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 3 * m_numGQPnts, 72, m_numLayerGQPnts) + D(4, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 4 * m_numGQPnts, 72, m_numLayerGQPnts) + D(4, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 5 * m_numGQPnts, 72, m_numLayerGQPnts);
        DScaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 5 * m_numGQPnts, 72, m_numLayerGQPnts) = D(5, 0)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts, 72, m_numLayerGQPnts) + D(5, 1)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + m_numGQPnts, 72, m_numLayerGQPnts) + D(5, 2)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 2 * m_numGQPnts, 72, m_numLayerGQPnts) + D(5, 3)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 3 * m_numGQPnts, 72, m_numLayerGQPnts) + D(5, 4)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 4 * m_numGQPnts, 72, m_numLayerGQPnts) + D(5, 5)*Scaled_Combined_partial_epsilon_partial_e_Transpose.block(0, kl*m_numLayerGQPnts + 5 * m_numGQPnts, 72, m_numLayerGQPnts);
    }
...
void ChElementShellANCF_3833ML_TR07B::ComputeInternalJacobianNoDamping(ChMatrixRef& H, double Kfactor, double Mfactor) {

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> partial_epsilon_partial_e_Transpose;
    partial_epsilon_partial_e_Transpose.resize(72, 6 * m_numGQPnts);

    for (auto i = 0; i < 24; i++) {
        partial_epsilon_partial_e_Transpose.block(3 * i, 0, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block(3 * i, m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block(3 * i, 2 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block(3 * i, 3 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block(3 * i, 4 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 0, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block(3 * i, 5 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 0, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 0, m_numGQPnts, 1).transpose());

        partial_epsilon_partial_e_Transpose.block((3 * i) + 1, 0, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 1, m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 1, 2 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 1, 3 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 1, 4 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 1, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 1, 5 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 1, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 1, m_numGQPnts, 1).transpose());

        partial_epsilon_partial_e_Transpose.block((3 * i) + 2, 0, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 2, m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 2, 2 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 2, 3 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 2, 4 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, 2 * m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(2 * m_numGQPnts, 2, m_numGQPnts, 1).transpose());
        partial_epsilon_partial_e_Transpose.block((3 * i) + 2, 5 * m_numGQPnts, 1, m_numGQPnts).noalias() = m_SD_precompute_col_ordered.block(i, m_numGQPnts, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(0, 2, m_numGQPnts, 1).transpose()) +
            m_SD_precompute_col_ordered.block(i, 0, 1, m_numGQPnts).cwiseProduct(m_F_Transpose_CombinedBlock_col_ordered.block(m_numGQPnts, 2, m_numGQPnts, 1).transpose());
    }
...
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi(Matrix3x3N& Sxi, double xi, double eta, double zeta, double thickness, double zoffset) {
    VectorN Sxi_compact;
    Calc_Sxi_compact(Sxi_compact, xi, eta, zeta, thickness, zoffset);
    Sxi.setZero();
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi_compact(VectorN& Sxi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi_xi(Matrix3x3N& Sxi_xi, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi_xi_compact(VectorN& Sxi_xi_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi_eta(Matrix3x3N& Sxi_eta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi_eta_compact(VectorN& Sxi_eta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1*I_3x3, s2*I_3x3, s3*I_3x3, ...]
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi_zeta(Matrix3x3N& Sxi_zeta, double xi, double eta, double zeta, double thickness, double zoffset) {
...
// [s1; s2; s3; ...]
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi_zeta_compact(VectorN& Sxi_zeta_compact, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR07B::Calc_Sxi_D(MatrixNx3c& Sxi_D, double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR07B::CalcCoordVector(Vector3N& e) {
...
void ChElementShellANCF_3833ML_TR07B::CalcCoordMatrix(Matrix3xN& ebar) {
...
void ChElementShellANCF_3833ML_TR07B::CalcCoordMatrix(MatrixNx3c& ebar) {
...
void ChElementShellANCF_3833ML_TR07B::CalcCoordMatrix(MatrixNx3& ebar) {
    ebar.row(0) = m_nodes[0]->GetPos().eigen();
    ebar.row(1) = m_nodes[0]->GetD().eigen();
    ebar.row(2) = m_nodes[0]->GetDD().eigen();
...
void ChElementShellANCF_3833ML_TR07B::CalcCoordDerivVector(Vector3N& edot) {
...
void ChElementShellANCF_3833ML_TR07B::CalcCoordDerivMatrix(Matrix3xN& ebardot) {
...
void ChElementShellANCF_3833ML_TR07B::CalcCoordDerivMatrix(MatrixNx3c& ebardot) {
...
void ChElementShellANCF_3833ML_TR07B::CalcCombinedCoordMatrix(MatrixNx6& ebar_ebardot) {
...
void ChElementShellANCF_3833ML_TR07B::Calc_J_0xi(ChMatrix33<double>& J_0xi, double xi, double eta, double zeta, double thickness, double zoffset) {
    MatrixNx3c Sxi_D;
    Calc_Sxi_D(Sxi_D, xi, eta, zeta, thickness, zoffset);
...
double ChElementShellANCF_3833ML_TR07B::Calc_det_J_0xi(double xi, double eta, double zeta, double thickness, double zoffset) {
...
void ChElementShellANCF_3833ML_TR07B::EvaluateSectionFrame(const double xi, const double eta, ChVector<>& point, ChQuaternion<>& rot) {
    Matrix3x3N Sxi;
    Matrix3x3N Sxi_xi;
    Matrix3x3N Sxi_eta;
...
void ChElementShellANCF_3833ML_TR07B::EvaluateSectionPoint(const double u, const double v, ChVector<>& point) {
    Matrix3x3N Sxi;
...
void ChElementShellANCF_3833ML_TR07B::EvaluateSectionVelNorm(double U, double V, ChVector<>& Result) {
    Matrix3x3N Sxi_zeta;
    Vector3N edot;
    ChVector<> r_zeta;
...
void ChElementShellANCF_3833ML_TR07B::LoadableGetStateBlock_x(int block_offset, ChState& mD) {
...
void ChElementShellANCF_3833ML_TR07B::LoadableGetStateBlock_w(int block_offset, ChStateDelta& mD) {
...
void ChElementShellANCF_3833ML_TR07B::LoadableStateIncrement(const unsigned int off_x,
                                                         ChState& x_new,
                                                         const ChState& x,
...
void ChElementShellANCF_3833ML_TR07B::LoadableGetVariables(std::vector<ChVariables*>& mvars) {
...
void ChElementShellANCF_3833ML_TR07B::ComputeNF(
...
double ChElementShellANCF_3833ML_TR07B::GetDensity() {
...
ChVector<> ChElementShellANCF_3833ML_TR07B::ComputeNormal(const double U, const double V) {
    Matrix3x3N Sxi_zeta;
    Vector3N e;
    ChVector<> r_zeta;
...
ChQuadratureTables static_tables_3833ML_TR07B(1, CH_QUADRATURE_STATIC_TABLES);
//#endif // !CH_QUADRATURE_STATIC_TABLES

ChQuadratureTables* ChElementShellANCF_3833ML_TR07B::GetStaticGQTables() {
    return &static_tables_3833ML_TR07B;
}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChElementShellANCF_3833ML_TR07B::Layer methods
// ============================================================================

// Private constructor (a layer can be created only by adding it to an element)
ChElementShellANCF_3833ML_TR07B::Layer::Layer(ChElementShellANCF_3833ML_TR07B* element,
    double thickness,
    double theta,
    std::shared_ptr<ChMaterialShellANCF_3833ML_TR07B> material)
    : m_element(element), m_thickness(thickness), m_theta(theta), m_material(material) {}

////////////////////////////////////////////////////////////////

// ============================================================================
// Implementation of ChMaterialShellANCF_3833ML_TR07B methods
// ============================================================================

// Construct an isotropic material.
ChMaterialShellANCF_3833ML_TR07B::ChMaterialShellANCF_3833ML_TR07B(
...
// Poisson effect
void ChMaterialShellANCF_3833ML_TR07B::Calc_D0_Dv(const ChVector<>& E,
                                              const ChVector<>& nu,
                                              const ChVector<>& G) {
...
void ChMaterialShellANCF_3833ML_TR07B::Get_Rotated_D(ChMatrixNM<double, 6, 6>& D, double theta) {
